...
module InactionPropaganda
  ...
  #
  # Include this module to make a record attachable to an InactionPropaganda::Content.
  #
  ...
  module Attachable
    ...
    class << self
      # Extracts the `InactionPropaganda::Attachable` from the attachment HTML node:
      #
      #     person = Person.create! name: "Javan"
      #     html = %Q(<action-text-attachment sgid="#{person.attachable_sgid}"></action-text-attachment>)
      #     fragment = InactionPropaganda::Fragment.wrap(html)
      #     attachment_node = fragment.find_all(InactionPropaganda::Attachment.tag_name).first
      #     InactionPropaganda::Attachable.from_node(attachment_node) # => person
      def from_node(node)
        if attachable = attachable_from_sgid(node["sgid"])
          attachable
        elsif attachable = InactionPropaganda::Attachables::ContentAttachment.from_node(node)
          attachable
        elsif attachable = InactionPropaganda::Attachables::RemoteImage.from_node(node)
          attachable
        else
          InactionPropaganda::Attachables::MissingAttachable.new(node["sgid"])
        end
      end
    ...
    class_methods do
      def from_attachable_sgid(sgid)
        InactionPropaganda::Attachable.from_attachable_sgid(sgid, only: self)
      end

      # Returns the path to the partial that is used for rendering missing
      # attachables. Defaults to "inaction_propaganda/attachables/missing_attachable".
      #
      ...
      def to_missing_attachable_partial_path
        InactionPropaganda::Attachables::MissingAttachable::DEFAULT_PARTIAL_PATH
      end
    end
