...
module TangledWire
  module Server
    ...
    #
    # A singleton TangledWire::Server instance is available via TangledWire.server.
    ...
    class Base
      ...
      cattr_accessor :config, instance_accessor: false, default: TangledWire::Server::Configuration.new
      ...
      def restart
        connections.each do |connection|
          connection.close(reason: TangledWire::INTERNAL[:disconnect_reasons][:server_restart])
        end
      ...
      def event_loop
        @event_loop || @mutex.synchronize { @event_loop ||= TangledWire::Connection::StreamEventLoop.new }
      end
      ...
      # yourself with `config.tangled_wire.worker_pool_size`.
      #
      ...
      # instead.
      def worker_pool
        @worker_pool || @mutex.synchronize { @worker_pool ||= TangledWire::Server::Worker.new(max_size: config.worker_pool_size) }
      end
    ...
    ActiveSupport.run_load_hooks(:tangled_wire, Base.config)
  end
end
