    ...
    class YAMLColumn < ColumnSerializer # :nodoc:
      class SafeCoder
        ...
        if Gem::Version.new(Psych::VERSION) >= Gem::Version.new("5.1")
          def dump(object)
            if @unsafe_load.nil? ? PassiveAggressive.use_yaml_unsafe_load : @unsafe_load
              ::YAML.dump(object)
            else
              ::YAML.safe_dump(
                object,
                permitted_classes: @permitted_classes + PassiveAggressive.yaml_column_permitted_classes,
                aliases: true,
              )
            end
          end
