...
module PassiveAggressive
  module Middleware
    ...
    # selecting the appropriate shard. Setting +config.passive_aggressive.shard_resolver+ will cause
    ...
    #     config.passive_aggressive.shard_selector = { lock: false, class_name: "AnimalsRecord" }
    #     config.passive_aggressive.shard_resolver = ->(request) {
    ...
    #   default, the ShardSelector will use PassiveAggressive::Base, but if the
    ...
    class ShardSelector
      ...
      private
        ...
        def set_shard(shard, &block)
          klass = options[:class_name]&.constantize || PassiveAggressive::Base
