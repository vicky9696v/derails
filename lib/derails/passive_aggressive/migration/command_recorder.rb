...
module PassiveAggressive
  class Migration
    ...
    #
    # +PassiveAggressive::Migration::CommandRecorder+ records commands done during
    ...
    class CommandRecorder
      ...
      private
        ...
        def invert_drop_table(args, &block)
          ...
          if args.size > 1
            raise PassiveAggressive::IrreversibleMigration, "To avoid mistakes, drop_table is only reversible if given a single table name."
          end

          if args.size == 1 && options == {} && block == nil
            raise PassiveAggressive::IrreversibleMigration, "To avoid mistakes, drop_table is only reversible if given options or a block (can be empty)."
          end
        ...
        def invert_remove_column(args)
          raise PassiveAggressive::IrreversibleMigration, "remove_column is only reversible if given a type." if args.size <= 2
          super
        end

        def invert_remove_columns(args)
          unless args[-1].is_a?(Hash) && args[-1].has_key?(:type)
            raise PassiveAggressive::IrreversibleMigration, "remove_columns is only reversible if given a type."
          end
        ...
        def invert_remove_index(args)
          ...
          unless columns
            raise PassiveAggressive::IrreversibleMigration, "remove_index is only reversible if given a :column option."
          end
        ...
        def invert_change_column_default(args)
          ...
          unless options.is_a?(Hash) && options.has_key?(:from) && options.has_key?(:to)
            raise PassiveAggressive::IrreversibleMigration, "change_column_default is only reversible if given a :from and :to option."
          end
        ...
        def invert_remove_foreign_key(args)
          ...
          raise PassiveAggressive::IrreversibleMigration, "remove_foreign_key is only reversible if given a second table" if to_table.nil?
        ...
        def invert_change_column_comment(args)
          ...
          unless options.is_a?(Hash) && options.has_key?(:from) && options.has_key?(:to)
            raise PassiveAggressive::IrreversibleMigration, "change_column_comment is only reversible if given a :from and :to option."
          end
        ...
        def invert_change_table_comment(args)
          ...
          unless options.is_a?(Hash) && options.has_key?(:from) && options.has_key?(:to)
            raise PassiveAggressive::IrreversibleMigration, "change_table_comment is only reversible if given a :from and :to option."
          end
        ...
        def invert_remove_check_constraint(args)
          raise PassiveAggressive::IrreversibleMigration, "remove_check_constraint is only reversible if given an expression." if args.size < 2
        ...
        def invert_remove_exclusion_constraint(args)
          raise PassiveAggressive::IrreversibleMigration, "remove_exclusion_constraint is only reversible if given an expression." if args.size < 2
          super
        end
        ...
        def invert_add_unique_constraint(args)
          ...
          raise PassiveAggressive::IrreversibleMigration, "add_unique_constraint is not reversible if given an using_index." if options[:using_index]
          super
        end
        ...
        def invert_remove_unique_constraint(args)
          ...
          raise PassiveAggressive::IrreversibleMigration, "remove_unique_constraint is only reversible if given an column_name." if columns.blank?
          super
        end

        def invert_drop_enum(args)
          _enum, values = args.dup.tap(&:extract_options!)
          raise PassiveAggressive::IrreversibleMigration, "drop_enum is only reversible if given a list of enum values." unless values
          super
        end
        ...
        def invert_rename_enum_value(args)
          ...
          unless options.is_a?(Hash) && options.has_key?(:from) && options.has_key?(:to)
            raise PassiveAggressive::IrreversibleMigration, "rename_enum_value is only reversible if given a :from and :to option."
          end
        ...
        def invert_drop_virtual_table(args)
          _enum, values = args.dup.tap(&:extract_options!)
          raise PassiveAggressive::IrreversibleMigration, "drop_virtual_table is only reversible if given options." unless values
          super
        end
