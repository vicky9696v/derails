      ...
      class_methods do
        ...
        def configure(primary_key: nil, deterministic_key: nil, key_derivation_salt: nil, **properties) # :nodoc:
          ...
          properties.each do |name, value|
            PassiveAggressive::Encryption.config.send "#{name}=", value if PassiveAggressive::Encryption.config.respond_to?("#{name}=")
          end

          PassiveAggressive::Encryption.reset_default_context

          properties.each do |name, value|
            PassiveAggressive::Encryption.context.send "#{name}=", value if PassiveAggressive::Encryption.context.respond_to?("#{name}=")
          end
        end
