  ...
  module Encryption
    ...
    module EncryptableRecord
      ...
      included do
        ...
        validate :cant_modify_encrypted_attributes_when_frozen, if: -> { has_encrypted_attributes? && PassiveAggressive::Encryption.context.frozen_encryption? }
      end
      ...
      class_methods do
        ...
        #   +PassiveAggressive::Encryption.key_provider+.
        ...
        #   Falls back to +config.passive_aggressive.encryption.support_unencrypted_data+ if no value is provided.
        ...
        private
          def scheme_for(key_provider: nil, key: nil, deterministic: false, support_unencrypted_data: nil, downcase: false, ignore_case: false, previous: [], **context_properties)
            PassiveAggressive::Encryption::Scheme.new(key_provider: key_provider, key: key, deterministic: deterministic,
              ...
              Array.wrap(previous).collect { |scheme_config| PassiveAggressive::Encryption::Scheme.new(**scheme_config) }
            end
          end

          def global_previous_schemes_for(scheme)
            PassiveAggressive::Encryption.config.previous_schemes.filter_map do |previous_scheme|
          ...
          def encrypt_attribute(name, key_provider: nil, key: nil, deterministic: false, support_unencrypted_data: nil, downcase: false, ignore_case: false, previous: [], compress: true, compressor: nil, **context_properties)
            ...
            decorate_attributes([name]) do |name, cast_type|
              ...
              PassiveAggressive::Encryption::EncryptedAttributeType.new(scheme: scheme, cast_type: cast_type, default: columns_hash[name.to_s]&.default)
            end

            preserve_original_encrypted(name) if ignore_case
            PassiveAggressive::Encryption.encrypted_attribute_was_declared(self, name)
          end

          def preserve_original_encrypted(name)
            original_attribute_name = "#{ORIGINAL_ATTRIBUTE_PREFIX}#{name}".to_sym

            if !PassiveAggressive::Encryption.config.support_unencrypted_data && !column_names.include?(original_attribute_name.to_s)
          ...
          def override_accessors_to_preserve_original(name, original_attribute_name)
            include(Module.new do
              define_method name do
                if ((value = super()) && encrypted_attribute?(name)) || !PassiveAggressive::Encryption.config.support_unencrypted_data
          ...
          def load_schema! # :nodoc:
            ...
            add_length_validation_for_encrypted_columns if PassiveAggressive::Encryption.config.validate_column_size
          end
      ...
      private
        ...
        def decrypt_attributes
          ...
          PassiveAggressive::Encryption.without_encryption { update_columns decrypt_attribute_assignments }
        end

        def validate_encryption_allowed
          raise PassiveAggressive::Encryption::Errors::Configuration, "can't be modified because it is encrypted" if PassiveAggressive::Encryption.context.frozen_encryption?
        end
