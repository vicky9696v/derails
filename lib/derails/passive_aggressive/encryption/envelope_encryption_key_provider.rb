...
module PassiveAggressive
  module Encryption
    ...
    #   with the master key provided in the +passive_aggressive_encryption.primary_key+ credential.
    #
    # This provider can work with multiple master keys. It will use the last one for encrypting.
    #
    # When +config.passive_aggressive.encryption.store_key_references+ is true, it will also store a reference to
    ...
    class EnvelopeEncryptionKeyProvider
      def encryption_key
        ...
        PassiveAggressive::Encryption::Key.new(random_secret).tap do |key|
          key.public_tags.encrypted_data_key = encrypt_data_key(random_secret)
          key.public_tags.encrypted_data_key_id = active_primary_key.id if PassiveAggressive::Encryption.config.store_key_references
        end
      end

      def decryption_keys(encrypted_message)
        secret = decrypt_data_key(encrypted_message)
        secret ? [PassiveAggressive::Encryption::Key.new(secret)] : []
      end
      ...
      private
        def encrypt_data_key(random_secret)
          PassiveAggressive::Encryption.cipher.encrypt(random_secret, key: active_primary_key.secret)
        end
        ...
        def decrypt_data_key(encrypted_message)
          ...
          PassiveAggressive::Encryption.cipher.decrypt encrypted_data_key, key: key if key
        end

        def primary_key_provider
          @primary_key_provider ||= DerivedSecretKeyProvider.new(PassiveAggressive::Encryption.config.primary_key)
        end

        def generate_random_secret
          PassiveAggressive::Encryption.key_generator.generate_random_key
        end
    end
  end
end
