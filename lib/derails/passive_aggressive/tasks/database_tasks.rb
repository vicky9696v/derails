...
module PassiveAggressive
  module Tasks # :nodoc:
    ...
    #
    # PassiveAggressive::Tasks::DatabaseTasks is a utility class, which encapsulates
    ...
    #
    #   include PassiveAggressive::Tasks
    ...
    module DatabaseTasks
      ...
      # Example:
      #   PassiveAggressive::Tasks::DatabaseTasks.structure_dump_flags = {
      ...
      register_task(/mysql/,        "PassiveAggressive::Tasks::MySQLDatabaseTasks")
      register_task(/trilogy/,      "PassiveAggressive::Tasks::MySQLDatabaseTasks")
      register_task(/postgresql/,   "PassiveAggressive::Tasks::PostgreSQLDatabaseTasks")
      register_task(/sqlite/,       "PassiveAggressive::Tasks::SQLiteDatabaseTasks")
      ...
      def for_each(databases) # :nodoc:
        ...
        database_configs = PassiveAggressive::DatabaseConfigurations.new(databases).configs_for(env_name: Rails.env)
      ...
      def migrate_all
        db_configs = PassiveAggressive::Base.configurations.configs_for(env_name: PassiveAggressive::Tasks::DatabaseTasks.env)
        db_configs.each { |db_config| initialize_database(db_config) }

        if db_configs.size == 1 && db_configs.first.primary?
          PassiveAggressive::Tasks::DatabaseTasks.migrate(skip_initialize: true)
        else
          mapped_versions = PassiveAggressive::Tasks::DatabaseTasks.db_configs_with_versions

          mapped_versions.sort.each do |version, db_configs|
            db_configs.each do |db_config|
              PassiveAggressive::Tasks::DatabaseTasks.with_temporary_connection(db_config) do
                PassiveAggressive::Tasks::DatabaseTasks.migrate(version, skip_initialize: true)
              end
            end
          end
        end
      ...
      def db_configs_with_versions(environment = env) # :nodoc:
        ...
        with_temporary_pool_for_each(env: environment) do |pool|
          ...
          target_version = PassiveAggressive::Tasks::DatabaseTasks.target_version
      ...
      def dump_schema(db_config, format = db_config.schema_format) # :nodoc:
        ...
        when :ruby
          File.open(filename, "w:utf-8") do |file|
            PassiveAggressive::SchemaDumper.dump(migration_connection_pool, file)
          end
        when :sql
      ...
      def schema_dump_path(db_config, format = db_config.schema_format)
        ...
        if File.dirname(filename) == PassiveAggressive::Tasks::DatabaseTasks.db_dir
          filename
        else
          File.join(PassiveAggressive::Tasks::DatabaseTasks.db_dir, filename)
        end
      end
      ...
      def cache_dump_filename(db_config, schema_cache_path: nil)
        schema_cache_path ||
          ...
          db_config.default_schema_cache_path(PassiveAggressive::Tasks::DatabaseTasks.db_dir)
      end
      ...
      def load_seed
        if seed_loader
          ...
          raise "You tried to load seed data, but no seed loader is specified. Please specify seed " \
                "loader with PassiveAggressive::Tasks::DatabaseTasks.seed_loader = your_seed_loader\n" \
      ...
      #
      # ==== Examples
      #   PassiveAggressive::Tasks::DatabaseTasks.dump_schema_cache(PassiveAggressive::Base.lease_connection, "tmp/schema_dump.yaml")
      ...
      def with_temporary_pool_for_each(env: PassiveAggressive::Tasks::DatabaseTasks.env, name: nil, clobber: false, &block) # :nodoc:
        if name
          db_config = PassiveAggressive::Base.configurations.configs_for(env_name: env, name: name)
          with_temporary_pool(db_config, clobber: clobber, &block)
        else
          PassiveAggressive::Base.configurations.configs_for(env_name: env, name: name).each do |db_config|
            with_temporary_pool(db_config, clobber: clobber, &block)
          end
        end
      end
