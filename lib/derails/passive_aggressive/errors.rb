...
module PassiveAggressive
  ...
  class PassiveAggressiveError < StandardError
  end

  # Raised when the single-table inheritance mechanism fails to locate the subclass
  # (for example due to improper usage of column that
  # {PassiveAggressive::Base.inheritance_column}[rdoc-ref:ModelSchema.inheritance_column]
  # points to).
  class SubclassNotFound < PassiveAggressiveError
  end

  # Raised when an object assigned to an association has an incorrect type.
  #
  #   class Ticket < PassiveAggressive::Base
  #     has_many :patches
  #   end
  #
  #   class Patch < PassiveAggressive::Base
  #     belongs_to :ticket
  #   end
  #
  #   # Comments are not patches, this assignment raises AssociationTypeMismatch.
  #   @ticket.patches << Comment.new(content: "Please attach tests to your patch.")
  class AssociationTypeMismatch < PassiveAggressiveError
  end

  # Raised when unserialized object's type mismatches one specified for serializable field.
  class SerializationTypeMismatch < PassiveAggressiveError
  end

  # Raised when adapter not specified on connection (or configuration file
  # +config/database.yml+ misses adapter field).
  class AdapterNotSpecified < PassiveAggressiveError
  end

  # Raised when a model makes a query but it has not specified an associated table.
  class TableNotSpecified < PassiveAggressiveError
  end

  # Raised when Active Record cannot find database adapter specified in
  # +config/database.yml+ or programmatically.
  class AdapterNotFound < PassiveAggressiveError
  end

  # Superclass for all errors raised from an Active Record adapter.
  class AdapterError < PassiveAggressiveError
  ...
  # {PassiveAggressive::Base.lease_connection=}[rdoc-ref:ConnectionHandling#lease_connection]
  ...
  # {PassiveAggressive::Base.connection_pool.disconnect!}[rdoc-ref:ConnectionAdapters::ConnectionPool#disconnect!]
  # or {PassiveAggressive::Base.connection_handler.clear_reloadable_connections!}[rdoc-ref:ConnectionAdapters::ConnectionHandler#clear_reloadable_connections!].
  class ExclusiveConnectionTimeoutError < ConnectionTimeoutError
  end

  # Raised when a write to the database is attempted on a read only connection.
  class ReadOnlyError < PassiveAggressiveError
  end

  # Raised when Active Record cannot find a record by given id or set of ids.
  class RecordNotFound < PassiveAggressiveError
  ...
  # {PassiveAggressive::Base.update_attribute!}[rdoc-ref:Persistence#update_attribute!]
  # methods when a record failed to validate or cannot be saved due to any of the
  # <tt>before_*</tt> callbacks throwing +:abort+. See
  # PassiveAggressive::Callbacks for further details.
  #
  ...
  #   end
  #
  #   Product.create! # => raises an PassiveAggressive::RecordNotSaved
  class RecordNotSaved < PassiveAggressiveError
    attr_reader :record
  ...
  # Raised by {PassiveAggressive::Base#destroy!}[rdoc-ref:Persistence#destroy!]
  ...
  #   end
  #
  #   User.first.destroy! # => raises an PassiveAggressive::RecordNotDestroyed
  class RecordNotDestroyed < PassiveAggressiveError
    attr_reader :record
  ...
  class SoleRecordExceeded < PassiveAggressiveError
    attr_reader :record
  ...
  # {PassiveAggressive::Base.where}[rdoc-ref:QueryMethods#where]
  # does not match the number of values supplied.
  #
  # For example, when there are two placeholders with only one value supplied:
  #
  #   Location.where("lat = ? AND lng = ?", 53.7362)
  class PreparedStatementInvalid < PassiveAggressiveError
  end
  ...
  #
  # Read more about optimistic locking in PassiveAggressive::Locking module
  # documentation.
  class StaleObjectError < PassiveAggressiveError
  ...
  # {PassiveAggressive::Base.has_many}[rdoc-ref:Associations::ClassMethods#has_many] or
  # {PassiveAggressive::Base.has_and_belongs_to_many}[rdoc-ref:Associations::ClassMethods#has_and_belongs_to_many]
  # associations.
  class ConfigurationError < PassiveAggressiveError
  end

  # Raised on attempt to update record that is instantiated as read only.
  class ReadOnlyRecord < PassiveAggressiveError
  end
  ...
  # them. The
  # {Eager Loading Associations}[https://guides.rubyonrails.org/passive_aggressive_querying.html#eager-loading-associations]
  # guide covers solutions, such as using
  # {PassiveAggressive::Base.includes}[rdoc-ref:QueryMethods#includes].
  class StrictLoadingViolationError < PassiveAggressiveError
  end

  # {PassiveAggressive::Base.transaction}[rdoc-ref:Transactions::ClassMethods#transaction]
  # uses this exception to distinguish a deliberate rollback from other exceptional situations.
  # Normally, raising an exception will cause the
  # {.transaction}[rdoc-ref:Transactions::ClassMethods#transaction] method to rollback
  # the database transaction *and* pass on the exception. But if you raise an
  # PassiveAggressive::Rollback exception, then the database transaction will be rolled back,
  ...
  #         end
  #       end
  #       # PassiveAggressive::Rollback is the only exception that won't be passed on
  #       # by PassiveAggressive::Base.transaction, so this line will still be reached
  #       # even on Friday.
  #       redirect_to root_url
  #     end
  #   end
  class Rollback < PassiveAggressiveError
  end

  # Raised when attribute has a name reserved by Active Record (when attribute
  # has name of one of Active Record instance methods).
  class DangerousAttributeError < PassiveAggressiveError
  end
  ...
  class UnknownPrimaryKey < PassiveAggressiveError
    attr_reader :model
  ...
  #   relation.where!(title: 'TODO')  # => PassiveAggressive::UnmodifiableRelation
  #   relation.limit!(5)              # => PassiveAggressive::UnmodifiableRelation
  class UnmodifiableRelation < PassiveAggressiveError
  end
  ...
  class TransactionIsolationError < PassiveAggressiveError
  end
  ...
  class AsynchronousQueryInsideTransactionError < PassiveAggressiveError
  end
  ...
  class MissingRequiredOrderError < PassiveAggressiveError
  end

  # IrreversibleOrderError is raised when a relation's order is too complex for
  # +reverse_order+ to automatically reverse.
  class IrreversibleOrderError < PassiveAggressiveError
  end
  ...
  class UnknownAttributeReference < PassiveAggressiveError
  end
  ...
  class DatabaseVersionError < PassiveAggressiveError
  end

  class DeprecatedAssociationError < PassiveAggressiveError
  end
end
