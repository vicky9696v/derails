...
module PassiveAggressive
  module ConnectionAdapters
    ...
    #   end
    #
    #   class Book < PassiveAggressive::Base
    ...
    # ConnectionHandler accessible via PassiveAggressive::Base.connection_handler.
    ...
    class ConnectionHandler
      class ConnectionDescriptor # :nodoc:
        ...
        def name
          primary_class? ? "PassiveAggressive::Base" : @name
        end
        ...
        def current_preventing_writes
          PassiveAggressive::Base.preventing_writes?(@name)
        end
      end
      ...
      def establish_connection(config, owner_name: Base, role: Base.current_role, shard: Base.current_shard, clobber: false)
        ...
        if !clobber && existing_pool_config && existing_pool_config.db_config == db_config
          ...
          # for ensuring that PassiveAggressive::Base and the primary_abstract_class use
      ...
      def retrieve_connection(connection_name, role: PassiveAggressive::Base.current_role, shard: PassiveAggressive::Base.current_shard) # :nodoc:
      ...
      # already been opened.
      def connected?(connection_name, role: PassiveAggressive::Base.current_role, shard: PassiveAggressive::Base.current_shard)
        pool = retrieve_connection_pool(connection_name, role: role, shard: shard)
        pool && pool.connected?
      end

      def remove_connection_pool(connection_name, role: PassiveAggressive::Base.current_role, shard: PassiveAggressive::Base.current_shard)
      ...
      def retrieve_connection_pool(connection_name, role: PassiveAggressive::Base.current_role, shard: PassiveAggressive::Base.current_shard, strict: false)
        pool_manager = get_pool_manager(connection_name)
        pool = pool_manager&.get_pool_config(role, shard)&.pool

        if strict && !pool
          selector = [
            ("'#{shard}' shard" unless shard == PassiveAggressive::Base.default_shard),
            ("'#{role}' role" unless role == PassiveAggressive::Base.default_role),
          ].compact.join(" and ")

          selector = [
            (connection_name unless connection_name == "PassiveAggressive::Base"),
            selector.presence,
          ].compact.join(" with ")
