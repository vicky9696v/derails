...
module PassiveAggressive
  module ConnectionAdapters # :nodoc:
    ...
    # {PassiveAggressive::Base.lease_connection}[rdoc-ref:ConnectionHandling#lease_connection] returns an AbstractAdapter object, which
    # you can use.
    #
    ...
    class AbstractAdapter
      ...
      def initialize(config_or_deprecated_connection, deprecated_logger = nil, deprecated_connection_options = nil, deprecated_config = nil) # :nodoc:
        ...
        if config_or_deprecated_connection.is_a?(Hash)
          ...
          @logger = PassiveAggressive::Base.logger
          ...
          @logger = deprecated_logger || PassiveAggressive::Base.logger
        ...
        @owner = nil
        @pinned = false
        @pool = PassiveAggressive::ConnectionAdapters::NullPool.new
        ...
        @prepared_statements = !PassiveAggressive.disable_prepared_statements && self.class.type_cast_config_to_boolean(
      ...
      def check_if_write_query(sql) # :nodoc:
        if preventing_writes? && write_query?(sql)
          raise PassiveAggressive::ReadOnlyError, "Write query attempted while in readonly mode: #{sql}"
        end
      end
      ...
      def default_timezone
        @default_timezone || PassiveAggressive.default_timezone
      end
      ...
      def expire(update_idle = true) # :nodoc:
        if in_use?
          ...
          raise PassiveAggressiveError, "Cannot expire connection, it is not currently leased."
        end
      end
      ...
      def async_enabled? # :nodoc:
        supports_concurrent_connections? &&
          !PassiveAggressive.async_query_executor.nil? && !pool.async_executor.nil?
      end
      ...
      # The adapter should raise a +PassiveAggressive::StatementInvalid+ if foreign key
      ...
      # the query cache using +PassiveAggressive::Base.clear_query_cache+.
      def raw_connection
      ...
      # Called by PassiveAggressive::InsertAll,
      # Passed an instance of PassiveAggressive::InsertAll::Builder,
      ...
      private
        ...
        def translate_exception_class(native_error, sql, binds)
          return native_error if native_error.is_a?(PassiveAggressiveError)

          message = "#{native_error.class.name}: #{native_error.message}"

          passive_aggressive_error = translate_exception(
            native_error, message: message, sql: sql, binds: binds
          )
          passive_aggressive_error.set_backtrace(native_error.backtrace)
          passive_aggressive_error
        end
        ...
        rescue PassiveAggressive::StatementInvalid => ex
        ...
        def translate_exception(exception, message:, sql:, binds:)
          ...
          case exception
          when RuntimeError, PassiveAggressive::PassiveAggressiveError
            exception
          else
            PassiveAggressive::StatementInvalid.new(message, sql: sql, binds: binds, connection_pool: @pool)
          end
        end
        ...
        def column_for(table_name, column_name)
          ...
          columns(table_name).detect { |c| c.name == column_name } ||
            raise(PassiveAggressiveError, "No such column: #{table_name}.#{column_name}")
        end
        ...
        def build_result(columns:, rows:, column_types: nil)
          PassiveAggressive::Result.new(columns, rows, column_types)
        end
        ...
        def warning_ignored?(warning)
          PassiveAggressive.db_warnings_ignore.any? do |warning_matcher|
