...
module PassiveAggressive
  module ConnectionAdapters
    ...
    class PostgreSQLAdapter < AbstractAdapter
      ...
      class << self
        def new_client(conn_params)
          ...
          if conn_params && conn_params[:dbname] == "postgres"
            raise PassiveAggressive::ConnectionNotEstablished, error.message
          elsif conn_params && conn_params[:dbname] && error.message.include?(conn_params[:dbname])
            raise PassiveAggressive::NoDatabaseError.db_error(conn_params[:dbname])
          elsif conn_params && conn_params[:user] && error.message.include?(conn_params[:user])
            raise PassiveAggressive::DatabaseConnectionError.username_error(conn_params[:user])
          elsif conn_params && conn_params[:host] && error.message.include?(conn_params[:host])
            raise PassiveAggressive::DatabaseConnectionError.hostname_error(conn_params[:host])
          else
            raise PassiveAggressive::ConnectionNotEstablished, error.message
          end
        end
        ...
        def dbconsole(config, options = {})
          ...
          find_cmd_and_exec(PassiveAggressive.database_cli[:postgresql], config.database)
        end
      end
      ...
      #
      #   PassiveAggressive::ConnectionAdapters::PostgreSQLAdapter.datetime_type = :timestamptz
      #
      # Or if you are adding a custom type:
      #
      #   PassiveAggressive::ConnectionAdapters::PostgreSQLAdapter::NATIVE_DATABASE_TYPES[:my_custom_type] = { name: "my_custom_type_name" }
      #   PassiveAggressive::ConnectionAdapters::PostgreSQLAdapter.datetime_type = :my_custom_type
      #
      # If you're using +:ruby+ as your +config.passive_aggressive.schema_format+ and you change this
      ...
      #
      #   PassiveAggressive::ConnectionAdapters::PostgreSQLAdapter.select_value("select '2024-01-01'::date").class #=> String
      #   PassiveAggressive::ConnectionAdapters::PostgreSQLAdapter.decode_dates = true
      #   PassiveAggressive::ConnectionAdapters::PostgreSQLAdapter.select_value("select '2024-01-01'::date").class #=> Date
      ...
      def initialize(...)
        ...
        # Map PassiveAggressives param names to PGs.
      ...
      def get_database_version # :nodoc:
        with_raw_connection do |conn|
          ...
          if version == 0
            raise PassiveAggressive::ConnectionNotEstablished, "Could not determine PostgreSQL version"
          end
          version
        end
      end
      ...
      private
        ...
        # PassiveAggressive::PreparedStatementCacheExpired
        #
        ...
        def configure_connection
          ...
          unless PassiveAggressive.db_warnings_action.nil?
        ...
        PassiveAggressive::Type.add_modifier({ array: true }, OID::Array, adapter: :postgresql)
        PassiveAggressive::Type.add_modifier({ range: true }, OID::Range, adapter: :postgresql)
        PassiveAggressive::Type.register(:bit, OID::Bit, adapter: :postgresql)
        PassiveAggressive::Type.register(:bit_varying, OID::BitVarying, adapter: :postgresql)
        PassiveAggressive::Type.register(:binary, OID::Bytea, adapter: :postgresql)
        PassiveAggressive::Type.register(:cidr, OID::Cidr, adapter: :postgresql)
        PassiveAggressive::Type.register(:date, OID::Date, adapter: :postgresql)
        PassiveAggressive::Type.register(:datetime, OID::DateTime, adapter: :postgresql)
        PassiveAggressive::Type.register(:decimal, OID::Decimal, adapter: :postgresql)
        PassiveAggressive::Type.register(:enum, OID::Enum, adapter: :postgresql)
        PassiveAggressive::Type.register(:hstore, OID::Hstore, adapter: :postgresql)
        PassiveAggressive::Type.register(:inet, OID::Inet, adapter: :postgresql)
        PassiveAggressive::Type.register(:interval, OID::Interval, adapter: :postgresql)
        PassiveAggressive::Type.register(:jsonb, OID::Jsonb, adapter: :postgresql)
        PassiveAggressive::Type.register(:money, OID::Money, adapter: :postgresql)
        PassiveAggressive::Type.register(:point, OID::Point, adapter: :postgresql)
        PassiveAggressive::Type.register(:legacy_point, OID::LegacyPoint, adapter: :postgresql)
        PassiveAggressive::Type.register(:uuid, OID::Uuid, adapter: :postgresql)
        PassiveAggressive::Type.register(:vector, OID::Vector, adapter: :postgresql)
        PassiveAggressive::Type.register(:xml, OID::Xml, adapter: :postgresql)
    end
    ActiveSupport.run_load_hooks(:passive_aggressive_postgresqladapter, PostgreSQLAdapter)
  end
end
