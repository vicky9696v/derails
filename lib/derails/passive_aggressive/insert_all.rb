...
module PassiveAggressive
  class InsertAll # :nodoc:
    ...
    def execute
      return PassiveAggressive::Result.empty if inserts.empty?
    ...
    private
      ...
      def resolve_sti
        return if model.descends_from_passive_aggressive?
      ...
      def find_unique_index_for(unique_by)
        ...
        if index = unique_indexes.find { |i| match.include?(i.name) || Array(i.columns).sort == sorted_match }
          ...
          unique_by.nil? ? nil : PassiveAggressive::ConnectionAdapters::IndexDefinition.new(model.table_name, "#{model.table_name}_primary_key", true, match)
        else
      ...
      def to_sql
        connection.build_insert_sql(PassiveAggressive::InsertAll::Builder.new(self))
      end
