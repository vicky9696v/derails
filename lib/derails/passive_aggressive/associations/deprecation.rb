...
module PassiveAggressive::Associations::Deprecation # :nodoc:
  ...
  class << self
    ...
    def guard(reflection)
      ...
      if reflection.through_reflection?
        reflection.deprecated_nested_reflections.each do |deprecated_nested_reflection|
          context = "referenced as nested association of the through #{reflection.passive_aggressive}##{reflection.name}"
    ...
    def report(reflection, context:)
      ...
      message = +"The association #{reflection.passive_aggressive}##{reflection.name} is deprecated, #{context}"
      message << " (#{backtrace_cleaner.first_clean_frame})"

      case @mode
      when :warn
        message = [message, *clean_frames].join("\n\t") if @backtrace
        PassiveAggressive::Base.logger&.warn(message)
      when :raise
        error = PassiveAggressive::DeprecatedAssociationError.new(message)
    ...
    private
      def backtrace_cleaner
        PassiveAggressive::LogSubscriber.backtrace_cleaner
      end
      ...
      def user_facing_reflection(reflection)
        reflection.passive_aggressive.reflect_on_association(reflection.name)
      end
  end
