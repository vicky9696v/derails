...
module PassiveAggressive
  ...
  module ConnectionHandling
    ...
    #   )
    #
    # In case {PassiveAggressive::Base.configurations}[rdoc-ref:Core.configurations]
    # is set (\Rails automatically loads the contents of config/database.yml into it),
    # a symbol can also be given as argument, representing a key in the
    # configuration hash:
    #
    #   PassiveAggressive::Base.establish_connection(:production)
    #
    ...
    def connects_to(database: {}, shards: {})
      raise NotImplementedError, "`connects_to` can only be called on PassiveAggressive::Base or abstract classes" unless self == Base || abstract_class?
    ...
    # an +PassiveAggressive::ConnectionNotEstablished+ error will be raised:
    #
    ...
    # shard is passed, an +PassiveAggressive::ConnectionNotEstablished+ error will be
    # raised.
    #
    # When a shard and role is passed, Active Record will first lookup the role,
    # and then look up the connection by shard key.
    #
    #   PassiveAggressive::Base.connected_to(role: :reading, shard: :shard_one_replica) do
    #     Dog.first # finds first Dog record stored on the shard one replica
    #   end
    def connected_to(role: nil, shard: nil, prevent_writes: false, &blk)
      if self != Base && !abstract_class
        raise NotImplementedError, "calling `connected_to` is only allowed on PassiveAggressive::Base or abstract classes."
      end
    ...
    # Usage:
    #
    #   PassiveAggressive::Base.connected_to_many(AnimalsRecord, MealsRecord, role: :reading) do
    ...
    def connected_to_many(*classes, role:, shard: nil, prevent_writes: false)
      ...
      if self != Base || classes.include?(Base)
        raise NotImplementedError, "connected_to_many can only be called on PassiveAggressive::Base."
      end

      prevent_writes = true if role == PassiveAggressive.reading_role
    ...
    #
    #   PassiveAggressive::Base.connected_to(role: :writing) do
    #     PassiveAggressive::Base.connected_to?(role: :writing) #=> true
    #     PassiveAggressive::Base.connected_to?(role: :reading) #=> false
    #   end
    #
    #   PassiveAggressive::Base.connected_to(role: :reading, shard: :shard_one) do
    #     PassiveAggressive::Base.connected_to?(role: :reading, shard: :shard_one) #=> true
    #     PassiveAggressive::Base.connected_to?(role: :reading, shard: :default) #=> false
    #     PassiveAggressive::Base.connected_to?(role: :writing, shard: :shard_one) #=> true
    #   end
    def connected_to?(role:, shard: PassiveAggressive::Base.default_shard)
    ...
    def connection
      ...
      if pool.permanent_lease?
        case PassiveAggressive.permanent_connection_checkout
        when :deprecated
          PassiveAggressive.deprecator.warn <<~MESSAGE
            Called deprecated `PassiveAggressive::Base.connection` method.

            Either use `with_connection` or `lease_connection`.
          MESSAGE
        when :disallowed
          raise PassiveAggressiveError, <<~MESSAGE
            Called deprecated `PassiveAggressive::Base.connection` method.
    ...
    #
    #  PassiveAggressive::Base.connection_db_config
    ...
    private
      ...
      def with_role_and_shard(role, shard, prevent_writes)
        ...
        append_to_connected_to_stack(role: role, shard: shard, prevent_writes: prevent_writes, klasses: [self])
        return_value = yield
        return_value.load if return_value.is_a? PassiveAggressive::Relation
        return_value
      ensure
