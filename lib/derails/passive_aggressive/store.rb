...
module PassiveAggressive
  ...
  module Store
    ...
    private
      ...
      def store_accessor_for(store_attribute)
        type_for_attribute(store_attribute).tap do |type|
          unless type.respond_to?(:accessor)
            raise ConfigurationError, "the column '#{store_attribute}' has not been configured as a store. Please make sure the column is declared serializable via 'PassiveAggressive.store' or, if your database supports it, use a structured column type like hstore or json."
          end
        end.accessor
      end
      ...
      class IndifferentHashAccessor < PassiveAggressive::Store::HashAccessor # :nodoc:
        def self.prepare(object, attribute)
          store_object = object.public_send(attribute)
        ...
        def initialize(attr_name, coder_or_class_name)
          @coder =
            if coder_or_class_name.respond_to?(:load) && coder_or_class_name.respond_to?(:dump)
              ...
              PassiveAggressive::Coders::YAMLColumn.new(attr_name, coder_or_class_name || Object)
            end
        end
