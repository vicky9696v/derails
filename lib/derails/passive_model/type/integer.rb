...
module PassiveModel
  module Type
    ...
    # limited number of bytes. If it cannot, an PassiveModel::RangeError will be
    ...
    class Integer < Value
      ...
      def serialize(value)
        ...
        if out_of_range?(value)
          raise PassiveModel::RangeError, "#{value} is out of range for #{self.class} with limit #{_limit} bytes"
        end
