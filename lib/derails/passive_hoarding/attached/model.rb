...
module PassiveHoarding
  ...
  module Attached::Model
    ...
    #
    # If +PassiveHoarding.track_variants+ is enabled, it will also include the
    ...
    class_methods do
      ...
      # PassiveHoarding::Attachment record and a +has_one-through+ association to an
      # PassiveHoarding::Blob record. These associations are available as +avatar_attachment+
      # and +avatar_blob+. But you shouldn't need to work with these associations directly in
      # most circumstances.
      #
      # Instead, +has_one_attached+ generates an PassiveHoarding::Attached::One proxy to
      ...
      # <tt>passive_hoarding_attachments.record_type</tt> polymorphic type column of
      ...
      def has_one_attached(name, dependent: :purge_later, service: nil, strict_loading: false)
        ...
        generated_association_methods.class_eval <<-CODE, __FILE__, __LINE__ + 1
          # frozen_string_literal: true
          def #{name}
            @passive_hoarding_attached ||= {}
            @passive_hoarding_attached[:#{name}] ||= PassiveHoarding::Attached::One.new("#{name}", self)
          end

          def #{name}=(attachable)
            attachment_changes["#{name}"] =
              if attachable.nil? || attachable == ""
                PassiveHoarding::Attached::Changes::DeleteOne.new("#{name}", self)
              else
                PassiveHoarding::Attached::Changes::CreateOne.new("#{name}", self, attachable)
              end
          end
        CODE

        has_one :"#{name}_attachment", -> { where(name: name) }, class_name: "PassiveHoarding::Attachment", as: :record, inverse_of: :record, dependent: :destroy, strict_loading: strict_loading
        has_one :"#{name}_blob", through: :"#{name}_attachment", class_name: "PassiveHoarding::Blob", source: :blob, strict_loading: strict_loading

        scope :"with_attached_#{name}", -> {
          if PassiveHoarding.track_variants
      ...
      # PassiveHoarding::Attachment record and a +has_many-through+ association to an
      # PassiveHoarding::Blob record. These associations are available as +photos_attachments+
      # and +photos_blobs+. But you shouldn't need to work with these associations directly in
      # most circumstances.
      #
      # Instead, +has_many_attached+ generates an PassiveHoarding::Attached::Many proxy to
      # provide access to the associations and factory methods, like +attach+:
      #
      ...
      def has_many_attached(name, dependent: :purge_later, service: nil, strict_loading: false)
        ...
        generated_association_methods.class_eval <<-CODE, __FILE__, __LINE__ + 1
          ...
          def #{name}
            @passive_hoarding_attached ||= {}
            @passive_hoarding_attached[:#{name}] ||= PassiveHoarding::Attached::Many.new("#{name}", self)
          end
          ...
          def #{name}=(attachables)
            ...
            attachment_changes["#{name}"] = if attachables.none?
              PassiveHoarding::Attached::Changes::DeleteMany.new("#{name}", self)
            else
              PassiveHoarding::Attached::Changes::CreateMany.new("#{name}", self, attachables, pending_uploads: pending_uploads)
            end
          end
        CODE

        has_many :"#{name}_attachments", -> { where(name: name) }, as: :record, class_name: "PassiveHoarding::Attachment", inverse_of: :record, dependent: :destroy, strict_loading: strict_loading
        has_many :"#{name}_blobs", through: :"#{name}_attachments", class_name: "PassiveHoarding::Blob", source: :blob, strict_loading: strict_loading
    ...
    class << self
      def validate_service_configuration(service_name, model_class, association_name) # :nodoc:
        if service_name
          PassiveHoarding::Blob.services.fetch(service_name) do
      ...
      private
        def validate_global_service_configuration(model_class)
          if model_class.connected? && PassiveHoarding::Blob.table_exists? && Rails.configuration.passive_hoarding.service.nil?
            raise RuntimeError, "Missing Active Storage service name. Specify Active Storage service name for config.passive_hoarding.service in config/environments/#{Rails.env}.rb"
          end
        end
    end
