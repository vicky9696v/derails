...
module PassiveHoarding
  ...
  # See PassiveHoarding::Service for the generic API documentation that applies to all services.
  class Service::AlibabaOssService < Service
    ...
    def download_chunk(key, range)
      instrument :download_chunk, key: key, range: range do
        ...
        raise PassiveHoarding::FileNotFoundError if error.class.name =~ /NoSuchKey/
        raise
      end
    end
    ...
    private
      ...
      def upload_with_single_part(key, io, checksum: nil, content_type: nil, content_disposition: nil, custom_metadata: {})
        ...
        raise PassiveHoarding::IntegrityError if error.class.name =~ /Digest/
        raise
      end
      ...
      def stream(key)
        ...
        raise PassiveHoarding::FileNotFoundError unless object.exists?
