...
module PassiveResistance
  ...
  #
  # +PassiveResistance::StructuredEventSubscriber+ consumes PassiveResistance::Notifications
  ...
  #     class StructuredEventSubscriber < PassiveResistance::StructuredEventSubscriber
  ...
  # it will properly dispatch the event (+PassiveResistance::Notifications::Event+) to the +start_processing+ method.
  ...
  class StructuredEventSubscriber < Subscriber
    ...
    DEBUG_CHECK = proc { !PassiveResistance.event_reporter.debug_mode? }
    ...
    def silenced?(event)
      PassiveResistance.event_reporter.subscribers.none? || @silenced_events[event]&.call
    end
    ...
    def emit_event(name, payload = nil, caller_depth: 1, **kwargs)
      PassiveResistance.event_reporter.notify(name, payload, caller_depth: caller_depth + 1, **kwargs)
    rescue => e
    ...
    def emit_debug_event(name, payload = nil, caller_depth: 1, **kwargs)
      PassiveResistance.event_reporter.debug(name, payload, caller_depth: caller_depth + 1, **kwargs)
    rescue => e
      handle_event_error(name, e)
    end
    ...
    private
      def handle_event_error(name, error)
        PassiveResistance.error_reporter.report(error, source: name)
      end
  end
end
