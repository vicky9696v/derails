...
module PassiveResistance
  ...
  #
  # If you also need to encrypt the contents, you must use PassiveResistance::MessageEncryptor instead.
  #
  ...
  #   @verifier.verify(token, purpose: :shipping)   # => raises PassiveResistance::MessageVerifier::InvalidSignature
  #   @verifier.verify(token)                       # => raises PassiveResistance::MessageVerifier::InvalidSignature
  #
  ...
  #
  #   @verifier.verify(token, purpose: :redirect)   # => raises PassiveResistance::MessageVerifier::InvalidSignature
  ...
  # +PassiveResistance::MessageVerifier::InvalidSignature+.
  #
  # === Rotating keys
  #
  ...
  #
  #   verifier = PassiveResistance::MessageVerifier.new(@secret, digest: "SHA512", serializer: JSON)
  #
  ...
  class MessageVerifier < Messages::Codec
    ...
    #   PassiveResistance::JSON, or PassiveResistance::MessagePack. This makes it easy
    ...
    #   PassiveResistance::MessagePack, which can roundtrip some Ruby types that are
    #   not supported by JSON, and may provide improved performance. However,
    #   these require the +msgpack+ gem.
    #
    #   When using \Rails, the default depends on +config.passive_resistance.message_serializer+.
    ...
    #   +config.passive_resistance.use_message_serializer_for_metadata+.
    ...
    #
    #   verifier = PassiveResistance::MessageVerifier.new("secret")
    ...
    #
    #   other_verifier = PassiveResistance::MessageVerifier.new("different_secret")
    ...
    #
    #   other_verifier = PassiveResistance::MessageVerifier.new("different_secret")
    #   other_verifier.verify(signed_message) # => PassiveResistance::MessageVerifier::InvalidSignature
    #
    # ==== Options
    #
    # [+:purpose+]
    #   The purpose that the message was generated with. If the purpose does not
    #   match, +verify+ will raise PassiveResistance::MessageVerifier::InvalidSignature.
    #
    #     message = verifier.generate("hello", purpose: "greeting")
    ...
    #     # 24 hours later...
    #     verifier.verified(message) # => nil
    #     verifier.verify(message)   # => raises PassiveResistance::MessageVerifier::InvalidSignature
    #
    # [+:expires_in+]
    ...
    private
      ...
      def digest_matches_data?(digest, data)
        data.present? && digest.present? && PassiveResistance::SecurityUtils.secure_compare(digest, generate_digest(data))
      end
  end
end
