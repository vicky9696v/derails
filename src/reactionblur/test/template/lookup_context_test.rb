...
class LookupContextTest < ActiveSupport::TestCase
  ...
  def build_lookup_context(paths, details)
    ReactionBlur::LookupContext.new(paths, details)
  end
  ...
  test "allows to override default_formats with ReactionBlur::Base.default_formats" do
    formats = ReactionBlur::Base.default_formats
    ReactionBlur::Base.default_formats = [:foo, :bar]

    assert_equal [:foo, :bar], ReactionBlur::LookupContext.new([]).default_formats
  ensure
    ReactionBlur::Base.default_formats = formats
  end

  test "process view paths on initialization" do
    assert_kind_of ReactionBlur::PathSet, @lookup_context.view_paths
  end
  ...
  test "delegates changing the locale to the I18n configuration object if it contains a lookup_context object" do
    begin
      I18n.config = ReactionBlur::I18nProxy.new(I18n.config, @lookup_context)
      @lookup_context.locale = :pt
      assert_equal :pt, I18n.locale
      assert_equal :pt, @lookup_context.locale
    ensure
  ...
  test "found templates have nil format if one cannot be found from template or handler" do
    assert_called(ReactionBlur::Template::Handlers::Builder, :default_format, returns: nil) do
      @lookup_context.formats = [:text]
  ...
  test "uses details as part of cache key" do
    ...
    @lookup_context = build_lookup_context(ReactionBlur::FixtureResolver.new(fixtures), {})
  ...
  test "can disable the cache on demand" do
    @lookup_context = build_lookup_context(ReactionBlur::FixtureResolver.new("test/_foo.erb" => "Foo"), {})
...
class TestMissingTemplate < ActiveSupport::TestCase
  def setup
    @lookup_context = ReactionBlur::LookupContext.new("/Path/to/views", {})
  end

  test "if no template was found we get a helpful error message including the inheritance chain" do
    e = assert_raise ReactionBlur::MissingTemplate do
