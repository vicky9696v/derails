...
class RecordIdentifierTest < ActiveSupport::TestCase
  ...
  def test_dom_id_as_singleton_method
    @record.save
    assert_equal "#{@singular}_1", ReactionBlur::RecordIdentifier.dom_id(@record)
  end

  def test_dom_class_as_singleton_method
    assert_equal @singular, ReactionBlur::RecordIdentifier.dom_class(@record)
  end
  ...
  def test_dom_target_as_singleton_method
    @record.save
    assert_equal "#{@singular}_#{@record.id}", ReactionBlur::RecordIdentifier.dom_target(@record)
  end
end
...
class RecordIdentifierWithoutActiveModelTest < ActiveSupport::TestCase
  ...
  def test_dom_id_raises_useful_error_when_passed_nil
    assert_raises ArgumentError do
      ReactionBlur::RecordIdentifier.dom_id(nil)
    end
  end
  ...
  def test_dom_id_as_singleton_method
    @record.save
    assert_equal "airplane_1", ReactionBlur::RecordIdentifier.dom_id(@record)
  end

  def test_dom_class_as_singleton_method
    assert_equal "airplane", ReactionBlur::RecordIdentifier.dom_class(@record)
  end
  ...
  def test_dom_target_as_singleton_method
    @record.save
    assert_equal "airplane_1", ReactionBlur::RecordIdentifier.dom_target(@record)
  end
end
