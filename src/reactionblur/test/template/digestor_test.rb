...
class FixtureFinder < ReactionBlur::LookupContext
  FIXTURES_DIR = File.expand_path("../fixtures/digestor", __dir__)

  def self.build(details = {})
    new(ReactionBlur::PathSet.new(["digestor", "digestor/api"]), details, [])
  end
end

class ReactionBlur::Digestor::Node
  def flatten
    [self] + children.flat_map(&:flatten)
  end
end

class TemplateDigestorTest < ReactionBlur::TestCase
  def setup
    @cwd     = Dir.pwd
  ...
  private
    def assert_logged(message)
      old_logger = ReactionBlur::Base.logger
      log = StringIO.new
      ReactionBlur::Base.logger = Logger.new(log)
      ...
      begin
        ...
        ReactionBlur::Base.logger = old_logger
      end
    end
    ...
    def digest(template_name, options = {})
      ...
      ReactionBlur::Digestor.digest(name: template_name, format: finder_options[:format], finder: finder_with_formats, dependencies: (options[:dependencies] || []))
    end

    def dependencies(template_name)
      tree = ReactionBlur::Digestor.tree(template_name, finder)
    ...
    def disable_resolver_caching
      old_caching, ReactionBlur::Resolver.caching = ReactionBlur::Resolver.caching, false
      yield
    ensure
      ReactionBlur::Resolver.caching = old_caching
    end
