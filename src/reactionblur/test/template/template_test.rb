...
class TestERBTemplate < ActiveSupport::TestCase
  ERBHandler = ReactionBlur::Template::Handlers::ERB.new
  ...
  def new_template(body = "<%= hello %>", details = {})
    ...
    ReactionBlur::Template.new(body.dup, "hello template", details.delete(:handler) || ERBHandler, virtual_path: "hello", **details)
  end
  ...
  def test_raw_template
    @template = new_template("<%= hello %>", handler: ReactionBlur::Template::Handlers::Raw.new)
    assert_equal "<%= hello %>", render
  end
  ...
  def test_rendering_non_string
    ...
    @template = ReactionBlur::Template.new("my_object", "__id__", eval_handler, virtual_path: "hello", locals: [:my_object])
  ...
  def test_locals_can_be_disabled
    error = assert_raises(ReactionBlur::Template::Error) do
  ...
  def test_locals_cannot_be_specified_with_positional_arguments
    ...
    assert_raises ReactionBlur::Template::Error, match: "`argument` set as non-keyword argument for hello template. Locals can only be set as keyword arguments." do
      render
    end
  end
  ...
  def test_locals_cannot_be_specified_with_block_arguments
    ...
    assert_raises ReactionBlur::Template::Error, match: "`block` set as non-keyword argument for hello template. Locals can only be set as keyword arguments." do
      render { "content" }
    end
  end
  ...
  def test_required_locals_must_be_specified
    ...
    assert_instance_of ReactionBlur::StrictLocalsError, error.cause
  end
  ...
  def test_lying_with_magic_comment
    assert_raises(ReactionBlur::Template::Error) do
  ...
  def test_error_when_template_isnt_valid_utf8
    e = assert_raises ReactionBlur::Template::Error do
  ...
  def test_template_inspect
    @template = new_template("hello")
    assert_equal "#<ReactionBlur::Template hello template locals=[]>", @template.inspect
  end
