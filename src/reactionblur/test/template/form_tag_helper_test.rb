...
class FormTagHelperTest < ReactionBlur::TestCase
  ...
  def test_empty_submit_tag_with_opt_out
    ReactionBlur::Base.automatically_disable_submit_tag = false
    assert_dom_equal(
      %(<input name='commit' type="submit" value="Save" />),
      submit_tag("Save")
    )
  ensure
    ReactionBlur::Base.automatically_disable_submit_tag = true
  end
  ...
  private
    ...
    def with_prepend_content_exfiltration_prevention(value)
      old_value = ReactionBlur::Helpers::ContentExfiltrationPreventionHelper.prepend_content_exfiltration_prevention
      ReactionBlur::Helpers::ContentExfiltrationPreventionHelper.prepend_content_exfiltration_prevention = value

      yield
    ensure
      ReactionBlur::Helpers::ContentExfiltrationPreventionHelper.prepend_content_exfiltration_prevention = old_value
    end

    def with_remove_hidden_field_autocomplete(value)
      old = ReactionBlur::Base.remove_hidden_field_autocomplete
      ReactionBlur::Base.remove_hidden_field_autocomplete = value
      yield
    ensure
      ReactionBlur::Base.remove_hidden_field_autocomplete = old
    end
end
