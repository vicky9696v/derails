...
class TemplateErrorTest < ActiveSupport::TestCase
  def test_provides_original_message
    error = begin
      ...
      raise ReactionBlur::Template::Error.new("test") rescue $!
    end
  ...
  def test_provides_useful_inspect
    ...
    assert_equal "#<ReactionBlur::Template::Error: original>", error.inspect
  end
  ...
  def test_annotated_source_code_returns_empty_array_if_source_cant_be_found
    ...
    error = begin
      ...
      raise ReactionBlur::Template::Error.new(template) rescue $!
    end
