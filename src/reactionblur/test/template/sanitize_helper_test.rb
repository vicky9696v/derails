...
#
class SanitizeHelperTest < ReactionBlur::TestCase
  tests ReactionBlur::Helpers::SanitizeHelper

  def setup
    super
    @saved_vendor = ReactionBlur::Helpers::SanitizeHelper.sanitizer_vendor
    @mock_vendor = ReactionBlur::Helpers::SanitizeHelper.sanitizer_vendor = new_mock_vendor
  end

  def teardown
    ReactionBlur::Helpers::SanitizeHelper.sanitizer_vendor = @saved_vendor
    super
  end

  test "sanitizer_vendor module attribute and class method" do
    assert_equal(@mock_vendor, ReactionBlur::Helpers::SanitizeHelper.sanitizer_vendor)
    assert_equal(@mock_vendor, self.class.sanitizer_vendor)

    vendor2 = ReactionBlur::Helpers::SanitizeHelper.sanitizer_vendor = new_mock_vendor

    assert_equal(vendor2, ReactionBlur::Helpers::SanitizeHelper.sanitizer_vendor)
...
module SanitizeHelperVendorTests
  def setup
    super
    @saved_vendor = ReactionBlur::Helpers::SanitizeHelper.sanitizer_vendor
    ReactionBlur::Helpers::SanitizeHelper.sanitizer_vendor = vendor

    @subject_class = Class.new do
      include ReactionBlur::Helpers::SanitizeHelper
    end
