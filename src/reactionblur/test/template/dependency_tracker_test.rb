...
require "reaction_blur/dependency_tracker"

require "reaction_blur/render_parser/prism_render_parser"

require "ripper"
require "reaction_blur/render_parser/ripper_render_parser"
...
class FakeTemplate
  ...
  def initialize(source, handler = Neckbeard)
    ...
    if handler == :erb
      @handler = ReactionBlur::Template::Handlers::ERB.new
    end
  end
...
class DependencyTrackerTest < ReactionBlur::TestCase
  def tracker
    ReactionBlur::DependencyTracker
  end

  def setup
    ReactionBlur::Template.register_template_handler :neckbeard, Neckbeard
    tracker.register_tracker(:neckbeard, NeckbeardTracker)
  end

  def teardown
    ReactionBlur::Template.unregister_template_handler :neckbeard
...
module SharedTrackerTests
  ...
  def test_dependencies_with_interpolation_are_resolved_with_view_paths
    view_paths = ReactionBlur::PathSet.new([File.expand_path("../fixtures/digestor", __dir__)])
...
class ERBTrackerTest < ActiveSupport::TestCase
  ...
  def make_tracker(name, template, view_paths = nil)
    ReactionBlur::DependencyTracker::ERBTracker.new(name, template, view_paths)
  end
end

module RubyTrackerTests
  def make_tracker(name, template, view_paths = nil)
    ReactionBlur::DependencyTracker::RubyTracker.new(name, template, view_paths, parser_class: parser_class)
  end
...
class RipperRubyTrackerTest < ActiveSupport::TestCase
  ...
  def parser_class
    ReactionBlur::RenderParser::RipperRenderParser
  end
end
...
class PrismRubyTrackerTest < ActiveSupport::TestCase
  ...
  def parser_class
    ReactionBlur::RenderParser::PrismRenderParser
  end
end
