...
module RenderTestCases
  def setup_view(paths)
    ...
    @view = Class.new(ReactionBlur::Base.with_empty_template_cache) do
  ...
  def test_render_template_with_a_missing_partial_of_another_format
    @view.lookup_context.formats = [:html]
    e = assert_raise ReactionBlur::Template::Error do
  ...
  def test_render_outside_path
    ...
    assert_raises ReactionBlur::MissingTemplate do
      @view.render(template: "../\\../test/abstract_unit.rb")
    end
  end

  def test_render_template_with_syntax_error
    e = assert_raises(ReactionBlur::Template::Error) { silence_warnings { @view.render(template: "test/syntax_error") } }
    assert_match %r!syntax!, e.message
    assert_equal "1:    <%= foo(", e.annotated_source_code[0].strip
  end

  def test_render_runtime_error
    ex = assert_raises(ReactionBlur::Template::Error) {
  ...
  def test_render_partial_with_missing_filename
    assert_raises(ReactionBlur::MissingTemplate) { @view.render(partial: "test/") }
  end
  ...
  def test_render_partial_with_errors
    e = assert_raises(ReactionBlur::Template::Error) { @view.render(partial: "test/raise") }
  ...
  def test_render_error_indentation
    e = assert_raises(ReactionBlur::Template::Error) { @view.render(partial: "test/raise_indentation") }
  ...
  def test_render_template_with_errors
    e = assert_raises(ReactionBlur::Template::Error) { @view.render(template: "test/_raise") }
    assert_match %r!method.*doesnt_exist!, e.message
    assert_equal "", e.sub_template_message
    assert_equal "1", e.line_number
    assert_equal "1: <%= doesnt_exist %>", e.annotated_source_code[0].strip
  ...
  def test_render_sub_template_with_errors
    e = assert_raises(ReactionBlur::Template::Error) { @view.render(template: "test/sub_template_raise") }
    assert_match %r!method.*doesnt_exist!, e.message
  ...
  def test_undefined_method_error_references_named_class
    e = assert_raises(ReactionBlur::Template::Error) { @view.render(inline: "<%= undefined %>") }
  ...
  def test_without_compiled_method_container_is_deprecated
    view = ReactionBlur::Base.with_view_paths(ActionController::Base.view_paths)
  ...
  def test_render_partial_without_object_or_collection_does_not_generate_partial_name_local_variable
    exception = assert_raises ReactionBlur::Template::Error do
  ...
  def test_render_partial_with_non_existent_format_and_raise_missing_template
    @view.formats = [:xml]
    assert_raises(ReactionBlur::MissingTemplate) { @view.render(partial: "test/layout_for_partial") }
  ensure
    @view.formats = nil
  end
  ...
  def test_render_inline_with_render_from_to_proc
    ReactionBlur::Template.register_template_handler :ruby_handler, lambda { |_, source| source }
    assert_equal "3", @view.render(inline: "(1 + 2).to_s", type: :ruby_handler)
  ensure
    ReactionBlur::Template.unregister_template_handler :ruby_handler
  end

  def test_optional_second_arg_works_without_deprecation
    assert_not_deprecated(ReactionBlur.deprecator) do
      ReactionBlur::Template.register_template_handler :ruby_handler, ->(view, source = nil) { source }
    end
    assert_equal "3", @view.render(inline: "(1 + 2).to_s", type: :ruby_handler)
  ensure
    ReactionBlur::Template.unregister_template_handler :ruby_handler
  ...
  def test_render_inline_with_compilable_custom_type
    ReactionBlur::Template.register_template_handler :foo, CustomHandler
    assert_equal 'source: "Hello, World!"', @view.render(inline: "Hello, World!", type: :foo)
  ensure
    ReactionBlur::Template.unregister_template_handler :foo
  end
  ...
  def test_render_knows_about_types_registered_when_extensions_are_checked_earlier_in_initialization
    ReactionBlur::Template::Handlers.extensions
    ReactionBlur::Template.register_template_handler :foo, CustomHandler
    assert_includes ReactionBlur::Template::Handlers.extensions, :foo
  ensure
    ReactionBlur::Template.unregister_template_handler :foo
  end
  ...
  def test_render_does_not_use_unregistered_extension_and_template_handler
    ReactionBlur::Template.register_template_handler :foo, CustomHandler
    ReactionBlur::Template.unregister_template_handler :foo
    assert_not ReactionBlur::Template::Handlers.extensions.include?(:foo)
    assert_equal "Hello, World!", @view.render(inline: "Hello, World!", type: :foo)
  ensure
    ReactionBlur::Template::Handlers.class_variable_get(:@@template_handlers).delete(:foo)
  end
  ...
  def test_render_partial_with_layout_raises_descriptive_error
    e = assert_raises(ReactionBlur::MissingTemplate) { @view.render(partial: "test/partial", layout: true) }
    assert_match "Missing partial /_true with", e.message
  end

  def test_render_partial_provides_spellcheck
    e = assert_raises(ReactionBlur::MissingTemplate) { @view.render(partial: "test/partail") }
  ...
  def test_spellcheck_doesnt_list_directories
    e = assert_raises(ReactionBlur::MissingTemplate) { @view.render(partial: "test/directory") }
  ...
  def test_spellcheck_only_lists_templates
    e = assert_raises(ReactionBlur::MissingTemplate) { @view.render(template: "test/partial") }
  ...
  def test_spellcheck_only_lists_partials
    e = assert_raises(ReactionBlur::MissingTemplate) { @view.render(partial: "test/template") }
  ...
  def test_render_partial_wrong_details_no_spellcheck
    e = assert_raises(ReactionBlur::MissingTemplate) { @view.render(partial: "test/partial_with_only_html_version", formats: [:xml]) }
  ...
  def test_render_with_passing_couple_extensions_to_one_register_template_handler_function_call
    ReactionBlur::Template.register_template_handler :foo1, :foo2, CustomHandler
    assert_equal @view.render(inline: +"Hello, World!", type: :foo1), @view.render(inline: +"Hello, World!", type: :foo2)
  ensure
    ReactionBlur::Template.unregister_template_handler :foo1, :foo2
  end

  def test_render_throws_exception_when_no_extensions_passed_to_register_template_handler_function_call
    assert_raises(ArgumentError) { ReactionBlur::Template.register_template_handler CustomHandler }
  end
...
class FrozenStringLiteralEnabledViewRenderTest < ActiveSupport::TestCase
  ...
  def setup
    ...
    @previous_frozen_literal = ReactionBlur::Template.frozen_string_literal
    ReactionBlur::Template.frozen_string_literal = true
  ...
  def teardown
    super
    ReactionBlur::Template.frozen_string_literal = @previous_frozen_literal
  end

  def test_render_mutate_string_literal
    error = assert_raise ReactionBlur::Template::Error do
...
class CachedViewRenderTest < ActiveSupport::TestCase
  ...
  def setup
    ReactionBlur::LookupContext::DetailsKey.clear
    view_paths = ActionController::Base.view_paths
    assert_equal ReactionBlur::FileSystemResolver, view_paths.first.class
    setup_view(view_paths)
  end
  ...
  def test_uncacheable
    e = assert_raises(ReactionBlur::Template::Error) { @view.render(template: "test/uncacheable") }
...
class LazyViewRenderTest < ActiveSupport::TestCase
  ...
  def setup
    ReactionBlur::LookupContext::DetailsKey.clear
    path = ReactionBlur::FileSystemResolver.new(FIXTURE_LOAD_PATH)
    view_paths = ReactionBlur::PathSet.new([path])
    assert_equal ReactionBlur::FileSystemResolver.new(FIXTURE_LOAD_PATH), view_paths.first
    setup_view(view_paths)
  end
  ...
  def test_render_utf8_template_with_incompatible_external_encoding
    with_external_encoding Encoding::SHIFT_JIS do
      e = assert_raises(ReactionBlur::Template::Error) { @view.render(template: "test/utf8", formats: [:html], layouts: "layouts/yield") }
  ...
  def test_render_utf8_template_with_partial_with_incompatible_encoding
    with_external_encoding Encoding::SHIFT_JIS do
      e = assert_raises(ReactionBlur::Template::Error) { @view.render(template: "test/utf8_magic_with_bare_partial", formats: [:html], layouts: "layouts/yield") }
      assert_match "Your template was not saved as valid Shift_JIS", e.cause.message
    end
  end
...
class CachedCollectionViewRenderTest < ActiveSupport::TestCase
  ...
  setup do
    ...
    ReactionBlur::PartialRenderer.collection_cache = ActiveSupport::Cache::MemoryStore.new
  ...
  test "template body written to cache" do
    ...
    assert_nil ReactionBlur::PartialRenderer.collection_cache.read(key)
    @view.render(partial: "test/customer", collection: [customer], cached: true)
    assert_equal "Hello: david", ReactionBlur::PartialRenderer.collection_cache.read(key)
  end
  ...
  test "collection caching does not cache by default" do
    ...
    ReactionBlur::PartialRenderer.collection_cache.write(key, "Cached")
  ...
  test "collection caching with empty collection and logger with level debug" do
    ReactionBlur::PartialRenderer.collection_cache.logger = Logger.new(nil, level: :debug)
  ...
  private
    def cache_key(*names, virtual_path)
      digest = ReactionBlur::Digestor.digest name: virtual_path, format: :html, finder: @view.lookup_context, dependencies: []
