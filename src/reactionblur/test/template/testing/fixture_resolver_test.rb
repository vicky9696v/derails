...
class FixtureResolverTest < ActiveSupport::TestCase
  def test_should_return_empty_list_for_unknown_path
    resolver = ReactionBlur::FixtureResolver.new()
  ...
  def test_should_return_template_for_declared_path
    resolver = ReactionBlur::FixtureResolver.new("arbitrary/path.erb" => "this text")
  ...
  def test_should_match_templates_with_variants
    resolver = ReactionBlur::FixtureResolver.new("arbitrary/path.html+variant.erb" => "this text")
  ...
  def test_should_match_locales
    resolver = ReactionBlur::FixtureResolver.new("arbitrary/path.erb" => "this text", "arbitrary/path.fr.erb" => "ce texte")
  ...
  def test_should_return_all_variants_for_any
    resolver = ReactionBlur::FixtureResolver.new("arbitrary/path.html.erb" => "this html", "arbitrary/path.html+variant.erb" => "this text")
    templates = resolver.find_all("path", "arbitrary", false, locale: [], formats: [:html], variants: [], handlers: [:erb])
    assert_equal 1, templates.size, "expected one template"
