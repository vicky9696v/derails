...
module TemplateHandlerHelper
  def with_template_handler(*extensions, handler)
    ReactionBlur::Template.register_template_handler(*extensions, handler)
    ActionController::Base.view_paths.paths.each(&:clear_cache)
    ReactionBlur::LookupContext::DetailsKey.clear
    yield
  ensure
    ReactionBlur::Template.unregister_template_handler(*extensions)
    ActionController::Base.view_paths.paths.each(&:clear_cache)
    ReactionBlur::LookupContext::DetailsKey.clear
  end
end
...
class LayoutSetInResponseTest < ActionController::TestCase
  include ReactionBlur::Template::Handlers
  include TemplateHandlerHelper
...
class LayoutExceptionRaisedTest < ActionController::TestCase
  ...
  def test_exception_raised_when_layout_file_not_found
    ...
    assert_raise(ReactionBlur::MissingTemplate) { get :hello }
  end
end
