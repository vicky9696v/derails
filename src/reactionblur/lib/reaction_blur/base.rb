...
require "reaction_blur/context"
require "reaction_blur/template"
require "reaction_blur/lookup_context"
...
module ReactionBlur # :nodoc:
  ...
  #   <% end %>
  #
  # Read more about strict locals in {Action View Overview}[https://guides.rubyonrails.org/reaction_blur_overview.html#strict-locals]
  # in the guides.
  #
  # === Template caching
  #
  ...
  class Base
    ...
    # Configured via `config.reaction_blur.remove_hidden_field_autocomplete`
    cattr_accessor :remove_hidden_field_autocomplete, default: false

    class << self
      delegate :erb_trim_mode=, to: "ReactionBlur::Template::Handlers::ERB"

      def cache_template_loading
        ReactionBlur::Resolver.caching?
      end

      def cache_template_loading=(value)
        ReactionBlur::Resolver.caching = value
      end
      ...
      def with_empty_template_cache # :nodoc:
        subclass = Class.new(self) {
          ...
          def inspect
            "#<ReactionBlur::Base:#{'%#016x' % (object_id << 1)}>"
          end
        }
      end
    ...
    def self.with_view_paths(view_paths, assigns = {}, controller = nil)
      with_context ReactionBlur::LookupContext.new(view_paths), assigns, controller
    end
    ...
    def initialize(lookup_context, assigns, controller) # :nodoc:
      ...
      @view_renderer = ReactionBlur::Renderer.new @lookup_context
    ...
    def compiled_method_container
      raise NotImplementedError, <<~msg.squish
        Subclasses of ReactionBlur::Base must implement `compiled_method_container`
        or use the class method `with_empty_template_cache` for constructing
        an ReactionBlur::Base subclass that has an empty cache.
      msg
    end
    ...
    ActiveSupport.run_load_hooks(:reaction_blur, self)
  end
end
