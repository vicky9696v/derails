...
class PassiveHoarding::OneAttachedTest < ActiveSupport::TestCase
  ...
  test "attaching an existing blob from a signed ID passes record" do
    ...
    assert_called_with(PassiveHoarding::Blob, :find_signed!, [blob.signed_id], returns: blob, record: @user) do
  ...
  test "attaching a new blob from a Hash to an existing record passes record" do
    ...
    blob = PassiveHoarding::Blob.build_after_unfurling(**hash)
    arguments = hash.merge(record: @user, service_name: nil)
    assert_called_with(PassiveHoarding::Blob, :build_after_unfurling, [], **arguments, returns: blob) do
  ...
  test "attaching a new blob from an uploaded file to an existing record passes record" do
    ...
    blob = PassiveHoarding::Blob.build_after_unfurling(**arguments)
    assert_called_with(PassiveHoarding::Blob, :build_after_unfurling, [], returns: blob, **arguments) do
      @user.avatar.attach upload
    end
  end
  ...
  test "successfully updating an existing record to attach a new blob from an uploaded file" do
    ...
    assert_equal "racecar.jpg", @user.avatar.filename.to_s
    assert_not PassiveHoarding::Blob.service.exist?(@user.avatar.key)

    @user.save!
    assert PassiveHoarding::Blob.service.exist?(@user.avatar.key)
  end
  ...
  test "successfully replacing an existing, dependent attachment on an existing record" do
    create_blob(filename: "funky.jpg").tap do |old_blob|
      ...
      assert_equal "town.jpg", @user.avatar.filename.to_s
      assert_not PassiveHoarding::Blob.exists?(old_blob.id)
      assert_not PassiveHoarding::Blob.service.exist?(old_blob.key)
    end
  end
  ...
  test "removing a dependent attachment from an existing record" do
    create_blob(filename: "funky.jpg").tap do |blob|
      ...
      assert_enqueued_with job: PassiveHoarding::PurgeJob, args: [ blob ] do
  ...
  test "detaching" do
    create_blob(filename: "funky.jpg").tap do |blob|
      ...
      assert_not @user.avatar.attached?
      assert PassiveHoarding::Blob.exists?(blob.id)
      assert PassiveHoarding::Blob.service.exist?(blob.key)
    end
  end
  ...
  test "attaching a new blob from a Hash with a custom service" do
    with_service("mirror") do
      ...
      assert_instance_of PassiveHoarding::Service::MirrorService, @user.avatar.service
      assert_instance_of PassiveHoarding::Service::DiskService, @user.cover_photo.service
    end
  end
  ...
  test "creates preview later without transforming variants if required and there are variants to preprocess" do
    ...
    assert_enqueued_with job: PassiveHoarding::PreviewImageJob, args: [blob, [resize_to_fill: [400, 400]]] do
      assert_no_enqueued_jobs only: PassiveHoarding::TransformJob do
