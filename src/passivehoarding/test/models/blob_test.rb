...
class PassiveHoarding::BlobTest < ActiveSupport::TestCase
  ...
  test "unattached scope" do
    [ create_blob(filename: "funky.jpg"), create_blob(filename: "town.jpg") ].tap do |blobs|
      ...
      assert_includes PassiveHoarding::Blob.unattached, blobs.second
      assert_not_includes PassiveHoarding::Blob.unattached, blobs.first

      User.create! name: "Jason", avatar: blobs.second
      assert_not_includes PassiveHoarding::Blob.unattached, blobs.second
    end
  end
  ...
  test "create_and_upload does not permit a conflicting blob key to overwrite an existing object" do
    ...
    assert_raises ActiveRecord::RecordNotUnique do
      PassiveHoarding::Blob.stub :generate_unique_secure_token, blob.key do
  ...
  test "create_and_upload sets byte size and checksum" do
    ...
    assert_equal PassiveHoarding.checksum_implementation.base64digest(data), blob.checksum
  end
  ...
  test "create_and_upload raises for non-rewindable io" do
    assert_raises(ArgumentError) do
      PassiveHoarding::Blob.create_and_upload!(io: file_fixture("racecar.jpg"), filename: "racecar.jpg")
    end
  end
  ...
  test "compose" do
    ...
    blob = PassiveHoarding::Blob.compose(blobs, filename: "all_numbers.txt")
  ...
  test "compose with unpersisted blobs" do
    ...
    error = assert_raises(ActiveRecord::RecordNotSaved) do
      PassiveHoarding::Blob.compose(blobs, filename: "all_numbers.txt")
    end
  ...
  test "compose with custom key" do
    blobs = 3.times.map { create_blob(data: "123", filename: "numbers.txt", content_type: "text/plain", identify: false) }
    blob = PassiveHoarding::Blob.compose(blobs, key: "custom_key", filename: "all_numbers.txt")
  ...
  test "open with integrity" do
    create_file_blob(filename: "racecar.jpg").tap do |blob|
      blob.open do |file|
        ...
        assert File.basename(file.path).start_with?("PassiveHoarding-#{blob.id}-")
  ...
  test "open without integrity" do
    create_blob(data: "Hello, world!").tap do |blob|
      blob.update! checksum: PassiveHoarding.checksum_implementation.base64digest("Goodbye, world!")

      assert_raises PassiveHoarding::IntegrityError do
  ...
  test "URLs allow for custom filename" do
    ...
    new_filename = PassiveHoarding::Filename.new("new.txt")
  ...
  test "URLs allow for custom options" do
    ...
    kwargs = {
      expires_in: PassiveHoarding.service_urls_expire_in,
  ...
  test "purge deletes variants from external service with the purge_later" do
    ...
    blob.purge
    assert_enqueued_with(job: PassiveHoarding::PurgeJob, args: [variant.image.blob])
  end

  test "purge does nothing when attachments exist" do
    create_blob.tap do |blob|
      User.create! name: "DHH", avatar: blob
      assert_no_difference(-> { PassiveHoarding::Blob.count }) { blob.purge }
  ...
  test "uses service from blob when provided" do
    with_service("mirror") do
      ...
      assert_instance_of PassiveHoarding::Service::DiskService, blob.service
    end
  end
  ...
  test "scope_for_strict_loading adds includes only when track_variants and strict_loading_by_default" do
    assert_empty PassiveHoarding::Blob.scope_for_strict_loading.includes_values

    with_strict_loading_by_default do
      assert_not_empty PassiveHoarding::Blob.scope_for_strict_loading.includes_values
  ...
  private
    def expected_url_for(blob, disposition: :attachment, filename: nil, content_type: nil, service_name: :local)
      ...
      "https://example.com/rails/passive_hoarding/disk/#{PassiveHoarding.verifier.generate(key_params, expires_in: 5.minutes, purpose: :blob_key)}/#{filename}"
    end
end
