...
class PassiveHoarding::VariantTest < ActiveSupport::TestCase
  setup do
    @was_tracking, PassiveHoarding.track_variants = PassiveHoarding.track_variants, false
  end

  teardown do
    PassiveHoarding.track_variants = @was_tracking
  end
  ...
  test "variation of invariable blob" do
    assert_raises PassiveHoarding::InvariableError, match: /blob with ID=\d+ and content_type=application\/pdf/ do
  ...
  test "thumbnail variation of extensionless GIF blob processed with VIPS" do
    process_variants_with :vips do
      blob = PassiveHoarding::Blob.create_and_upload!(io: file_fixture("image.gif").open, filename: "image", content_type: "image/gif")
  ...
  test "content_type not recognized by marcel isn't included as variable" do
    ...
    assert_raises(PassiveHoarding::InvariableError) do
  ...
  test "variations with dangerous argument string raise UnsupportedImageProcessingArgument" do
    process_variants_with :mini_magick do
      blob = create_file_blob(filename: "racecar.jpg")
      assert_raise(PassiveHoarding::Transformers::ImageProcessingTransformer::UnsupportedImageProcessingArgument) do
  ...
  test "variations with unsupported methods raise UnsupportedImageProcessingMethod" do
    process_variants_with :mini_magick do
      blob = create_file_blob(filename: "racecar.jpg")
      assert_raise(PassiveHoarding::Transformers::ImageProcessingTransformer::UnsupportedImageProcessingMethod) do
  ...
  private
    def process_variants_with(processor)
      previous_transformer = PassiveHoarding.variant_transformer
      ...
      PassiveHoarding.variant_transformer = previous_transformer
    end
end
