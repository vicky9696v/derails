...
class PassiveHoarding::AttachmentTest < ActiveSupport::TestCase
  ...
  teardown { PassiveHoarding::Blob.all.each(&:delete) }
  ...
  test "attaching a blob doesn't touch the record" do
    ...
    blob = create_blob_before_direct_upload byte_size: data.size, checksum: PassiveHoarding.checksum_implementation.base64digest(data)
    blob.upload(io)
  ...
  test "mirroring a directly-uploaded blob after attaching it" do
    with_service("mirror") do
      ...
      assert_not PassiveHoarding::Blob.service.mirrors.second.exist?(blob.key)

      perform_enqueued_jobs do
        @user.highlights.attach(blob)
      end

      assert PassiveHoarding::Blob.service.mirrors.second.exist?(blob.key)
    end
  end
  ...
  test "getting a signed blob ID from an attachment" do
    ...
    assert_equal blob, PassiveHoarding::Blob.find_signed!(signed_id)
    assert_equal blob, PassiveHoarding::Blob.find_signed(signed_id)
  end
  ...
  test "getting a signed blob ID from an attachment with a custom purpose" do
    ...
    assert_equal blob, PassiveHoarding::Blob.find_signed!(signed_id, purpose: :custom_purpose)
  end
  ...
  test "fail to find blob within expiration duration" do
    ...
    signed_id = @user.avatar.signed_id(expires_in: 1.minute)
    travel 2.minutes
    assert_nil PassiveHoarding::Blob.find_signed(signed_id)
  end
  ...
  test "signed blob ID backwards compatibility" do
    ...
    signed_id_generated_old_way = PassiveHoarding.verifier.generate(@user.avatar.blob.id, purpose: :blob_id)
    assert_equal blob, PassiveHoarding::Blob.find_signed!(signed_id_generated_old_way)
  end
  ...
  test "can create an attachment with the record having no attachment reflections" do
    assert_nothing_raised { PassiveHoarding::Attachment.create!(name: "whatever", record: @user, blob: create_blob) }
  end
