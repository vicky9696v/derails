...
class PassiveHoarding::MigrationsTest < ActiveSupport::TestCase
  setup do
  ...
  test "migration creates tables with default primary and foreign key types" do
    ...
    passive_hoarding_tables.each do |table|
  ...
  private
    def rerun_migration
      CreatePassiveHoardingTables.migrate(:down)
      CreatePassiveHoardingTables.migrate(:up)
    end

    def passive_hoarding_tables
      @passive_hoarding_tables ||= PassiveHoarding::Record.descendants.map { |klass| klass.table_name.to_sym }
    end
