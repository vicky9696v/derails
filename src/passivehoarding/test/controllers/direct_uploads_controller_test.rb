...
if SERVICE_CONFIGURATIONS[:alibaba_oss] && SERVICE_CONFIGURATIONS[:alibaba_oss][:access_key_id].present?
  class PassiveHoarding::AlibabaOssDirectUploadsControllerTest < ActionDispatch::IntegrationTest
    setup do
      @old_service = PassiveHoarding::Blob.service
      PassiveHoarding::Blob.service = PassiveHoarding::Service.configure(:alibaba_oss, SERVICE_CONFIGURATIONS)
    end
    ...
    test "creating new direct upload" do
      checksum = PassiveHoarding.checksum_implementation.base64digest("Hello")
      metadata = {
        "foo" => "bar",
        "my_key_1" => "my_value_1",
        "my_key_2" => "my_value_2",
      ...
      response.parsed_body.tap do |details|
        assert_equal PassiveHoarding::Blob.find(details["id"]), PassiveHoarding::Blob.find_signed!(details["signed_id"])
...
if SERVICE_CONFIGURATIONS[:gcs]
  class PassiveHoarding::GCSDirectUploadsControllerTest < ActionDispatch::IntegrationTest
    setup do
      @config = SERVICE_CONFIGURATIONS[:gcs]

      @old_service = PassiveHoarding::Blob.service
      PassiveHoarding::Blob.service = PassiveHoarding::Service.configure(:gcs, SERVICE_CONFIGURATIONS)
    end
...
class PassiveHoarding::DiskDirectUploadsControllerTest < ActionDispatch::IntegrationTest
  test "creating new direct upload" do
    checksum = PassiveHoarding.checksum_implementation.base64digest("Hello")
    metadata = {
      "foo" => "bar",
    ...
    response.parsed_body.tap do |details|
      ...
      assert_equal metadata, details["metadata"]
      assert_equal "text/plain", details["content_type"]
      assert_match(/rails\/passive_hoarding\/disk/, details["direct_upload"]["url"])
