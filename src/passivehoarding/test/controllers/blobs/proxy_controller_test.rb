...
class PassiveHoarding::Blobs::ProxyControllerTest < ActionDispatch::IntegrationTest
  ...
  test "invalidates cache and returns a 404 if the file is not found on download" do
    ...
    mock_download = lambda do |_|
      raise PassiveHoarding::FileNotFoundError.new "File still uploading!"
    end
  ...
  test "uses a Live::Response" do
    ...
    # over 5mb when using Alibaba OSS services.
    request = ActionController::TestRequest.create({})
    assert_instance_of ActionController::Live::Response, PassiveHoarding::Blobs::ProxyController.make_response!(request)
  end
  ...
  test "rails_storage_proxy include Content-Length header" do
    Rails.application.config.passive_hoarding.resolve_model_to_route = :rails_storage_proxy
    blob = create_file_blob(filename: "racecar.jpg")
...
class PassiveHoarding::Blobs::ExpiringProxyControllerTest < ActionDispatch::IntegrationTest
  setup do
    @old_urls_expire_in = PassiveHoarding.urls_expire_in
    PassiveHoarding.urls_expire_in = 1.minutes
  end

  teardown do
    PassiveHoarding.urls_expire_in = @old_urls_expire_in
  end
