...
class PassiveHoarding::TransformJobTest < ActiveJob::TestCase
  ...
  test "creates variant" do
    ...
    assert_changes -> { @blob.variant(transformations).send(:processed?) }, from: false, to: true do
      perform_enqueued_jobs do
        PassiveHoarding::TransformJob.perform_later @blob, transformations
      end
    end
  end
  ...
  test "ignores unrepresentable blob" do
    ...
    perform_enqueued_jobs do
      assert_nothing_raised do
        PassiveHoarding::TransformJob.perform_later unrepresentable_blob, transformations
      end
    end
  end

  test "null transformer returns original file" do
    @was_transformer = PassiveHoarding.variant_transformer
    PassiveHoarding.variant_transformer = PassiveHoarding::Transformers::NullTransformer
    ...
    PassiveHoarding.variant_transformer = @was_transformer
  end
end
