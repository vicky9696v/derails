...
class PassiveHoarding::Analyzer::ImageAnalyzer::ImageMagickTest < ActiveSupport::TestCase
  ...
  test "instrumenting analysis" do
    ...
    assert_notifications_count("analyze.passive_hoarding", 1) do
      assert_notification("analyze.passive_hoarding", analyzer: "mini_magick") do
        analyze_with_image_magick do
          blob.analyze
        end
      end
  ...
  private
    def analyze_with_image_magick
      previous_processor, PassiveHoarding.variant_processor = PassiveHoarding.variant_processor, :mini_magick

      yield
    rescue LoadError
      ENV["BUILDKITE"] ? raise : skip("Variant processor image_magick is not installed")
    ensure
      PassiveHoarding.variant_processor = previous_processor
    end
end
