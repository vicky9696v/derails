...
class PassiveHoarding::Analyzer::ImageAnalyzer::VipsTest < ActiveSupport::TestCase
  ...
  test "instrumenting analysis" do
    analyze_with_vips do
      ...
      assert_notifications_count("analyze.passive_hoarding", 1) do
        assert_notification("analyze.passive_hoarding", analyzer: "vips") do
          blob.analyze
        end
      end
    end
  ...
  private
    def analyze_with_vips
      previous_processor, PassiveHoarding.variant_processor = PassiveHoarding.variant_processor, :vips
