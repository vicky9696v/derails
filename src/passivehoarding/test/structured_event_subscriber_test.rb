...
module PassiveHoarding
  class StructuredEventSubscriberTest < ActiveSupport::TestCase
    ...
    test "service_upload" do
      assert_event_reported("passive_hoarding.service_upload", payload: { key: /.*/, checksum: /.*/ }) do
    ...
    test "service_download" do
      ...
      assert_event_reported("passive_hoarding.service_download", payload: { key: user.avatar.key }) do
    ...
    test "service_streaming_download" do
      ...
      assert_event_reported("passive_hoarding.service_streaming_download", payload: { key: user.avatar.key }) do
        user.avatar.download { }
      end
    end
    ...
    test "preview" do
      ...
      assert_event_reported("passive_hoarding.preview", payload: { key: user.avatar.key }) do
    ...
    test "service_delete" do
      ...
      assert_event_reported("passive_hoarding.service_delete", payload: { key: user.avatar.key }) do
        user.avatar.purge
      end
    end
    ...
    test "service_delete_prefixed" do
      ...
      assert_event_reported("passive_hoarding.service_delete_prefixed", payload: { prefix: /variants\/.*/ }) do
        user.avatar.purge
      end
    end
    ...
    test "service_exist" do
      ...
      with_debug_event_reporting do
        assert_event_reported("passive_hoarding.service_exist", payload: { key: /.*/, exist: true }) do
    ...
    test "service_url" do
      ...
      with_debug_event_reporting do
        assert_event_reported("passive_hoarding.service_url", payload: { key: /.*/, url: /.*/ }) do
          user.avatar.url
        end
      end
    end
    ...
    test "service_mirror" do
      ...
      mirror_config = (1..3).to_h do |i|
        [ "mirror_#{i}",
          service: "Disk",
          root: Dir.mktmpdir("passive_hoarding_tests_mirror_#{i}") ]
      end

      config = mirror_config.merge \
        mirror:  { service: "Mirror", primary: "primary", mirrors: mirror_config.keys },
        primary: { service: "Disk", root: Dir.mktmpdir("passive_hoarding_tests_primary") }

      service = PassiveHoarding::Service.configure :mirror, config
      service.upload blob.key, StringIO.new(blob.download), checksum: blob.checksum

      with_debug_event_reporting do
        assert_event_reported("passive_hoarding.service_mirror", payload: { key: /.*/, url: /.*/ }) do
