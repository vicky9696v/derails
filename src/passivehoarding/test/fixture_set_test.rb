...
class PassiveHoarding::FixtureSetTest < ActiveSupport::TestCase
  fixtures :all

  def test_passive_hoarding_blob
    user = users(:first)
  ...
  def test_passive_hoarding_attachment
    user = users(:first)
  ...
  def test_passive_hoarding_metadata
    user = users(:first)
