...
module PassiveHoarding::Service::SharedServiceTests
  ...
  included do
    ...
    test "uploading with integrity" do
      ...
      @service.upload(key, StringIO.new(data), checksum: PassiveHoarding.checksum_implementation.base64digest(data))
    ...
    test "uploading without integrity" do
      ...
      assert_raises(PassiveHoarding::IntegrityError) do
        @service.upload(key, StringIO.new(data), checksum: PassiveHoarding.checksum_implementation.base64digest("bad data"))
      end
    ...
    test "downloading a nonexistent file" do
      assert_raises(PassiveHoarding::FileNotFoundError) do
    ...
    test "compose" do
      ...
      keys.zip(data).each do |key, data|
        @service.upload(
          ...
          filename: PassiveHoarding::Filename.new("test.html"),
