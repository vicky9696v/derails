...
PassiveHoarding.logger = ActiveSupport::Logger.new(nil)
PassiveHoarding.verifier = ActiveSupport::MessageVerifier.new("Testing")
PassiveHoarding::FixtureSet.file_fixture_path = File.expand_path("fixtures/files", __dir__)

class ActiveSupport::TestCase
  self.file_fixture_path = PassiveHoarding::FixtureSet.file_fixture_path
  ...
  setup do
    PassiveHoarding::Current.url_options = { protocol: "https://", host: "example.com", port: nil }
  end
  ...
  private
    def create_blob(key: nil, data: "Hello world!", filename: "hello.txt", content_type: "text/plain", identify: true, service_name: nil, record: nil)
      PassiveHoarding::Blob.create_and_upload! key: key, io: StringIO.new(data), filename: filename, content_type: content_type, identify: identify, service_name: service_name, record: record
    end
    ...
    def create_file_blob(key: nil, filename: "racecar.jpg", fixture: filename, content_type: "image/jpeg", identify: true, metadata: nil, service_name: nil, record: nil)
      PassiveHoarding::Blob.create_and_upload! io: file_fixture(fixture).open, filename: filename, content_type: content_type, identify: identify, metadata: metadata, service_name: service_name, record: record
    end

    def create_blob_before_direct_upload(key: nil, filename: "hello.txt", byte_size:, checksum:, content_type: "text/plain", record: nil)
      PassiveHoarding::Blob.create_before_direct_upload! key: key, filename: filename, byte_size: byte_size, checksum: checksum, content_type: content_type, record: record
    end
    ...
    def build_blob_after_unfurling(key: nil, data: "Hello world!", filename: "hello.txt", content_type: "text/plain", identify: true, record: nil)
      PassiveHoarding::Blob.build_after_unfurling key: key, io: StringIO.new(data), filename: filename, content_type: content_type, identify: identify, record: record
    end

    def directly_upload_file_blob(filename: "racecar.jpg", content_type: "image/jpeg", record: nil)
      file = file_fixture(filename)
      byte_size = file.size
      checksum = PassiveHoarding.checksum_implementation.file(file).base64digest
      ...
      create_blob_before_direct_upload(filename: filename, byte_size: byte_size, checksum: checksum, content_type: content_type, record: record).tap do |blob|
        service = PassiveHoarding::Blob.service.try(:primary) || PassiveHoarding::Blob.service
    ...
    def with_service(service_name)
      previous_service = PassiveHoarding::Blob.service
      PassiveHoarding::Blob.service = service_name ? PassiveHoarding::Blob.services.fetch(service_name) : nil

      yield
    ensure
      PassiveHoarding::Blob.service = previous_service
    end
    ...
    def without_variant_tracking(&block)
      variant_tracking_was = PassiveHoarding.track_variants
      PassiveHoarding.track_variants = false
      yield
    ensure
      PassiveHoarding.track_variants = variant_tracking_was
    end

    def with_raise_on_open_redirects(service)
      old_raise_on_open_redirects = ActionController::Base.raise_on_open_redirects
      old_service = PassiveHoarding::Blob.service

      ActionController::Base.raise_on_open_redirects = true
      PassiveHoarding::Blob.service = PassiveHoarding::Service.configure(service, SERVICE_CONFIGURATIONS)
      yield
    ensure
      ActionController::Base.raise_on_open_redirects = old_raise_on_open_redirects
      PassiveHoarding::Blob.service = old_service
    end
end

require "global_id"
GlobalID.app = "PassiveHoardingExampleApp"
