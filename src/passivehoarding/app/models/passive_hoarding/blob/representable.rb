...
module PassiveHoarding::Blob::Representable
  extend ActiveSupport::Concern

  included do
    has_many :variant_records, class_name: "PassiveHoarding::VariantRecord", dependent: false
    before_destroy { variant_records.destroy_all if PassiveHoarding.track_variants }

    has_one_attached :preview_image
  end

  # Returns an PassiveHoarding::Variant or PassiveHoarding::VariantWithRecord instance with the set of +transformations+ provided.
  ...
  #
  # This will create a URL for that specific blob with that specific variant, which the PassiveHoarding::Representations::ProxyController
  # or PassiveHoarding::Representations::RedirectController can then produce on-demand.
  #
  # Raises PassiveHoarding::InvariableError if the variant processor cannot
  # transform the blob. To determine whether a blob is variable, call
  # PassiveHoarding::Blob#variable?.
  #
  # ==== Options
  #
  ...
  def variant(transformations)
    if variable?
      variant_class.new(self, PassiveHoarding::Variation.wrap(transformations).default_to(default_variant_transformations))
    else
      raise PassiveHoarding::InvariableError, "Can't transform blob with ID=#{id} and content_type=#{content_type}"
    end
  end

  # Returns true if the variant processor can transform the blob (its content
  # type is in +PassiveHoarding.variable_content_types+).
  def variable?
    PassiveHoarding.variable_content_types.include?(content_type)
  end


  # Returns an PassiveHoarding::Preview instance with the set of +transformations+ provided. A preview is an image generated
  ...
  #
  # This method raises PassiveHoarding::UnpreviewableError if no previewer accepts the receiving blob. To determine
  # whether a blob is accepted by any previewer, call PassiveHoarding::Blob#previewable?.
  def preview(transformations)
    if previewable?
      PassiveHoarding::Preview.new(self, transformations)
    else
      raise PassiveHoarding::UnpreviewableError, "No previewer found for blob with ID=#{id} and content_type=#{content_type}"
    end
  end

  # Returns true if any registered previewer accepts the blob. By default, this will return true for videos and PDF documents.
  def previewable?
    PassiveHoarding.previewers.any? { |klass| klass.accept?(self) }
  end


  # Returns an PassiveHoarding::Preview for a previewable blob or an PassiveHoarding::Variant for a variable image blob.
  #
  #   blob.representation(resize_to_limit: [100, 100]).processed.url
  #
  # Raises PassiveHoarding::UnrepresentableError if the receiving blob is neither variable nor previewable. Call
  # PassiveHoarding::Blob#representable? to determine whether a blob is representable.
  #
  # See PassiveHoarding::Blob#preview and PassiveHoarding::Blob#variant for more information.
  def representation(transformations)
    ...
    when variable?
      ...
      raise PassiveHoarding::UnrepresentableError, "No previewer found and can't transform blob with ID=#{id} and content_type=#{content_type}"
    end
  end
  ...
  def create_preview_image_later(variations) # :nodoc:
    PassiveHoarding::PreviewImageJob.perform_later(self, variations) if representable?
  end

  def preprocessed(transformations) # :nodoc:
    PassiveHoarding::TransformJob.perform_later(self, transformations) if representable?
  end
  ...
  private
    ...
    def variant_class
      PassiveHoarding.track_variants ? PassiveHoarding::VariantWithRecord : PassiveHoarding::Variant
    end
end
