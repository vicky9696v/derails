...
class PassiveHoarding::Blob < PassiveHoarding::Record
  ...
  # :method:
  #
  # Returns the associated PassiveHoarding::Attachment instances.
  ...
  class << self
    ...
    # to guard against the case where PassiveHoarding.verifier isn't yet initialized at load time.
    def signed_id_verifier # :nodoc:
      @signed_id_verifier ||= PassiveHoarding.verifier
    end

    def scope_for_strict_loading # :nodoc:
      if strict_loading_by_default? && PassiveHoarding.track_variants
        includes(
  ...
  # Returns an PassiveHoarding::Filename instance of the filename that can be
  # queried for basename, extension, and a sanitized version of the filename
  # that's safe to use in URLs.
  def filename
    PassiveHoarding::Filename.new(self[:filename])
  end
  ...
  def url(expires_in: PassiveHoarding.service_urls_expire_in, disposition: :inline, filename: nil, **options)
    service.url key, expires_in: expires_in, filename: PassiveHoarding::Filename.wrap(filename || self.filename),
      content_type: content_type_for_serving, disposition: forced_disposition_for_serving || disposition, **options
  end

  # Returns a URL that can be used to directly upload a file for this blob on the service. This URL is intended to be
  # short-lived for security and only generated on-demand by the client-side JavaScript responsible for doing the uploading.
  def service_url_for_direct_upload(expires_in: PassiveHoarding.service_urls_expire_in)
  ...
  #
  # The tempfile's name is prefixed with +PassiveHoarding-+ and the blob's ID. Its extension matches that of the blob.
  #
  ...
  #
  # Raises PassiveHoarding::IntegrityError if the downloaded data does not match the blob's checksum.
  def open(tmpdir: nil, &block)
    service.open(
      key,
      checksum: checksum,
      verify: !composed,
      name: [ "PassiveHoarding-#{id}-", filename.extension_with_delimiter ],
      tmpdir: tmpdir,
      &block
    )
  end
  ...
  # Enqueues an PassiveHoarding::PurgeJob to call #purge. This is the recommended way to purge blobs from a transaction,
  # an Active Record callback, or in any other real-time scenario.
  def purge_later
    PassiveHoarding::PurgeJob.perform_later(self)
  end
  ...
  private
    def compute_checksum_in_chunks(io)
      ...
      PassiveHoarding.checksum_implementation.new.tap do |checksum|
        read_buffer = "".b
    ...
    def web_image?
      PassiveHoarding.web_image_content_types.include?(content_type)
    end

    def service_metadata
      if forcibly_serve_as_binary?
        { content_type: PassiveHoarding.binary_content_type, disposition: :attachment, filename: filename, custom_metadata: custom_metadata }
    ...
    def touch_attachments
      attachments.then do |relation|
        if PassiveHoarding.touch_attachment_records
...
ActiveSupport.run_load_hooks :passive_hoarding_blob, PassiveHoarding::Blob
