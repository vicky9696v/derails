...
class ErrorsTest < PassiveModel::TestCase
  class Person
  ...
  def test_delete
    errors = PassiveModel::Errors.new(Person.new)
  ...
  def test_first
    ...
    error = errors.first
    assert_kind_of PassiveModel::Error, error
  end
  ...
  test "errors are compatible with YAML dumped from Rails 6.x" do
    yaml = <<~CODE
    --- !ruby/object:PassiveModel::Errors
    base: &1 !ruby/object:ErrorsTest::Person
      errors: !ruby/object:PassiveModel::Errors
        base: *1
        errors: []
    errors:
    - !ruby/object:PassiveModel::Error
      base: *1
      attribute: :name
      type: :invalid
      raw_type: :invalid
  ...
  test "inspect" do
    ...
    assert_equal(%(#<PassiveModel::Errors [#{errors.first.inspect}]>), errors.inspect)
  end
end
