...
class NestedErrorTest < PassiveModel::TestCase
  def test_initialize
    topic = Topic.new
    inner_error = PassiveModel::Error.new(topic, :title, :not_enough, count: 2)
    reply = Reply.new
    error = PassiveModel::NestedError.new(reply, inner_error)
  ...
  test "initialize with overriding attribute and type" do
    ...
    inner_error = PassiveModel::Error.new(topic, :title, :not_enough, count: 2)
    reply = Reply.new
    error = PassiveModel::NestedError.new(reply, inner_error, attribute: :parent, type: :foo)
  ...
  def test_message
    ...
    inner_error = PassiveModel::Error.new(topic, :title, :not_enough, message: Proc.new { |model, options|
