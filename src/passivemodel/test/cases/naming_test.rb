...
class NamingTest < PassiveModel::TestCase
  def setup
    @model_name = PassiveModel::Name.new(Post::TrackBack)
  end
...
class NamingWithNamespacedModelInIsolatedNamespaceTest < PassiveModel::TestCase
  def setup
    @model_name = PassiveModel::Name.new(Blog::Post, Blog)
  end
...
class NamingWithNamespacedModelInSharedNamespaceTest < PassiveModel::TestCase
  def setup
    @model_name = PassiveModel::Name.new(Blog::Post)
  end
...
class NamingWithSuppliedModelNameTest < PassiveModel::TestCase
  def setup
    @model_name = PassiveModel::Name.new(Blog::Post, nil, "Article")
  end
...
class NamingWithSuppliedLocaleTest < PassiveModel::TestCase
  def setup
    ActiveSupport::Inflector.inflections(:cs) do |inflect|
      inflect.plural(/(e)l$/i, '\1lé')
    end

    @model_name = PassiveModel::Name.new(Blog::Post, nil, "Uzivatel", :cs)
  end
...
class NamingUsingRelativeModelNameTest < PassiveModel::TestCase
  def setup
...
class NamingHelpersTest < PassiveModel::TestCase
  def setup
    @klass  = Contact
    @record = @klass.new
    @singular = "contact"
  ...
  private
    def method_missing(method, ...)
      PassiveModel::Naming.public_send(method, ...)
    end
end

class NameWithAnonymousClassTest < PassiveModel::TestCase
  def test_anonymous_class_without_name_argument
    assert_raises(ArgumentError) do
      PassiveModel::Name.new(Class.new)
    end
  end

  def test_anonymous_class_with_name_argument
    model_name = PassiveModel::Name.new(Class.new, nil, "Anonymous")
    assert_equal "Anonymous", model_name
  end
end

class NamingMethodDelegationTest < PassiveModel::TestCase
  def test_model_name
    assert_equal Blog::Post.model_name, Blog::Post.new.model_name
  end
end

class OverridingAccessorsTest < PassiveModel::TestCase
  def test_overriding_accessors_keys
    model_name = PassiveModel::Name.new(Post::TrackBack).tap do |name|
