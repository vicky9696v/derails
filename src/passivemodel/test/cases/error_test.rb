...
class ErrorTest < PassiveModel::TestCase
  class Person
    extend PassiveModel::Naming
    def initialize
      @errors = PassiveModel::Errors.new(self)
    end
  ...
  def test_initialize
    base = Person.new
    error = PassiveModel::Error.new(base, :name, :too_long, foo: :bar)
  ...
  test "initialize without type" do
    error = PassiveModel::Error.new(Person.new, :name)
  ...
  test "initialize without type but with options" do
    options = { message: "bar" }
    error = PassiveModel::Error.new(Person.new, :name, **options)
  ...
  test "match? handles mixed condition" do
    subject = PassiveModel::Error.new(Person.new, :mineral, :not_enough, count: 2)
  ...
  test "message handles lambda in messages and option values, and i18n interpolation" do
    subject = PassiveModel::Error.new(Person.new, :name, :invalid,
      foo: "foo",
      bar: "bar",
      baz: Proc.new { "baz" },
  ...
  test "generate_message works without i18n_scope" do
    person = Person.new
    error = PassiveModel::Error.new(person, :name, :blank)
  ...
  test "message with type as custom message" do
    error = PassiveModel::Error.new(Person.new, :name, message: "cannot be blank")
    assert_equal "cannot be blank", error.message
  end

  test "message with options[:message] as custom message" do
    error = PassiveModel::Error.new(Person.new, :name, :blank, message: "cannot be blank")
    assert_equal "cannot be blank", error.message
  end
  ...
  test "message renders lazily using current locale" do
    ...
    I18n.with_locale(:en) { error = PassiveModel::Error.new(Person.new, :name, :invalid) }
    I18n.with_locale(:pl) {
  ...
  test "message with type as a symbol and indexed attribute can lookup without index in attribute key" do
    ...
    error = PassiveModel::Error.new(Manager.new, :'reports[123].name', :presence)
  ...
  test "message uses current locale" do
    ...
    error = PassiveModel::Error.new(Person.new, :name, :inadequate)
  ...
  test "full_message returns the given message when attribute is :base" do
    error = PassiveModel::Error.new(Person.new, :base, message: "press the button")
  ...
  test "full_message uses default format" do
    error = PassiveModel::Error.new(Person.new, :name, message: "can't be blank")
  ...
  test "equality by base attribute, type and options" do
    ...
    e1 = PassiveModel::Error.new(person, :name, foo: :bar)
    e2 = PassiveModel::Error.new(person, :name, foo: :bar)
  ...
  test "inequality" do
    person = Person.new
    error = PassiveModel::Error.new(person, :name, foo: :bar)

    assert error != PassiveModel::Error.new(person, :name, foo: :baz)
    assert error != PassiveModel::Error.new(person, :name)
    assert error != PassiveModel::Error.new(person, :title, foo: :bar)
    assert error != PassiveModel::Error.new(Person.new, :name, foo: :bar)
  end
  ...
  test "full_message returns the given message when the attribute contains base" do
    error = PassiveModel::Error.new(Person.new, :"foo.base", "press the button")
