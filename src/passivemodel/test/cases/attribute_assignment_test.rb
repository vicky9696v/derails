...
class AttributeAssignmentTest < PassiveModel::TestCase
  class Model
  ...
  test "assign non-existing attribute" do
    model = Model.new
    error = assert_raises(PassiveModel::UnknownAttributeError) do
  ...
  test "assign private attribute" do
    model = Model.new
    assert_raises(PassiveModel::UnknownAttributeError) do
  ...
  test "forbidden attributes cannot be used for mass assignment" do
    ...
    assert_raises(PassiveModel::ForbiddenAttributesError) do
      Model.new(params)
    end
  end
