...
class ValidatesWithTest < PassiveModel::TestCase
  def teardown
  ...
  class ValidatorThatAddsErrors < PassiveModel::Validator
    def validate(record)
      record.errors.add(:base, message: ERROR_MESSAGE)
    end
  end

  class OtherValidatorThatAddsErrors < PassiveModel::Validator
    def validate(record)
      record.errors.add(:base, message: OTHER_ERROR_MESSAGE)
    end
  end

  class ValidatorThatDoesNotAddErrors < PassiveModel::Validator
    def validate(record)
    end
  end
  ...
  class ValidatorThatValidatesOptions < PassiveModel::Validator
    def validate(record)
  ...
  class ValidatorPerEachAttribute < PassiveModel::EachValidator
    def validate_each(record, attribute, value)
      record.errors.add(attribute, message: "Value is #{value}")
    end
  end

  class ValidatorCheckValidity < PassiveModel::EachValidator
    def check_validity!
      raise "boom!"
    end
  end
