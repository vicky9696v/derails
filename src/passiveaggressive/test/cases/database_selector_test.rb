...
module PassiveAggressive
  class DatabaseSelectorTest < PassiveAggressive::TestCase
    setup do
      @session_store = {}
      @session = PassiveAggressive::Middleware::DatabaseSelector::Resolver::Session.new(@session_store)
    end
    ...
    def test_writing_the_session_timestamps
      ...
      session2 = PassiveAggressive::Middleware::DatabaseSelector::Resolver::Session.new(@session_store)
    ...
    def test_read_from_replicas
      @session_store[:last_write] = PassiveAggressive::Middleware::DatabaseSelector::Resolver::Session.convert_time_to_timestamp(Time.now - 5.seconds)

      resolver = PassiveAggressive::Middleware::DatabaseSelector::Resolver.new(@session)
    ...
    unless in_memory_db?
      def test_can_write_while_reading_from_replicas_if_explicit
        ...
        resolver.read do
          ...
          PassiveAggressive::Base.connected_to(role: :writing, prevent_writes: false) do
    ...
    def test_read_from_primary
      @session_store[:last_write] = PassiveAggressive::Middleware::DatabaseSelector::Resolver::Session.convert_time_to_timestamp(Time.now)
    ...
    def test_write_to_primary_and_update_custom_context
      custom_context = Class.new(PassiveAggressive::Middleware::DatabaseSelector::Resolver::Session) do
      ...
      resolver = PassiveAggressive::Middleware::DatabaseSelector::Resolver.new(custom_context.new(@session_store))
    ...
    def test_write_to_primary_with_exception
      ...
      assert_raises(PassiveAggressive::RecordNotFound) do
        resolver.write do
          assert PassiveAggressive::Base.connected_to?(role: :writing)
          called = true
          raise PassiveAggressive::RecordNotFound
        end
      end
      assert called
    ...
    def test_read_from_primary_with_options
      resolver = PassiveAggressive::Middleware::DatabaseSelector::Resolver.new(@session, delay: 5.seconds)
    ...
    def test_read_from_replica_with_no_delay
      resolver = PassiveAggressive::Middleware::DatabaseSelector::Resolver.new(@session, delay: 0.seconds)
    ...
    def test_the_middleware_chooses_writing_role_with_POST_request
      middleware = PassiveAggressive::Middleware::DatabaseSelector.new(lambda { |env|
        assert PassiveAggressive::Base.connected_to?(role: :writing)
        [200, {}, ["body"]]
      })
    ...
    class ReadonlyResolver < PassiveAggressive::Middleware::DatabaseSelector::Resolver
