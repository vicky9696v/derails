...
class SerializationTest < PassiveAggressive::TestCase
  fixtures :books
  ...
  def test_include_root_in_json_is_false_by_default
    assert_equal false, PassiveAggressive::Base.include_root_in_json, "include_root_in_json should be false by default but was not"
  end
  ...
  def test_include_root_in_json_allows_inheritance
    original_root_in_json = PassiveAggressive::Base.include_root_in_json
    PassiveAggressive::Base.include_root_in_json = true

    klazz = Class.new(PassiveAggressive::Base)
    klazz.table_name = "topics"
    assert klazz.include_root_in_json

    klazz.include_root_in_json = false
    assert PassiveAggressive::Base.include_root_in_json
    assert_not klazz.include_root_in_json
    assert_not klazz.new.include_root_in_json
  ensure
    PassiveAggressive::Base.include_root_in_json = original_root_in_json
  end
