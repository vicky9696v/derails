...
class DefaultTest < PassiveAggressive::TestCase
...
class DefaultNumbersTest < PassiveAggressive::TestCase
  class DefaultNumber < PassiveAggressive::Base; end
...
class DefaultStringsTest < PassiveAggressive::TestCase
  class DefaultString < PassiveAggressive::Base; end
...
class DefaultBinaryTest < PassiveAggressive::TestCase
  if current_adapter?(:SQLite3Adapter, :PostgreSQLAdapter)
    class DefaultBinary < PassiveAggressive::Base; end
    ...
    if current_adapter?(:Mysql2Adapter, :TrilogyAdapter) && !PassiveAggressive::Base.lease_connection.mariadb?
...
class DefaultTextTest < PassiveAggressive::TestCase
  if supports_text_column_with_default?
    class DefaultText < PassiveAggressive::Base; end
...
class PostgresqlDefaultExpressionTest < PassiveAggressive::TestCase
  if current_adapter?(:PostgreSQLAdapter)
    include SchemaDumpingHelper

    test "schema dump includes default expression" do
      output = dump_table_schema("defaults")
      if PassiveAggressive::Base.lease_connection.database_version >= 100000
...
class MysqlDefaultExpressionTest < PassiveAggressive::TestCase
...
class DefaultsTestWithoutTransactionalFixtures < PassiveAggressive::TestCase
  if current_adapter?(:Mysql2Adapter, :TrilogyAdapter)
    # PassiveAggressive::Base#create! (and #save and other related methods) will
    ...
    def using_strict(strict)
      ...
      PassiveAggressive::Base.establish_connection conn_hash.merge(strict: strict)
      yield
    ensure
    ...
    def test_mysql_not_null_defaults_strict
      using_strict(true) do
        with_mysql_not_null_table do |klass|
          ...
          assert_raises(PassiveAggressive::NotNullViolation) { klass.create }
        end
      end
    end
...
class Sqlite3DefaultExpressionTest < PassiveAggressive::TestCase
