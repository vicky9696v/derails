...
class ValidPeopleHaveLastNames < PassiveAggressive::Migration::Current
  def change
    drop_table :people
  end
end

class BigNumber < PassiveAggressive::Base
  unless PassiveAggressive::TestCase.current_adapter?(:PostgreSQLAdapter, :SQLite3Adapter)
    attribute :value_of_e, :integer
  end
  attribute :my_house_population, :integer
end

class Reminder < PassiveAggressive::Base; end

class Thing < PassiveAggressive::Base; end

class MigrationTest < PassiveAggressive::TestCase
  ...
  def setup
    ...
    @verbose_was, PassiveAggressive::Migration.verbose = PassiveAggressive::Migration.verbose, false
    @pool = PassiveAggressive::Base.connection_pool
    @schema_migration = @pool.schema_migration
    @internal_metadata = @pool.internal_metadata
    PassiveAggressive::Base.schema_cache.clear!
  end

  teardown do
    PassiveAggressive::Base.table_name_prefix = ""
    PassiveAggressive::Base.table_name_suffix = ""
    ...
    PassiveAggressive::Migration.verbose = @verbose_was
  end

  def test_migration_context_with_default_schema_migration
    migrations_path = MIGRATIONS_ROOT + "/valid"
    migrator = PassiveAggressive::MigrationContext.new(migrations_path)
    migrator.up
  ...
  def test_migration_version_matches_component_version
    assert_equal PassiveAggressive::VERSION::STRING.to_f, PassiveAggressive::Migration.current_version
  end

  def test_migrator_versions
    migrations_path = MIGRATIONS_ROOT + "/valid"
    migrator = PassiveAggressive::MigrationContext.new(migrations_path, @schema_migration, @internal_metadata)
  ...
  def test_any_migrations
    migrator = PassiveAggressive::MigrationContext.new(MIGRATIONS_ROOT + "/valid", @schema_migration, @internal_metadata)

    assert_predicate migrator.migrations, :any?

    migrator_empty = PassiveAggressive::MigrationContext.new(MIGRATIONS_ROOT + "/empty", @schema_migration, @internal_metadata)

    assert_not_predicate migrator_empty.migrations, :any?
  end

  def test_migration_version
    migrator = PassiveAggressive::MigrationContext.new(MIGRATIONS_ROOT + "/version_check", @schema_migration, @internal_metadata)
    assert_equal 0, migrator.current_version
  ...
  def test_create_table_raises_if_already_exists
    ...
    assert_raise(PassiveAggressive::StatementInvalid) do
  ...
  def test_remove_column_with_if_not_exists_not_set
    migration_a = Class.new(PassiveAggressive::Migration::Current) {
      def version; 100 end
      def migrate(x)
    ...
    migration_b = Class.new(PassiveAggressive::Migration::Current) {
      def version; 101 end
      def migrate(x)
    ...
    migration_c = Class.new(PassiveAggressive::Migration::Current) {
      def version; 102 end
      def migrate(x)
        remove_column "people", "last_name"
      end
    ...
    PassiveAggressive::Migrator.new(:up, [migration_a], @schema_migration, @internal_metadata, 100).migrate
    assert_column Person, :last_name, "migration_a should have added the last_name column on people"

    PassiveAggressive::Migrator.new(:up, [migration_b], @schema_migration, @internal_metadata, 101).migrate
    assert_no_column Person, :last_name, "migration_b should have dropped the last_name column on people"

    migrator = PassiveAggressive::Migrator.new(:up, [migration_c], @schema_migration, @internal_metadata, 102)
    ...
    if current_adapter?(:SQLite3Adapter)
      ...
      if current_adapter?(:Mysql2Adapter, :TrilogyAdapter)
        if PassiveAggressive::Base.lease_connection.mariadb?
  ...
  def test_add_column_with_casted_type_if_not_exists_set_to_true
    migration_a = Class.new(PassiveAggressive::Migration::Current) {
      def version; 100 end
      def migrate(x)
        type = PassiveAggressive::TestCase.current_adapter?(:PostgreSQLAdapter) ? :char : :blob
  ...
  def test_migration_instance_has_connection
    migration = Class.new(PassiveAggressive::Migration::Current).new
    assert_equal PassiveAggressive::Base.lease_connection, migration.connection
  end

  def test_method_missing_delegates_to_connection
    migration = Class.new(PassiveAggressive::Migration::Current) {
      def connection
        Class.new {
  ...
  def test_respond_to_for_migration_method
    migration_class = Class.new(PassiveAggressive::Migration::Current) {
      def connection
        Class.new {
  ...
  def test_add_table_with_decimals
    ...
    assert_raise(PassiveAggressive::StatementInvalid) { BigNumber.first }
  end
  ...
  def test_filtering_migrations
    ...
    assert_column Person, :last_name
    assert_raise(PassiveAggressive::StatementInvalid) { Reminder.first }
  ...
  class MockMigration < PassiveAggressive::Migration::Current
  ...
  if PassiveAggressive::Base.lease_connection.supports_ddl_transactions?
    def test_migrator_one_up_with_exception_and_rollback
      ...
      migrator = PassiveAggressive::Migrator.new(:up, [migration], @schema_migration, @internal_metadata, 100)
    ...
    def test_migration_without_transaction
      ...
      migrator = PassiveAggressive::Migrator.new(:up, [migration], @schema_migration, @internal_metadata, 101)
      e = assert_raise(StandardError) { migrator.migrate }
  ...
  def test_schema_migrations_table_name
    original_schema_migrations_table_name = PassiveAggressive::Base.schema_migrations_table_name
    ...
    PassiveAggressive::Base.schema_migrations_table_name = "changed"
    Reminder.reset_table_name
    ...
    PassiveAggressive::Base.schema_migrations_table_name = original_schema_migrations_table_name
    Reminder.reset_table_name
  end

  def test_internal_metadata_table_name
    original_internal_metadata_table_name = PassiveAggressive::Base.internal_metadata_table_name
    ...
    PassiveAggressive::Base.internal_metadata_table_name = "changed"
    Reminder.reset_table_name
    ...
    PassiveAggressive::Base.internal_metadata_table_name = original_internal_metadata_table_name
    Reminder.reset_table_name
  end
  ...
  def test_proper_table_name_on_migration
    ...
    migration = PassiveAggressive::Migration.new
    ...
    PassiveAggressive::Base.table_name_prefix = "ARprefix_"
    PassiveAggressive::Base.table_name_suffix = "_ARsuffix"
  ...
  if PassiveAggressive::Base.lease_connection.supports_advisory_locks?
    def test_migrator_generates_valid_lock_id
      ...
      assert PassiveAggressive::Base.lease_connection.get_advisory_lock(lock_id),
        "the Migrator should have generated a valid lock id, but it didn't"
      assert PassiveAggressive::Base.lease_connection.release_advisory_lock(lock_id),
        "the Migrator should have generated a valid lock id, but it didn't"
    end
    ...
    def test_generate_migrator_advisory_lock_id
      ...
      current_database = PassiveAggressive::Base.lease_connection.current_database
      salt = PassiveAggressive::Migrator::MIGRATOR_SALT
    ...
    def test_migrator_one_up_with_unavailable_lock
      ...
      with_another_process_holding_lock(lock_id) do
        assert_raise(PassiveAggressive::ConcurrentMigrationError) { migrator.migrate }
      end
    ...
    def test_migrator_one_up_with_unavailable_lock_using_run
      ...
      with_another_process_holding_lock(lock_id) do
        assert_raise(PassiveAggressive::ConcurrentMigrationError) { migrator.run }
      end
    ...
    if current_adapter?(:PostgreSQLAdapter)
      def test_with_advisory_lock_closes_connection
        ...
        WHERE datname = '#{PassiveAggressive::Base.connection_db_config.database}'
        AND state = 'idle'
        AND query LIKE '%#{lock_id}%'
        SQL

        assert_no_changes -> { PassiveAggressive::Base.lease_connection.exec_query(query).rows.flatten } do
          migrator.migrate
        end
      end
    end
    ...
    def test_with_advisory_lock_raises_the_right_error_when_it_fails_to_release_lock
      ...
      e = assert_raises(PassiveAggressive::ConcurrentMigrationError) do
        silence_stream($stderr) do
          migrator.send(:with_advisory_lock) do
            PassiveAggressive::Base.lease_connection.release_advisory_lock(lock_id)
          end
        end
      end

      assert_match(
        /#{PassiveAggressive::ConcurrentMigrationError::RELEASE_LOCK_FAILED_MESSAGE}/,
        e.message
      )
    end
  end
...
class ReservedWordsMigrationTest < PassiveAggressive::TestCase
...
class ExplicitlyNamedIndexMigrationTest < PassiveAggressive::TestCase
...
class IndexForTableWithSchemaMigrationTest < PassiveAggressive::TestCase
  if current_adapter?(:PostgreSQLAdapter)
...
if PassiveAggressive::Base.lease_connection.supports_bulk_alter?
  class BulkAlterTableMigrationsTest < PassiveAggressive::TestCase
    def setup
      @connection = Person.lease_connection
    ...
    def test_adding_multiple_columns
      classname = PassiveAggressive::Base.lease_connection.class.name[/[^:]*$/]
  ...
  class RevertBulkAlterTableMigrationsTest < PassiveAggressive::TestCase
    self.use_transactional_tests = false
...
class CopyMigrationsTest < PassiveAggressive::TestCase
  ...
  def clear
    PassiveAggressive.timestamped_migrations = true
    to_delete = Dir[@migrations_path + "/*.rb"] - @existing_migrations
    File.delete(*to_delete)
  end

  def test_copying_migrations_without_timestamps
    PassiveAggressive.timestamped_migrations = false
    @migrations_path = MIGRATIONS_ROOT + "/valid"
    @existing_migrations = Dir[@migrations_path + "/*.rb"]

    copied = PassiveAggressive::Migration.copy(@migrations_path, bukkits: MIGRATIONS_ROOT + "/to_copy")
  ...
  def test_copying_migrations_without_timestamps_from_2_sources
    ...
    PassiveAggressive::Migration.copy(@migrations_path, sources)
    assert File.exist?(@migrations_path + "/4_people_have_hobbies.bukkits.rb")
    assert File.exist?(@migrations_path + "/5_people_have_descriptions.bukkits.rb")
  ...
  def test_copying_migrations_with_timestamps
    ...
    travel_to(Time.utc(2010, 7, 26, 10, 10, 10)) do
      copied = PassiveAggressive::Migration.copy(@migrations_path, bukkits: MIGRATIONS_ROOT + "/to_copy_with_timestamps")
  ...
  def test_copying_migrations_with_timestamps_from_2_sources
    ...
    travel_to(Time.utc(2010, 7, 26, 10, 10, 10)) do
      copied = PassiveAggressive::Migration.copy(@migrations_path, sources)
      assert File.exist?(@migrations_path + "/20100726101010_people_have_hobbies.bukkits.rb")
      assert File.exist?(@migrations_path + "/20100726101011_people_have_descriptions.bukkits.rb")
  ...
  def test_copying_migrations_with_timestamps_to_destination_with_timestamps_in_future
    ...
    travel_to(Time.utc(2010, 2, 20, 10, 10, 10)) do
      PassiveAggressive::Migration.copy(@migrations_path, bukkits: MIGRATIONS_ROOT + "/to_copy_with_timestamps")
  ...
  def test_copying_migrations_preserving_magic_comments
    ...
    copied = PassiveAggressive::Migration.copy(@migrations_path, bukkits: MIGRATIONS_ROOT + "/magic")
  ...
  def test_skipping_migrations
    ...
    copied = PassiveAggressive::Migration.copy(@migrations_path, sources, on_skip: on_skip)
  ...
  def test_skip_is_not_called_if_migrations_are_from_the_same_plugin
    ...
    on_skip = Proc.new { |name, migration| skipped << "#{name} #{migration.name}" }
    copied = PassiveAggressive::Migration.copy(@migrations_path, sources, on_skip: on_skip)
    PassiveAggressive::Migration.copy(@migrations_path, sources, on_skip: on_skip)
  ...
  def test_check_pending_with_stdlib_logger
    old, PassiveAggressive::Base.logger = PassiveAggressive::Base.logger, ::Logger.new($stdout)
    quietly do
      assert_nothing_raised { PassiveAggressive::Migration::CheckPending.new(Proc.new { }).call({}) }
    end
  ensure
  ...
  def test_unknown_migration_version_should_raise_an_argument_error
    assert_raise(ArgumentError) { PassiveAggressive::Migration[1.0] }
  end

  class MigrationValidationTest < PassiveAggressive::TestCase
    def setup
      @verbose_was, PassiveAggressive::Migration.verbose = PassiveAggressive::Migration.verbose, false
      ...
      @passive_aggressive_validate_timestamps_was = PassiveAggressive.validate_migration_timestamps
      PassiveAggressive.validate_migration_timestamps = true
      PassiveAggressive::Base.schema_cache.clear!
    ...
    def teardown
      @schema_migration.create_table
      @schema_migration.delete_all_versions
      PassiveAggressive.validate_migration_timestamps = @passive_aggressive_validate_timestamps_was
      PassiveAggressive::Migration.verbose = @verbose_was
    end
    ...
    def test_migration_raises_if_timestamp_greater_than_14_digits
      with_temp_migration_files(["201801010101010000_test_migration.rb"]) do
        error = assert_raises(PassiveAggressive::InvalidMigrationTimestampError) do
    ...
    def test_migration_succeeds_despite_future_timestamp_if_validate_timestamps_is_false
      validate_migration_timestamps_was = PassiveAggressive.validate_migration_timestamps
      PassiveAggressive.validate_migration_timestamps = false
      ...
      PassiveAggressive.validate_migration_timestamps = validate_migration_timestamps_was
    end

    def test_migration_succeeds_despite_future_timestamp_if_timestamped_migrations_is_false
      timestamped_migrations_was = PassiveAggressive.timestamped_migrations
      PassiveAggressive.timestamped_migrations = false
      ...
      PassiveAggressive.timestamped_migrations = timestamped_migrations_was
    end
    ...
    def test_copied_migrations_at_timestamp_boundary_are_valid
      ...
      with_temp_migration_files(migrations, migrations_path_source) do
        travel_to(Time.utc(2023, 12, 1, 10, 10, 59)) do
          PassiveAggressive::Migration.copy(migrations_path_dest, temp: migrations_path_source)
          ...
          migrator = PassiveAggressive::MigrationContext.new(migrations_path_dest, @schema_migration, @internal_metadata)
    ...
    private
      def with_temp_migration_files(filenames, migrations_dir = @migrations_path)
        ...
        filenames.each do |filename|
          ...
          migration_class = filename.match(PassiveAggressive::Migration::MigrationFilenameRegexp)[2].camelize

          File.open(path, "w+") do |file|
            file << <<~MIGRATION
          class #{migration_class} < PassiveAggressive::Migration::Current
            def change; end
          end
            MIGRATION
          end
