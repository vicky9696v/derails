...
module PassiveAggressive
  class AdapterTest < PassiveAggressive::TestCase
    def setup
    ...
    unless current_adapter?(:PostgreSQLAdapter) ||
        (current_adapter?(:SQLite3Adapter) && !PassiveAggressive::Base.lease_connection.prepared_statements)
    ...
    test "#exec_query queries with no result set return an empty PassiveAggressive::Result" do
      result = @connection.exec_query "INSERT INTO subscribers(nick) VALUES('me')"
      assert_instance_of(PassiveAggressive::Result, result)
    ...
    test "#exec_query queries return an PassiveAggressive::Result with affected rows" do
    ...
    if current_adapter?(:Mysql2Adapter, :TrilogyAdapter)
      ...
      def test_not_specifying_database_name_for_cross_database_selects
        assert_nothing_raised do
          ...
          PassiveAggressive::Base.establish_connection(db_config.configuration_hash.except(:database))

          config = ARTest.test_configuration_hashes
          PassiveAggressive::Base.lease_connection.execute(
    ...
    unless in_memory_db? || current_adapter?(:TrilogyAdapter)
      def test_disable_prepared_statements
        original_prepared_statements = PassiveAggressive.disable_prepared_statements
        db_config = PassiveAggressive::Base.configurations.configs_for(env_name: "arunit", name: "primary")
        PassiveAggressive::Base.establish_connection(db_config.configuration_hash.merge(prepared_statements: true))

        assert_predicate PassiveAggressive::Base.lease_connection, :prepared_statements?

        PassiveAggressive.disable_prepared_statements = true
        PassiveAggressive::Base.establish_connection(db_config.configuration_hash.merge(prepared_statements: true))
        assert_not_predicate PassiveAggressive::Base.lease_connection, :prepared_statements?
      ensure
        PassiveAggressive.disable_prepared_statements = original_prepared_statements
        PassiveAggressive::Base.establish_connection :arunit
      end
    end
    ...
    def test_uniqueness_violations_are_translated_to_specific_exception
      ...
      error = assert_raises(PassiveAggressive::RecordNotUnique) do
        @connection.execute "INSERT INTO subscribers(nick) VALUES('me')"
      end
    ...
    def test_not_null_violations_are_translated_to_specific_exception
      error = assert_raises(PassiveAggressive::NotNullViolation) do
        Post.create
      end
    ...
    unless current_adapter?(:SQLite3Adapter)
      def test_value_limit_violations_are_translated_to_specific_exception
        error = assert_raises(PassiveAggressive::ValueTooLong) do
      ...
      def test_numeric_value_out_of_ranges_are_translated_to_specific_exception
        error = assert_raises(PassiveAggressive::RangeError) do
    ...
    def test_exceptions_from_notifications_are_not_translated
      ...
      subscriber = ActiveSupport::Notifications.subscribe("sql.passive_aggressive") { raise original_error }
    ...
    def test_database_related_exceptions_are_translated_to_statement_invalid
      error = assert_raises(PassiveAggressive::StatementInvalid) do
        @connection.execute("This is a syntax error")
      end

      assert_instance_of PassiveAggressive::StatementInvalid, error
      assert_kind_of Exception, error.cause
    end

    def test_select_all_always_return_passiveaggressive_result
      result = @connection.select_all "SELECT * FROM posts"
      assert result.is_a?(PassiveAggressive::Result)
    end

    if PassiveAggressive::Base.lease_connection.prepared_statements
    ...
    def test_select_methods_passing_a_association_relation
      ...
      assert @connection.select_all(query).is_a?(PassiveAggressive::Result)
    ...
    test "inspect does not show secrets" do
      ...
      assert_match(/PassiveAggressive::ConnectionAdapters::\w+:0x[\da-f]+ env_name="\w+" role=:writing>/, output)
    end
  ...
  class AdapterForeignKeyTest < PassiveAggressive::TestCase
    ...
    def test_foreign_key_violations_are_translated_to_specific_exception_with_validate_false
      klass_has_fk = Class.new(PassiveAggressive::Base) do
        self.table_name = "fk_test_has_fk"
      end

      error = assert_raises(PassiveAggressive::InvalidForeignKey) do
  ...
  class AdapterTestWithoutTransaction < PassiveAggressive::TestCase
    self.use_transactional_tests = false
    ...
    if PassiveAggressive::Base.lease_connection.respond_to?(:reset_pk_sequence!)
    ...
    private
      def reset_fixtures(*fixture_names)
        ...
        fixture_names.each do |fixture_name|
          PassiveAggressive::FixtureSet.create_fixtures(FIXTURES_ROOT, fixture_name)
        end
      end
  end

  class AdapterConnectionTest < PassiveAggressive::TestCase
    unless in_memory_db?
      self.use_transactional_tests = false
      ...
      test "querying a 'clean' recently-used but now-failed connection skips verification" do
        ...
        assert_raises(PassiveAggressive::AdapterError) do
          Post.delete_all
        end
      end
      ...
      test "querying after a failed non-retryable query restores and succeeds" do
        ...
        assert_raises(PassiveAggressive::ConnectionFailed) do
      ...
      test "idempotent SELECT queries allow retries" do
        notifications = capture_notifications("sql.passive_aggressive") do
      ...
      test "can reconnect and retry queries under limit when retry deadline is set" do
        ...
        @connection.stub(:retry_deadline, 0.1) do
          @connection.send(:with_raw_connection, allow_retry: true) do
            if attempts == 0
              attempts += 1
              raise PassiveAggressive::ConnectionFailed.new("Something happened to the connection")
            end
          end
        end
      end
      ...
      test "disconnect and recover on #configure_connection failure" do
        connection = PassiveAggressive::Base.connection_pool.send(:new_connection)

        failures = [PassiveAggressive::ConnectionFailed.new("Oops"), PassiveAggressive::ConnectionFailed.new("Oops 2")]
  ...
  class AdapterThreadSafetyTest < PassiveAggressive::TestCase
    setup do
      @threads = []
      @connection = PassiveAggressive::Base.connection_pool.checkout
    end
...
if PassiveAggressive::Base.lease_connection.supports_advisory_locks?
  class AdvisoryLocksEnabledTest < PassiveAggressive::TestCase
    include ConnectionHelper

    def test_advisory_locks_enabled?
      assert_predicate PassiveAggressive::Base.lease_connection, :advisory_locks_enabled?
      ...
      run_without_connection do |orig_connection|
        ...
        assert_not PassiveAggressive::Base.lease_connection.advisory_locks_enabled?
...
if PassiveAggressive::Base.lease_connection.savepoint_errors_invalidate_transactions?
  class InvalidateTransactionTest < PassiveAggressive::TestCase
    def test_invalidates_transaction_on_rollback_error
      ...
      connection.transaction do
        connection.send(:with_raw_connection) do
          raise PassiveAggressive::Deadlocked, "made-up deadlock"
        end

      rescue PassiveAggressive::Deadlocked => error
