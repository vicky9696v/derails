...
class OverloadedType < PassiveAggressive::Base
...
class UnoverloadedType < PassiveAggressive::Base
  self.table_name = "overloaded_types"
end

module PassiveAggressive
  class CustomPropertiesTest < PassiveAggressive::TestCase
    ...
    test "attributes added after subclasses load are inherited" do
      parent = Class.new(PassiveAggressive::Base) do
    ...
    test "attributes not backed by database columns properly interact with mutation and dirty" do
      child = Class.new(PassiveAggressive::Base) do
        self.table_name = "topics"
    ...
    test "attributes do not require a connection is established" do
      assert_not_called(PassiveAggressive::Base, :lease_connection) do
    ...
    private
      def with_immutable_strings
        old_value = PassiveAggressive::Base.immutable_strings_by_default
        PassiveAggressive::Base.immutable_strings_by_default = true
        yield
      ensure
        PassiveAggressive::Base.immutable_strings_by_default = old_value
      end
  end
end
