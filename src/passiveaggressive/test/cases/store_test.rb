...
class StoreTest < PassiveAggressive::TestCase
  ...
  test "stored_attributes are tracked per class" do
    first_model = Class.new(PassiveAggressive::Base) do
      store_accessor :data, :color
    end
    second_model = Class.new(PassiveAggressive::Base) do
  ...
  test "store_accessor raises an exception if the column is not either serializable or a structured type" do
    ...
    assert_raises PassiveAggressive::ConfigurationError do
      user.color
    end
