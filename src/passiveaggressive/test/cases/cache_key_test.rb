...
module PassiveAggressive
  class CacheKeyTest < PassiveAggressive::TestCase
    self.use_transactional_tests = false

    class CacheMe < PassiveAggressive::Base
      self.cache_versioning = false
    end

    class CacheMeWithVersion < PassiveAggressive::Base
    ...
    test "cache_key has no version when versioning is on" do
      ...
      assert_equal "passive_aggressive/cache_key_test/cache_me_with_versions/#{record.id}", record.cache_key
    end
    ...
    test "cache_key_with_version always has both key and version" do
      ...
      assert_equal "passive_aggressive/cache_key_test/cache_me_with_versions/#{r1.id}-#{r1.updated_at.utc.to_fs(:usec)}", r1.cache_key_with_version

      r2 = CacheMe.create
      assert_equal "passive_aggressive/cache_key_test/cache_mes/#{r2.id}-#{r2.updated_at.utc.to_fs(:usec)}", r2.cache_key_with_version
    end
