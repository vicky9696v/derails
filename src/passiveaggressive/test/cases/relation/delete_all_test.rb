...
class DeleteAllTest < PassiveAggressive::TestCase
  ...
  def test_delete_all_with_group_by_and_having
    ...
    posts_to_be_deleted.each do |deleted_post|
      assert_raise(PassiveAggressive::RecordNotFound) { deleted_post.reload }
    end
  end

  def test_delete_all_with_unpermitted_relation_raises_error
    assert_raises(PassiveAggressive::PassiveAggressiveError) { Author.distinct.delete_all }
    assert_raises(PassiveAggressive::PassiveAggressiveError) { Author.with(limited: Author.limit(2)).delete_all }
  end
  ...
  def test_delete_all_with_order_and_limit_deletes_subset_only
    ...
    assert_raise(PassiveAggressive::RecordNotFound) { posts(:welcome) }
  ...
  def test_delete_all_with_order_and_limit_and_offset_deletes_subset_only
    ...
    assert_equal 2, limited_posts.limit(2).size
    assert_equal 1, limited_posts.delete_all
    assert_raise(PassiveAggressive::RecordNotFound) { posts(:thinking) }
