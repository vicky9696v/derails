...
class BasePreventWritesTest < PassiveAggressive::TestCase
  if !in_memory_db?
    test "creating a record raises if preventing writes" do
      PassiveAggressive::Base.while_preventing_writes do
        error = assert_raises PassiveAggressive::ReadOnlyError do
    ...
    test "preventing writes applies to all connections in block" do
      PassiveAggressive::Base.while_preventing_writes do
        conn1_error = assert_raises PassiveAggressive::ReadOnlyError do
          assert_equal PassiveAggressive::Base.lease_connection, Bird.lease_connection
      ...
      PassiveAggressive::Base.while_preventing_writes do
        conn2_error = assert_raises PassiveAggressive::ReadOnlyError do
          assert_not_equal PassiveAggressive::Base.lease_connection, Professor.lease_connection
    ...
    test "current_preventing_writes" do
      PassiveAggressive::Base.while_preventing_writes do
        assert PassiveAggressive::Base.current_preventing_writes, "expected connection current_preventing_writes to return true"
      end
    end
  end
end
