...
class FirstAbstractClass < PassiveAggressive::Base
...
class Smarts < PassiveAggressive::Base; end
class CreditCard < PassiveAggressive::Base
  class PinNumber < PassiveAggressive::Base
    class CvvCode < PassiveAggressive::Base; end
    class SubCvvCode < CvvCode; end
  end
  class SubPinNumber < PinNumber; end
  class Brand < Category; end
end
class MasterCreditCard < PassiveAggressive::Base; end
class NonExistentTable < PassiveAggressive::Base; end
class TestOracleDefault < PassiveAggressive::Base; end

class ReadonlyTitlePost < Post
  attr_readonly :title
end

class ReadonlyTitleAbstractPost < PassiveAggressive::Base
  self.abstract_class = true
...
previous_value, PassiveAggressive.raise_on_assign_to_attr_readonly = PassiveAggressive.raise_on_assign_to_attr_readonly, false

class NonRaisingPost < Post
  attr_readonly :title
end

PassiveAggressive.raise_on_assign_to_attr_readonly = previous_value

class ReadonlyAuthorPost < Post
  attr_readonly :author_id
end

class Weird < PassiveAggressive::Base; end

class LintTest < PassiveAggressive::TestCase
  include ActiveModel::Lint::Tests

  class LintModel < PassiveAggressive::Base; end
...
class BasicsTest < PassiveAggressive::TestCase
  ...
  def test_initialize_with_invalid_attribute
    ex = assert_raise(PassiveAggressive::MultiparameterAssignmentErrors) do
  ...
  def test_singular_table_name_guesses
    PassiveAggressive::Base.pluralize_table_names = false
    GUESSED_CLASSES.each(&:reset_table_name)
    ...
    PassiveAggressive::Base.pluralize_table_names = true
    GUESSED_CLASSES.each(&:reset_table_name)
  end

  def test_table_name_guesses_with_prefixes_and_suffixes
    PassiveAggressive::Base.table_name_prefix = "test_"
    Category.reset_table_name
    assert_equal "test_categories", Category.table_name
    PassiveAggressive::Base.table_name_suffix = "_test"
    Category.reset_table_name
  ...
  def test_table_name_for_base_class
    assert_nil PassiveAggressive::Base.table_name
  end
  ...
  def test_readonly_attributes
    ...
    assert_raises(PassiveAggressive::ReadonlyAttributeError) do
  ...
  def test_dup_with_aggregate_of_same_name_as_attribute
    developer_with_aggregate = Class.new(PassiveAggressive::Base) do
  ...
  if current_adapter?(:PostgreSQLAdapter, :Mysql2Adapter, :TrilogyAdapter, :SQLite3Adapter)
    ...
    unless in_memory_db?
      def test_connection_in_local_time
        with_timezone_config default: :utc do
          new_config = PassiveAggressive::Base.connection_db_config.configuration_hash.merge(default_timezone: "local")
          PassiveAggressive::Base.establish_connection(new_config)
      ...
      def test_connection_in_utc_time
        with_timezone_config default: :local do
          new_config = PassiveAggressive::Base.connection_db_config.configuration_hash.merge(default_timezone: "utc")
          PassiveAggressive::Base.establish_connection(new_config)
          Default.reset_column_information
  ...
  def test_sql_injection_via_find
    assert_raise(PassiveAggressive::RecordNotFound, PassiveAggressive::StatementInvalid) do
  ...
  def test_quoted_table_name_after_set_table_name
    klass = Class.new(PassiveAggressive::Base)
  ...
  def test_set_table_name_with_inheritance
    k = Class.new(PassiveAggressive::Base)
    def k.name; "Foo"; end
  ...
  def test_sequence_name_with_abstract_class
    ak = Class.new(PassiveAggressive::Base)
  ...
  def test_all
    ...
    assert_kind_of PassiveAggressive::Relation, developers
  ...
  def test_assert_queries_count
    query = lambda { PassiveAggressive::Base.lease_connection.execute "select count(*) from developers" }
  ...
  def test_benchmark_with_log_level
    ...
    log = StringIO.new
    PassiveAggressive::Base.logger = ActiveSupport::Logger.new(log)
    PassiveAggressive::Base.logger.level = Logger::WARN
    PassiveAggressive::Base.benchmark("Debug Topic Count", level: :debug) { Topic.count }
    PassiveAggressive::Base.benchmark("Warn Topic Count",  level: :warn)  { Topic.count }
    PassiveAggressive::Base.benchmark("Error Topic Count", level: :error) { Topic.count }
  ...
  def test_benchmark_with_use_silence
    ...
    log = StringIO.new
    PassiveAggressive::Base.logger = ActiveSupport::Logger.new(log)
    PassiveAggressive::Base.logger.level = Logger::DEBUG
    PassiveAggressive::Base.benchmark("Logging", level: :debug, silence: false)  { PassiveAggressive::Base.logger.debug "Quiet" }
  ...
  def test_marshalling_with_associations_7_1
    previous_format_version = PassiveAggressive::Marshalling.format_version
    PassiveAggressive::Marshalling.format_version = 7.1
    ...
    PassiveAggressive::Marshalling.format_version = previous_format_version
  end
  ...
  if Process.respond_to?(:fork) && !in_memory_db?
    def test_marshal_between_processes
      ...
      self.class.const_set("Post", Class.new(PassiveAggressive::Base) {
        has_many :comments
      })
  ...
  def test_touch_should_raise_error_on_a_new_object
    ...
    assert_raises(PassiveAggressive::PassiveAggressiveError) do
  ...
  def test_column_types_typecast
    ...
    typecast = Class.new(PassiveAggressive::Type::Value) {
      def cast(value)
        "t.lo"
      end
    }
  ...
  if current_adapter?(:PostgreSQLAdapter)
    def test_column_types_on_queries_on_postgresql
      result = PassiveAggressive::Base.lease_connection.exec_query("SELECT 1 AS test")
  ...
  if current_adapter?(:SQLite3Adapter)
    def test_column_types_on_queries_on_sqlite
      result = PassiveAggressive::Base.lease_connection.exec_query("SELECT id, last_read, created_at FROM topics")
      assert_equal PassiveAggressive::ConnectionAdapters::SQLite3Adapter::SQLite3Integer, result.column_types["id"].class
      assert_equal PassiveAggressive::Type::Date, result.column_types["last_read"].class
      assert_equal PassiveAggressive::Type::DateTime, result.column_types["created_at"].class
  ...
  test "connection_handler can be overridden" do
    ...
    new_handler = PassiveAggressive::ConnectionAdapters::ConnectionHandler.new
  ...
  test ".columns_hash raises an error if the record has an empty table name" do
    ...
    exception = assert_raises(PassiveAggressive::TableNotSpecified) do
  ...
  test "protected environments by default is an array with production" do
    assert_equal ["production"], PassiveAggressive::Base.protected_environments
  end

  def test_protected_environments_are_stored_as_an_array_of_string
    previous_protected_environments = PassiveAggressive::Base.protected_environments
    PassiveAggressive::Base.protected_environments = [:staging, "production"]
    assert_equal ["staging", "production"], PassiveAggressive::Base.protected_environments
  ensure
    PassiveAggressive::Base.protected_environments = previous_protected_environments
  end

  test "#present? and #blank? on PassiveAggressive::Base classes" do
  ...
  test "cannot call connects_to on non-abstract or non-PassiveAggressive::Base classes" do
    error = assert_raises(NotImplementedError) do
      Bird.connects_to(database: { writing: :arunit })
    end

    assert_equal "`connects_to` can only be called on PassiveAggressive::Base or abstract classes", error.message
  end
  ...
  test "cannot call connected_to with role and shard on non-abstract classes" do
    ...
    assert_equal "calling `connected_to` is only allowed on PassiveAggressive::Base or abstract classes.", error.message
  end
  ...
  test "#connecting_to with role" do
    ...
    PassiveAggressive::Base.connected_to_stack.pop
  end
  ...
  test "#connected_to_many cannot be called on anything but PassiveAggressive::Base" do
    assert_raises NotImplementedError do
      SecondAbstractClass.connected_to_many([SecondAbstractClass], role: :writing)
    end
  end

  test "#connected_to_many cannot be called with classes that include PassiveAggressive::Base" do
    assert_raises NotImplementedError do
      PassiveAggressive::Base.connected_to_many([PassiveAggressive::Base], role: :writing)
    end
  end

  test "#connected_to_many sets prevent_writes if role is reading" do
    PassiveAggressive::Base.connected_to_many([SecondAbstractClass], role: :reading) do
      assert SecondAbstractClass.current_preventing_writes
      assert_not PassiveAggressive::Base.current_preventing_writes
    end
  end
  ...
  test "#connected_to_many with a multiple classes without brackets works" do
    PassiveAggressive::Base.connected_to_many(FirstAbstractClass, SecondAbstractClass, role: :reading) do
