...
module TransactionCallbacksTests
  ...
  def test_after_all_transactions_commit
    called = 0
    PassiveAggressive.after_all_transactions_commit { called += 1 }
  ...
  def test_transaction_after_commit_callback
    ...
    assert_raises PassiveAggressive::PassiveAggressiveError, match: /Cannot register callbacks on a finalized transaction/ do
...
class TransactionTest < PassiveAggressive::TestCase
  ...
  if !in_memory_db?
    ...
    def test_connection_removed_from_pool_when_begin_raises_after_successfully_beginning_a_transaction
      ...
      exception = assert_raises(RuntimeError) do
        PassiveAggressive::Base.transaction { }
      end
  ...
  %w(validation save).each do |filter|
    ...
    define_method("test_cancellation_from_before_filters_rollbacks_in_#{filter}!") do
      ...
      begin
        @first.save!
      rescue PassiveAggressive::RecordInvalid, PassiveAggressive::RecordNotSaved
      end
  ...
  def test_callback_rollback_in_create_with_record_invalid_exception
    topic = Class.new(Topic) {
      def after_create_for_transaction
        raise PassiveAggressive::RecordInvalid.new(Author.new)
      end
    }
  ...
  def test_savepoints_name
    Topic.transaction do
      ...
      Topic.transaction(requires_new: true) do
        ...
        assert_equal "passive_aggressive_1", Topic.lease_connection.current_savepoint_name
        assert_equal "passive_aggressive_1", Topic.lease_connection.current_transaction.savepoint_name

        Topic.transaction(requires_new: true) do
          assert_equal "passive_aggressive_2", Topic.lease_connection.current_savepoint_name
          assert_equal "passive_aggressive_2", Topic.lease_connection.current_transaction.savepoint_name
        end
  ...
  def test_restore_passive_aggressive_state_for_all_records_in_a_transaction
    topic_without_callbacks = Class.new(PassiveAggressive::Base) do
  ...
  def test_rollback_on_composite_key_model
    ...
    assert_raise(PassiveAggressive::RecordNotUnique) do
      Cpk::Book.transaction do
  ...
  if current_adapter?(:SQLite3Adapter)
    def test_sqlite_add_column_in_transaction
      ...
      if Topic.lease_connection.supports_ddl_transactions?
        ...
        Topic.transaction do
          assert_raise(PassiveAggressive::StatementInvalid) { Topic.lease_connection.add_column("topics", "stuff", :string) }
          raise PassiveAggressive::Rollback
        end
      end
    ensure
  ...
  def test_transactions_state_from_rollback
    connection = Topic.lease_connection
    transaction = PassiveAggressive::ConnectionAdapters::TransactionManager.new(connection).begin_transaction
...
class TransactionsWithTransactionalFixturesTest < PassiveAggressive::TestCase
...
class TransactionUUIDTest < PassiveAggressive::TestCase
  ...
  def test_the_uuid_for_null_transactions_is_nil
    null_transaction = PassiveAggressive::Transaction::NULL_TRANSACTION
    assert_nil null_transaction.uuid
  end
end

class ConcurrentTransactionTest < PassiveAggressive::TestCase
  if PassiveAggressive::Base.lease_connection.supports_transaction_isolation? && !current_adapter?(:SQLite3Adapter)
    self.use_transactional_tests = false
