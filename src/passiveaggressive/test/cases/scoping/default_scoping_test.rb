...
class DefaultScopingTest < PassiveAggressive::TestCase
  ...
  def test_sti_association_with_unscoped_not_affected_by_default_scope
    ...
    assert_raises(PassiveAggressive::RecordNotFound) { Post.joins(:special_comments).find(post.id) }
...
class DefaultScopingWithThreadTest < PassiveAggressive::TestCase
  unless in_memory_db?
