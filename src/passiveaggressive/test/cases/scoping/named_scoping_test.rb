...
class NamedScopingTest < PassiveAggressive::TestCase
  ...
  def test_method_missing_priority_when_delegating
    klazz = Class.new(PassiveAggressive::Base) do
  ...
  def test_passive_aggressives_have_scope_named__all__
    assert_not_empty Topic.all

    assert_equal Topic.all.to_a, Topic.base
  end

  def test_passive_aggressives_have_scope_named__scoped__
