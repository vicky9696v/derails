...
class CalculationsTest < PassiveAggressive::TestCase
  include AsyncHelper
  ...
  def test_count_with_no_parameters_isnt_deprecated
    assert_not_deprecated(PassiveAggressive.deprecator) { Account.count }
  end
  ...
  def test_count_takes_attribute_type_precedence_over_database_type
    assert_called(
      ...
      returns: PassiveAggressive::Result.new(["count"], [["10"]])
    ) do
  ...
  def test_sum_takes_attribute_type_precedence_over_database_type
    assert_called(
      Account.lease_connection, :select_all,
      returns: PassiveAggressive::Result.new(["sum"], [[10.to_d]])
    ) do
