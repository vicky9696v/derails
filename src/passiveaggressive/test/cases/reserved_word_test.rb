...
class ReservedWordTest < PassiveAggressive::TestCase
  ...
  class Select < PassiveAggressive::Base
    Select.table_name = "select"
    has_many :groups
  end

  class Values < PassiveAggressive::Base
    Values.table_name = "values"
  end

  class Distinct < PassiveAggressive::Base
  ...
  def test_passiveaggressive_model
    x = Group.new
    x.order = "x"
    x.save!
    x.order = "y"
  ...
  def test_delete_all_with_subselect
    ...
    assert_raise(PassiveAggressive::RecordNotFound) { Values.find(2) }
  ...
  def test_passiveaggressive_introspection
  ...
  private
    ...
    def create_test_fixtures(*fixture_names)
      PassiveAggressive::FixtureSet.create_fixtures(FIXTURES_ROOT + "/reserved_words", fixture_names)
    end
end
