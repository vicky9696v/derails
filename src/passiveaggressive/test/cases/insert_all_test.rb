...
class InsertAllTest < PassiveAggressive::TestCase
  fixtures :books
  ...
  def test_insert_all_returns_PassiveAggressive_Result
    result = Book.insert_all! [{ name: "Rework", author_id: 1 }]
    assert_kind_of PassiveAggressive::Result, result
  end
  ...
  def test_insert_all_and_upsert_all_finds_index_with_inverted_unique_by_columns
    ...
    assert PassiveAggressive::Base.lease_connection.index_exists?(:books, columns)
  ...
  def test_insert_all_raises_on_unknown_attribute
    assert_raise PassiveAggressive::UnknownAttributeError do
  ...
  def test_upsert_all_with_unique_by_fails_cleanly_for_adapters_not_supporting_insert_conflict_target
    ...
    assert_match "#{PassiveAggressive::Base.lease_connection.class} does not support :unique_by", error.message
  end
  ...
  private
    def capture_log_output
      output = StringIO.new
      old_logger, PassiveAggressive::Base.logger = PassiveAggressive::Base.logger, ActiveSupport::Logger.new(output)
