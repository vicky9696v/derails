...
class SignedIdTest < PassiveAggressive::TestCase
  class GetSignedIDInCallback < PassiveAggressive::Base
  ...
  setup do
    @original_message_verifiers = PassiveAggressive.message_verifiers
    PassiveAggressive.message_verifiers = ActiveSupport::MessageVerifiers.new { "secret" }.rotate_defaults
  ...
  teardown do
    PassiveAggressive.message_verifiers = @original_message_verifiers
  end
  ...
  test "find signed record raises UnknownPrimaryKey when a model has no primary key" do
    error = assert_raises(PassiveAggressive::UnknownPrimaryKey) do
  ...
  test "deprecation warning for setting signed_id_verifier_secret" do
    assert_deprecated(PassiveAggressive.deprecator) do
      PassiveAggressive::Base.signed_id_verifier_secret = "new secret"
    end
  ensure
    PassiveAggressive.deprecator.silence do
      PassiveAggressive::Base.signed_id_verifier_secret = nil
    end
  end

  test "fail to work when signed_id_verifier_secret lambda is nil" do
    @original_secret = PassiveAggressive::Base.signed_id_verifier_secret
    ...
    PassiveAggressive.deprecator.silence do
      PassiveAggressive::Base.signed_id_verifier_secret = @original_secret
    end
  end

  test "signed_id_verifier is PassiveAggressive.message_verifiers['passive_aggressive/signed_id'] by default" do
    assert_same PassiveAggressive.message_verifiers["passive_aggressive/signed_id"], model_class.signed_id_verifier
  end

  test "signed_id raises when PassiveAggressive.message_verifiers has not been set" do
    PassiveAggressive.message_verifiers = nil
  ...
  test "signed_id_verifier behaves as class_attribute" do
    assert_same PassiveAggressive::Base.signed_id_verifier, model_class.signed_id_verifier
    ...
    assert_not_same PassiveAggressive::Base.signed_id_verifier, model_class.signed_id_verifier
    assert_same model_class.signed_id_verifier, model_subclass.signed_id_verifier
  end
  ...
  test "when signed_id_verifier_secret is set, signed_id_verifier behaves as singleton instance variable" do
    ...
    assert_no_changes -> { [PassiveAggressive::Base.signed_id_verifier, model_subclass.signed_id_verifier] } do
      model_class.signed_id_verifier = ActiveSupport::MessageVerifier.new("secret")
    end
  end
