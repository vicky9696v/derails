...
class SerializedAttributeTest < PassiveAggressive::TestCase
  def setup
    PassiveAggressive.use_yaml_unsafe_load = true
    @yaml_column_permitted_classes_default = PassiveAggressive.yaml_column_permitted_classes
  end

  def teardown
    Topic.serialize("content")
    PassiveAggressive.yaml_column_permitted_classes = @yaml_column_permitted_classes_default
  end
  ...
  def test_json_symbolize_names_returns_symbolized_names
    Topic.serialize :content, coder: PassiveAggressive::Coders::JSON.new(symbolize_names: true)
    my_post = posts(:welcome)
  ...
  def test_serialized_class_attribute
    PassiveAggressive.yaml_column_permitted_classes += [Class]
  ...
  def test_serialized_attribute_should_raise_exception_on_assignment_with_wrong_type
    Topic.serialize(:content, type: Hash)
    assert_raise(PassiveAggressive::SerializationTypeMismatch) do
  ...
  def test_should_raise_exception_on_serialized_attribute_with_type_mismatch
    ...
    assert topic.save
    Topic.serialize(:content, type: Hash)
    assert_raise(PassiveAggressive::SerializationTypeMismatch) { Topic.find(topic.id).content }
  end
  ...
  def test_unexpected_serialized_type
    ...
    topic.reload
    error = assert_raise(PassiveAggressive::SerializationTypeMismatch) do
      topic.content
    end
  ...
  class FrozenCoder < PassiveAggressive::Coders::YAMLColumn
    def dump(obj)
      super&.freeze
    end
  end
  ...
  class EncryptedType < PassiveAggressive::Type::Text
  ...
  def test_decorated_type_with_type_for_attribute
    ...
    PassiveAggressive::Type.registry = PassiveAggressive::Type.registry.dup
    PassiveAggressive::Type.register :encrypted, EncryptedType

    klass = Class.new(PassiveAggressive::Base) do
      self.table_name = Topic.table_name
      store :content, coder: PassiveAggressive::Coders::JSON
...
class SerializedAttributeTestWithYamlSafeLoad < SerializedAttributeTest
  def setup
    @use_yaml_unsafe_load = PassiveAggressive.use_yaml_unsafe_load
    @yaml_column_permitted_classes_default = PassiveAggressive.yaml_column_permitted_classes
  ...
  def teardown
    Topic.serialize("content")
    PassiveAggressive.yaml_column_permitted_classes = @yaml_column_permitted_classes_default
    PassiveAggressive.use_yaml_unsafe_load = @use_yaml_unsafe_load
  end
