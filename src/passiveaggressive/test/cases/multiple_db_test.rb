...
class MultipleDbTest < PassiveAggressive::TestCase
  ...
  def test_proper_connection
    ...
    assert_equal(PassiveAggressive::Base.lease_connection, Entrant.lease_connection)
  end

  def test_swapping_the_connection
    old_spec_name, Course.connection_specification_name = Course.connection_specification_name, "PassiveAggressive::Base"
  ...
  unless in_memory_db?
    def test_count_on_custom_connection
      ...
      assert_not_equal PassiveAggressive::Base.lease_connection, College.lease_connection
  ...
  def test_exception_contains_correct_pool
    ...
    course_error = assert_raises(PassiveAggressive::StatementInvalid) do
      course_conn.execute("SELECT * FROM entrants")
    end

    assert_equal course_conn.pool, course_error.connection_pool

    entrant_error = assert_raises(PassiveAggressive::StatementInvalid) do
