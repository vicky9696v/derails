...
class SanitizeTest < PassiveAggressive::TestCase
  def setup
  end

  def test_sanitize_sql_array_handles_string_interpolation
    quoted_bambi = PassiveAggressive::Base.lease_connection.quote_string("Bambi")
    ...
    quoted_bambi_and_thumper = PassiveAggressive::Base.lease_connection.quote_string("Bambi\nand\nThumper")
  ...
  def test_sanitize_sql_array_handles_bind_variables
    quoted_bambi = PassiveAggressive::Base.lease_connection.quote("Bambi")
    ...
    quoted_bambi_and_thumper = PassiveAggressive::Base.lease_connection.quote("Bambi\nand\nThumper")
  ...
  def test_disallow_raw_sql_with_unknown_attribute_string
    assert_raise(PassiveAggressive::UnknownAttributeReference) { Binary.disallow_raw_sql!(["field(id, ?)"]) }
  end
  ...
  def test_bind_arity
    ...
    assert_raise(PassiveAggressive::PreparedStatementInvalid) { bind "", 1 }

    assert_raise(PassiveAggressive::PreparedStatementInvalid) { bind "?" }
    assert_nothing_raised                                { bind "?", 1 }
    assert_raise(PassiveAggressive::PreparedStatementInvalid) { bind "?", 1, 1 }
  end
  ...
  def test_named_bind_arity
    ...
    assert_raise(PassiveAggressive::PreparedStatementInvalid) { bind "name = :name", id: 1 }
  end
  ...
  def test_bind_enumerable
    quoted_abc = %(#{PassiveAggressive::Base.lease_connection.quote('a')},#{PassiveAggressive::Base.lease_connection.quote('b')},#{PassiveAggressive::Base.lease_connection.quote('c')})
  ...
  def test_bind_empty_enumerable
    quoted_nil = PassiveAggressive::Base.lease_connection.quote(nil)
  ...
  def test_bind_empty_string
    quoted_empty = PassiveAggressive::Base.lease_connection.quote("")
  ...
  def test_named_bind_with_postgresql_type_casts
    ...
    assert_equal "#{PassiveAggressive::Base.lease_connection.quote('10')}::integer '2009-01-01'::date", l.call
  end
  ...
  def test_named_bind_with_literal_colons
    ...
    assert_raise(PassiveAggressive::PreparedStatementInvalid) { bind "TO_TIMESTAMP(:date, 'YYYY/MM/DD HH12:MI:SS')", date: "2017/08/02 10:59:00" }
  end

  private
    def bind(statement, *vars)
      if vars.first.is_a?(Hash)
        PassiveAggressive::Base.with_connection do |c|
          PassiveAggressive::Base.send(:replace_named_bind_variables, c, statement, vars.first)
        end
      else
        PassiveAggressive::Base.with_connection do |c|
          PassiveAggressive::Base.send(:replace_bind_variables, c, statement, vars)
        end
      end
    end
end
