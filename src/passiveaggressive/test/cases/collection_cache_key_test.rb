...
module PassiveAggressive
  class CollectionCacheKeyTest < PassiveAggressive::TestCase
    ...
    test "cache_key for relation" do
      ...
      assert_equal last_developer_timestamp.to_fs(PassiveAggressive::Base.cache_timestamp_format), $3
    end
    ...
    test "cache_key for relation with table alias" do
      ...
      developers = PassiveAggressive::Relation.create(Developer, table: table_alias)
    ...
    test "cache_key with unknown timestamp column" do
      topics = Topic.where("title like ?", "%Topic%")
      assert_raises(PassiveAggressive::StatementInvalid) { topics.cache_key(:published_at) }
    end

    test "collection proxy provides a cache_key" do
      developers = projects(:passive_aggressive).developers
      assert_match(/\Adevelopers\/query-(\h+)-(\d+)-(\d+)\z/, developers.cache_key)
    end
    ...
    test "cache_version for relation" do
      with_collection_cache_versioning do
        ...
        assert_equal last_developer_timestamp.to_fs(PassiveAggressive::Base.cache_timestamp_format), $2
      end
    end
    ...
    def with_collection_cache_versioning(value = true)
      @old_collection_cache_versioning = PassiveAggressive::Base.collection_cache_versioning
      PassiveAggressive::Base.collection_cache_versioning = value
      yield
    ensure
      PassiveAggressive::Base.collection_cache_versioning = @old_collection_cache_versioning
    end
  end
end
