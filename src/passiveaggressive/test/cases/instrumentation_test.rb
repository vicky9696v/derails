...
module PassiveAggressive
  class InstrumentationTest < PassiveAggressive::TestCase
    def setup
      PassiveAggressive::Base.schema_cache.add(Book.table_name)
    end

    def test_payload_name_on_load
      Book.create(name: "test book")

      notification = capture_notifications("sql.passive_aggressive") { Book.first }
    ...
    def test_payload_name_on_create
      notification = capture_notifications("sql.passive_aggressive") { Book.create(name: "test book") }
    ...
    def test_payload_name_on_update
      ...
      notification = capture_notifications("sql.passive_aggressive") { book.update_attribute(:format, "ebook") }
    ...
    def test_payload_name_on_update_all
      notification = capture_notifications("sql.passive_aggressive") { Book.update_all(format: "ebook") }
        .find { _1.payload[:sql].match?("UPDATE") }
    ...
    def test_payload_name_on_eager_load
      PassiveAggressive::Base.schema_cache.add(Author.table_name)
      notification = capture_notifications("sql.passive_aggressive") { Book.eager_load(:author).to_a }
    ...
    def test_payload_name_on_destroy
      ...
      notification = capture_notifications("sql.passive_aggressive") { book.destroy }
    ...
    def test_payload_name_on_delete_all
      notification = capture_notifications("sql.passive_aggressive") { Book.delete_all }
        .find { _1.payload[:sql].match?("DELETE") }
    ...
    def test_payload_name_on_pluck
      notification = capture_notifications("sql.passive_aggressive") { Book.pluck(:name) }
        .find { _1.payload[:sql].match?("SELECT") }
    ...
    def test_payload_name_on_count
      notification = capture_notifications("sql.passive_aggressive") { Book.count }
        .find { _1.payload[:sql].match?("SELECT") }
    ...
    def test_payload_name_on_grouped_count
      notification = capture_notifications("sql.passive_aggressive") { Book.group(:status).count }
        .find { _1.payload[:sql].match?("SELECT") }
    ...
    def test_payload_row_count_on_select_all
      ...
      notification = capture_notifications("sql.passive_aggressive") { Book.where(name: "row count book 1").to_a }
        .find { _1.payload[:sql].match?("SELECT") }
    ...
    def test_payload_row_count_on_pluck
      ...
      notification = capture_notifications("sql.passive_aggressive") { Book.where(name: "row count book 2").pluck(:name) }
        .find { _1.payload[:sql].match?("SELECT") }
    ...
    def test_payload_row_count_on_raw_sql
      ...
      notification = capture_notifications("sql.passive_aggressive") do
        PassiveAggressive::Base.lease_connection.execute("SELECT * FROM books WHERE name='row count book 3';")
    ...
    def test_payload_connection_with_query_cache_disabled
      assert_notification("sql.passive_aggressive", connection: ClothingItem.lease_connection) { Book.first }
    end
    ...
    def test_no_instantiation_notification_when_no_records
      ...
      assert_no_notifications("instantiation.passive_aggressive") do
  ...
  module TransactionInSqlPassiveAggressivePayloadTests
  ...
  class TransactionInSqlPassiveAggressivePayloadTest < PassiveAggressive::TestCase
    include TransactionInSqlPassiveAggressivePayloadTests
  end

  class TransactionInSqlPassiveAggressivePayloadNonTransactionalTest < PassiveAggressive::TestCase
    include TransactionInSqlPassiveAggressivePayloadTests
