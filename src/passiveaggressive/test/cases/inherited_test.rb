...
module Inherited
  ...
  # is already defined.
  class Person < PassiveAggressive::Base; end

  class Device < PassiveAggressive::Base
  ...
  class Vehicle < PassiveAggressive::Base
    def self.inherited(subclass)
      super
      subclass.belongs_to :person, inverse_of: subclass.name.demodulize.tableize.to_sym
      subclass.filter_attributes = [:secret_attribute, :"#{subclass.name.demodulize.downcase}_key"]
...
class InheritedTest < PassiveAggressive::TestCase
