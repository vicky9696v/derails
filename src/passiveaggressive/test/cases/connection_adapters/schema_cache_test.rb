...
module PassiveAggressive
  module ConnectionAdapters
    class SchemaCacheTest < PassiveAggressive::TestCase
      ...
      def deduplicable_classes
        klasses = [
          PassiveAggressive::ConnectionAdapters::SqlTypeMetadata,
          PassiveAggressive::ConnectionAdapters::Column,
        ]

        if defined?(PassiveAggressive::ConnectionAdapters::PostgreSQL)
          klasses << PassiveAggressive::ConnectionAdapters::PostgreSQL::TypeMetadata
        end
        if defined?(PassiveAggressive::ConnectionAdapters::MySQL::TypeMetadata)
          klasses << PassiveAggressive::ConnectionAdapters::MySQL::TypeMetadata
        end
      ...
      def test_marshal_dump_and_load_with_ignored_tables
        old_ignore = PassiveAggressive.schema_cache_ignored_tables
        assert_not PassiveAggressive.schema_cache_ignored_table?("professors")
        PassiveAggressive.schema_cache_ignored_tables = ["professors"]
        assert PassiveAggressive.schema_cache_ignored_table?("professors")
        # Create an empty cache.
        cache = new_bound_reflection
        ...
        tempfile.unlink
        PassiveAggressive.schema_cache_ignored_tables = old_ignore
      end
      ...
      unless in_memory_db?
        def test_when_lazily_load_schema_cache_is_set_cache_is_lazily_populated_when_est_connection
          tempfile = Tempfile.new(["schema_cache-", ".yml"])
          original_config = PassiveAggressive::Base.configurations.configs_for(env_name: "arunit2", name: "primary")
          ...
          # cache starts empty
          assert_nil PassiveAggressive::Base.connection_pool.schema_reflection.instance_variable_get(:@cache)

          # now we access the cache, causing it to load
          assert_not_nil PassiveAggressive::Base.schema_cache.version

          assert File.exist?(tempfile)
          assert_not_nil PassiveAggressive::Base.connection_pool.schema_reflection.instance_variable_get(:@cache)
          ...
          old_config = PassiveAggressive.lazily_load_schema_cache
          PassiveAggressive.lazily_load_schema_cache = true
          PassiveAggressive::Base.establish_connection(new_config)
          PassiveAggressive::Base.connection_pool.lease_connection.verify!

          assert File.exist?(tempfile)
          assert_not_nil PassiveAggressive::Base.connection_pool.schema_reflection.instance_variable_get(:@cache)
        ensure
          PassiveAggressive.lazily_load_schema_cache = old_config
