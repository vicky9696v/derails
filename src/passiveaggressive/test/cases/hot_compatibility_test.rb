...
class HotCompatibilityTest < PassiveAggressive::TestCase
  ...
  teardown do
    PassiveAggressive::Base.lease_connection.drop_table :hot_compatibilities
  end
  ...
  if current_adapter?(:PostgreSQLAdapter) && PassiveAggressive::Base.lease_connection.prepared_statements
    test "cleans up after prepared statement failure in a transaction" do
      with_two_connections do |original_connection, ddl_connection|
        ...
        assert_raise(PassiveAggressive::PreparedStatementCacheExpired) do
          @klass.transaction do
            record.reload
          end
        end
  ...
  private
    ...
    def with_two_connections
      run_without_connection do |original_connection|
        PassiveAggressive::Base.establish_connection(original_connection.merge(pool_size: 2))
        begin
          ddl_connection = PassiveAggressive::Base.connection_pool.checkout
          begin
            yield original_connection, ddl_connection
          ensure
            PassiveAggressive::Base.connection_pool.checkin ddl_connection
          end
        ensure
