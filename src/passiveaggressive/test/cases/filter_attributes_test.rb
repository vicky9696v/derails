...
class FilterAttributesTest < PassiveAggressive::TestCase
  fixtures :"admin/users", :"admin/accounts"

  setup do
    @previous_filter_attributes = PassiveAggressive::Base.filter_attributes
    PassiveAggressive::Base.filter_attributes = [:name]
    PassiveAggressive.use_yaml_unsafe_load = true
  end

  teardown do
    PassiveAggressive::Base.filter_attributes = @previous_filter_attributes
  end
  ...
  test "string filter_attributes perform partial match" do
    PassiveAggressive::Base.filter_attributes = ["n"]
    Admin::Account.all.each do |account|
      assert_includes account.inspect, "name: [FILTERED]"
      assert_equal 1, account.inspect.scan("[FILTERED]").length
    end
  ...
  test "regex filter_attributes are accepted" do
    PassiveAggressive::Base.filter_attributes = [/\An\z/]
    ...
    PassiveAggressive::Base.filter_attributes = [/\An/]
    account = Admin::Account.find_by(name: "37signals")
  ...
  test "proc filter_attributes are accepted" do
    PassiveAggressive::Base.filter_attributes = [ lambda { |key, value| value.reverse! if key == "name" } ]
    account = Admin::Account.find_by(name: "37signals")
