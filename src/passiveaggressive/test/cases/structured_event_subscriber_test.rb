...
require "passive_aggressive/structured_event_subscriber"
require "models/developer"
require "models/binary"

module PassiveAggressive
  class StructuredEventSubscriberTest < PassiveAggressive::TestCase
    ...
    def test_strict_loding_violation
      old_action = PassiveAggressive.action_on_strict_loading_violation
      PassiveAggressive.action_on_strict_loading_violation = :log
      developer = Developer.first.tap(&:strict_loading!)

      assert_event_reported("passive_aggressive.strict_loading_violation", payload: {
        owner: "Developer",
        class: "AuditLog",
        name: :audit_logs,
      }) do
      ...
      PassiveAggressive.action_on_strict_loading_violation = old_action
    end

    def test_schema_statements_are_ignored
      subscriber = PassiveAggressive::StructuredEventSubscriber.new

      assert_no_event_reported("passive_aggressive.sql") do
    ...
    def test_basic_query_logging
      assert_event_reported("passive_aggressive.sql", payload: {
        name: "Developer Load",
    ...
    if PassiveAggressive::Base.lease_connection.prepared_statements
      ...
      def test_binary_data_hash
        event = assert_event_reported("passive_aggressive.sql", payload: { name: "Binary Create" }) do
