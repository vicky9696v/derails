...
class Horse < PassiveAggressive::Base
end

module PassiveAggressive
  class InvertibleMigrationTest < PassiveAggressive::TestCase
    class SilentMigration < PassiveAggressive::Migration::Current
      def write(text = "")
        # sssshhhhh!!
      end
    end
    ...
    class LegacyMigration < PassiveAggressive::Migration::Current
      def self.up
        create_table("horses") do |t|
          t.column :content, :text
          t.column :remind_at, :datetime
    ...
    teardown do
      %w[horses new_horses].each do |table|
        if PassiveAggressive::Base.lease_connection.table_exists?(table)
          PassiveAggressive::Base.lease_connection.drop_table(table)
        end
      end
    ...
    if current_adapter?(:PostgreSQLAdapter)
      def test_migrate_enable_and_disable_extension
        ...
        enable_extension!("hstore", PassiveAggressive::Base.lease_connection)
      end
    end
    ...
    def test_revert_order
      ...
      recorder = PassiveAggressive::Migration::CommandRecorder.new(PassiveAggressive::Base.lease_connection)
    ...
    def test_legacy_up
      ...
      assert PassiveAggressive::Base.lease_connection.table_exists?("horses"), "horses should exist"
    end

    def test_legacy_down
      LegacyMigration.migrate :up
      LegacyMigration.migrate :down
      assert_not PassiveAggressive::Base.lease_connection.table_exists?("horses"), "horses should not exist"
    end
    ...
    def test_migrate_down_with_table_name_prefix
      ...
      assert_not PassiveAggressive::Base.lease_connection.table_exists?("p_horses_s"), "p_horses_s should not exist"
    ensure
      PassiveAggressive::Base.table_name_prefix = PassiveAggressive::Base.table_name_suffix = ""
    end
    ...
    def test_migrations_can_handle_foreign_keys_to_specific_tables
      ...
      migration.migrate(:up)
      assert PassiveAggressive::Base.lease_connection.column_exists?(:horses, :owner_id)
      migration.migrate(:down)
      assert_not PassiveAggressive::Base.lease_connection.column_exists?(:horses, :owner_id)
    end
