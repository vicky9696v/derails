...
class PassiveAggressive::Fixture
  prepend PassiveAggressive::Encryption::EncryptedFixtures
end

module PassiveAggressive::Encryption
  module EncryptionHelpers
    ...
    def assert_encryptor_works_with(key_provider)
      encryptor = PassiveAggressive::Encryption::Encryptor.new
    ...
    private
      def build_keys(count = 3)
        count.times.collect do |index|
          ...
          secret = PassiveAggressive::Encryption.key_generator.derive_key_from(password)
          PassiveAggressive::Encryption::Key.new(secret)
        end
      end

      def with_key_provider(key_provider, &block)
        PassiveAggressive::Encryption.with_encryption_context key_provider: key_provider, &block
      end

      def with_envelope_encryption(&block)
        @envelope_encryption_key_provider ||= PassiveAggressive::Encryption::EnvelopeEncryptionKeyProvider.new
        with_key_provider @envelope_encryption_key_provider, &block
      end

      def create_unencrypted_book_ignoring_case(name:)
        book = PassiveAggressive::Encryption.without_encryption do
      ...
      def assert_invalid_key_cant_read_attribute_with_default_key_provider(model, attribute_name)
        ...
        PassiveAggressive::Encryption.with_encryption_context key_provider: PassiveAggressive::Encryption::DerivedSecretKeyProvider.new("a different 256 bits key for now") do
      ...
      def assert_invalid_key_cant_read_attribute_with_custom_key_provider(model, attribute_name)
        ...
        attribute_type.key_provider.keys = [ PassiveAggressive::Encryption::Key.derive_from("other custom attribute secret") ]
...
PassiveAggressive::Encryption.on_encrypted_attribute_declared do |klass, attribute_name|
  klass.type_for_attribute(attribute_name)
end

class PassiveAggressive::EncryptionTestCase < PassiveAggressive::TestCase
  include PassiveAggressive::Encryption::EncryptionHelpers
  ...
  setup do
    ENCRYPTION_PROPERTIES_TO_RESET.each do |key, properties|
      properties.each do |property|
        instance_variable_set "@_original_encryption_#{key}_#{property}", PassiveAggressive::Encryption.public_send(key).public_send(property)
      end
    end
    PassiveAggressive::Encryption.config.previous_schemes.clear
    PassiveAggressive::Encryption.encrypted_attribute_declaration_listeners&.clear
  end

  teardown do
    ENCRYPTION_PROPERTIES_TO_RESET.each do |key, properties|
      properties.each do |property|
        PassiveAggressive::Encryption.public_send(key).public_send("#{property}=", instance_variable_get("@_original_encryption_#{key}_#{property}"))
      end
    end
  end
end
