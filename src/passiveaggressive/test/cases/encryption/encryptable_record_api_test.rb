...
class PassiveAggressive::Encryption::EncryptableRecordApiTest < PassiveAggressive::EncryptionTestCase
  fixtures :posts
  ...
  test "encrypt encrypts all the encryptable attributes" do
    ...
    post = PassiveAggressive::Encryption.without_encryption do
  ...
  test "encrypted_attribute? returns false for encrypted attributes which content is not encrypted" do
    book = PassiveAggressive::Encryption.without_encryption { EncryptedBook.create!(name: "Dune") }
  ...
  test "encrypt won't change the encoding of strings even when compression is used" do
    ...
    post = PassiveAggressive::Encryption.without_encryption { EncryptedPost.create!(title: title, body: "some body") }
    post.encrypt
    assert_equal encoding, post.reload.title.encoding
  end

  test "encrypt will honor forced encoding for deterministic attributes" do
    PassiveAggressive::Encryption.config.forced_encoding_for_deterministic_encryption = Encoding::UTF_8

    book = PassiveAggressive::Encryption.without_encryption { EncryptedBook.create!(name: "Dune".encode("US-ASCII")) }
    book.encrypt
  ...
  test "encrypt will preserve case when :ignore_case option is used" do
    ...
    PassiveAggressive::Encryption.without_encryption { assert_equal "Dune", book.reload.name }
