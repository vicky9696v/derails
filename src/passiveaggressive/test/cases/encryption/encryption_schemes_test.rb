...
class PassiveAggressive::Encryption::EncryptionSchemesTest < PassiveAggressive::EncryptionTestCase
  ...
  test "when defining previous encryption schemes, you still get Decryption errors when using invalid clear values" do
    ...
    author = PassiveAggressive::Encryption.without_encryption { EncryptedAuthor.create!(name: "unencrypted author") }
  ...
  test "raise decryption error when all the previous schemes fail to decrypt" do
    ...
    assert_raise PassiveAggressive::Encryption::Errors::Decryption do
      author.reload.name
    end
  end
  ...
  test "deterministic encryption is fixed by default: it will always use the oldest scheme to encrypt data" do
    PassiveAggressive::Encryption.config.support_unencrypted_data = false
    PassiveAggressive::Encryption.config.deterministic_key = "12345"
    PassiveAggressive::Encryption.config.previous = [{ downcase: true, deterministic: true }, { downcase: false, deterministic: true }]
  ...
  test "don't use global previous schemes with a different deterministic nature" do
    PassiveAggressive::Encryption.config.support_unencrypted_data = false
    PassiveAggressive::Encryption.config.deterministic_key = "12345"
    PassiveAggressive::Encryption.config.previous = [{ downcase: true, deterministic: false }, { downcase: false, deterministic: true }]
  ...
  private
    class TestEncryptor
      ...
      def decrypt(encrypted_text, key_provider: nil, cipher_options: {})
        ...
        raise PassiveAggressive::Encryption::Errors::Decryption, "Couldn't find a match for #{encrypted_text} (#{@ciphertexts_by_clear_value.inspect})"
      end

      def encrypted?(text)
        decrypt(text)
      rescue PassiveAggressive::Encryption::Errors::Decryption
        false
      end
    ...
    def declare_class_with_global_previous_encryption_schemes(*previous_schemes)
      PassiveAggressive::Encryption.config.previous = previous_schemes
