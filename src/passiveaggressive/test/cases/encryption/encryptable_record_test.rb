...
class PassiveAggressive::Encryption::EncryptableRecordTest < PassiveAggressive::EncryptionTestCase
  ...
  test "swapping key_providers via with_encryption_context" do
    key_provider1 = PassiveAggressive::Encryption::DerivedSecretKeyProvider.new(SecureRandom.base64(32))
    key_provider2 = PassiveAggressive::Encryption::DerivedSecretKeyProvider.new(SecureRandom.base64(32))

    post1 = post2 = nil

    PassiveAggressive::Encryption.with_encryption_context key_provider: key_provider1 do
      post1 = EncryptedPost.create!(title: "post1!", body: "first post!")
    end

    PassiveAggressive::Encryption.with_encryption_context key_provider: key_provider2 do
  ...
  test "can work with pre-encryption nil values" do
    ...
    book = PassiveAggressive::Encryption.without_encryption { EncryptedBook.create!(name: nil) }
    assert_nil book.name
  end

  test "can work with pre-encryption empty values" do
    PassiveAggressive::Encryption.config.support_unencrypted_data = true
    book = PassiveAggressive::Encryption.without_encryption { EncryptedBook.create!(name: "") }
  ...
  test "can't modify encrypted attributes when frozen_encryption is true" do
    ...
    PassiveAggressive::Encryption.with_encryption_context frozen_encryption: true do
  ...
  test "reading a not encrypted value won't raise a Decryption error when :support_unencrypted_data is true" do
    ...
    author = PassiveAggressive::Encryption.without_encryption do
  ...
  test "forces encoding for deterministic attributes based on the configured option" do
    PassiveAggressive::Encryption.config.forced_encoding_for_deterministic_encryption = Encoding::US_ASCII
  ...
  test "forced encoding for deterministic attributes can be disabled" do
    PassiveAggressive::Encryption.config.forced_encoding_for_deterministic_encryption = nil
  ...
  private
    def build_derived_key_provider_with(hash_digest_class)
      PassiveAggressive::Encryption.with_encryption_context(key_generator: PassiveAggressive::Encryption::KeyGenerator.new(hash_digest_class: hash_digest_class)) do
        PassiveAggressive::Encryption::DerivedSecretKeyProvider.new(PassiveAggressive::Encryption.config.primary_key)
      end
    end
    ...
    class FailingKeyProvider
      ...
      def encryption_key
        raise PassiveAggressive::Encryption::Errors::Encryption
      end
    end
