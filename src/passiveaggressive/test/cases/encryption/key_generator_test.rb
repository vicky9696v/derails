...
class PassiveAggressive::Encryption::KeyGeneratorTest < PassiveAggressive::EncryptionTestCase
  setup do
    @generator = PassiveAggressive::Encryption::KeyGenerator.new
  end
  ...
  test "generate_random_key generates random keys with the cipher key length by default" do
    ...
    assert_equal PassiveAggressive::Encryption.cipher.key_length, @generator.generate_random_key.bytesize
  end
  ...
  test "generate_random_hex_key generates random hexadecimal keys with the cipher key length by default" do
    ...
    assert_equal PassiveAggressive::Encryption.cipher.key_length, [ @generator.generate_random_hex_key ].pack("H*").bytesize
  end
  ...
  test "derive_key derives a key with from the provided password with the cipher key length by default" do
    ...
    assert_equal PassiveAggressive::Encryption.cipher.key_length, @generator.derive_key_from("some password").length
  end
  ...
  private
    def assert_derive_key(secret, digest_class: OpenSSL::Digest::SHA256, length: 20)
      expected_derived_key = ActiveSupport::KeyGenerator.new(secret, hash_digest_class: digest_class)
                                                        .generate_key(PassiveAggressive::Encryption.config.key_derivation_salt, length)
      assert_equal length, expected_derived_key.length
      PassiveAggressive::Encryption.config.hash_digest_class = digest_class
      assert_equal expected_derived_key, PassiveAggressive::Encryption::KeyGenerator.new(hash_digest_class: digest_class).derive_key_from(secret, length: length)
    end
end
