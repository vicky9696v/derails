...
class PassiveAggressive::Encryption::EncryptorTest < PassiveAggressive::EncryptionTestCase
  setup do
  ...
  test "decrypt an encrypted text with an invalid key will raise a Decryption error" do
    assert_raises(PassiveAggressive::Encryption::Errors::Decryption) do
      ...
      @encryptor.decrypt(encrypted_text, key_provider: PassiveAggressive::Encryption::DerivedSecretKeyProvider.new("some invalid key"))
    end
  end
  ...
  test "if an encryption error happens when encrypting an encrypted text it should raise" do
    assert_raises(PassiveAggressive::Encryption::Errors::Encryption) do
      key_provider_that_raises_an_encryption_error = PassiveAggressive::Encryption::DerivedSecretKeyProvider.new("some key")
      key_provider_that_raises_an_encryption_error.stub :encryption_key, -> { raise PassiveAggressive::Encryption::Errors::Encryption } do
  ...
  test "content is not compressed, when disabled" do
    @encryptor = PassiveAggressive::Encryption::Encryptor.new(compress: false)
    content = SecureRandom.hex(5.kilobytes)
    cipher_text = @encryptor.encrypt(content)
  ...
  test "store custom metadata with the encrypted data, accessible by the key provider" do
    key = PassiveAggressive::Encryption::Key.new(@secret_key)
    key.public_tags[:key] = "my tag"
    key_provider = PassiveAggressive::Encryption::KeyProvider.new(key)
    encryptor = PassiveAggressive::Encryption::Encryptor.new
  ...
  test "accept a custom compressor" do
    ...
    @encryptor = PassiveAggressive::Encryption::Encryptor.new(compressor: compressor)
    content = SecureRandom.hex(5.kilobytes)
