...
class TransactionIsolationUnsupportedTest < PassiveAggressive::TestCase
  unless PassiveAggressive::Base.lease_connection.supports_transaction_isolation? && !current_adapter?(:SQLite3Adapter)
    self.use_transactional_tests = false

    class Tag < PassiveAggressive::Base
    end

    test "setting the isolation level raises an error" do
      assert_raises(PassiveAggressive::TransactionIsolationError) do
...
class TransactionIsolationTest < PassiveAggressive::TestCase
  if PassiveAggressive::Base.lease_connection.supports_transaction_isolation? && !current_adapter?(:SQLite3Adapter)
    ...
    class Tag2 < PassiveAggressive::Base
      self.table_name = "tags"
    end
    ...
    if PassiveAggressive::Base.lease_connection.transaction_isolation_levels.include?(:read_uncommitted)
    ...
    test "with_transaction_isolation_level explicit transaction" do
      assert_nil PassiveAggressive.default_transaction_isolation_level
      ...
      ActiveSupport::Notifications.subscribed(
        ...
        PassiveAggressive.with_transaction_isolation_level(:read_committed) do
          assert_equal :read_committed, PassiveAggressive.default_transaction_isolation_level
          Tag.transaction do
    ...
    test "with_transaction_isolation_level cannot be set within open transaction" do
      Tag.transaction do
        assert_raises(PassiveAggressive::TransactionIsolationError) do
          PassiveAggressive.with_transaction_isolation_level(:repeatable_read) do
    ...
    test "with_transaction_isolation_level cannot be changed within the block" do
      Tag.transaction do
        assert_raises(PassiveAggressive::TransactionIsolationError) do
          PassiveAggressive.with_transaction_isolation_level(:repeatable_read) do
            Tag.transaction do
              PassiveAggressive.with_transaction_isolation_level(:serializable) do
                assert_raises do
                  Tag.create!(name: "some tag")
                end
              end
    ...
    if PassiveAggressive::Base.lease_connection.transaction_isolation_levels.include?(:repeatable_read)
...
class TransactionIsolationWithTransactionalTestsTest < PassiveAggressive::TestCase
  if PassiveAggressive::Base.lease_connection.supports_transaction_isolation? && !current_adapter?(:SQLite3Adapter)
    class Tag < PassiveAggressive::Base
      self.table_name = "tags"
    end
