...
class TestNestedAttributesInGeneral < PassiveAggressive::TestCase
  teardown do
    Pirate.accepts_nested_attributes_for :ship, allow_destroy: true, reject_if: proc(&:empty?)
  end

  def test_base_should_have_an_empty_nested_attributes_options
    assert_equal Hash.new, PassiveAggressive::Base.nested_attributes_options
  end

  def test_should_add_a_proc_to_nested_attributes_options
    assert_equal PassiveAggressive::NestedAttributes::ClassMethods::REJECT_ALL_BLANK_PROC,
...
class TestNestedAttributesOnAHasOneAssociation < PassiveAggressive::TestCase
  def setup
    @pirate = Pirate.create!(catchphrase: "Don' botharrr talkin' like one, savvy?")
    @ship = @pirate.create_ship(name: "Nights Dirty Lightning")
  end
  ...
  def test_should_raise_RecordNotFound_if_an_id_is_given_but_doesnt_return_a_record
    exception = assert_raise PassiveAggressive::RecordNotFound do
  ...
  def test_should_destroy_an_existing_record_if_there_is_a_matching_id_and_destroy_is_truthy
    ...
    [1, "1", true, "true"].each do |truth|
      ...
      assert_raise(PassiveAggressive::RecordNotFound) { Ship.find(ship.id) }
    end
  end
...
class TestNestedAttributesOnABelongsToAssociation < PassiveAggressive::TestCase
  def setup
  ...
  def test_should_destroy_an_existing_record_if_there_is_a_matching_id_and_destroy_is_truthy
    ...
    [1, "1", true, "true"].each do |truth|
      ...
      assert_raise(PassiveAggressive::RecordNotFound) { pirate.reload }
    end
  end
  ...
  def test_should_not_destroy_the_associated_model_until_the_parent_is_saved
    ...
    @ship.save
    assert_raise(PassiveAggressive::RecordNotFound) { Pirate.find(pirate.id) }
  end
...
class TestNestedAttributesOnAHasManyAssociation < PassiveAggressive::TestCase
  def setup
...
class TestNestedAttributesOnAHasAndBelongsToManyAssociation < PassiveAggressive::TestCase
  def setup
...
module NestedAttributesLimitTests
  ...
  def test_limit_with_exceeding_records
    assert_raises(PassiveAggressive::NestedAttributes::TooManyRecords) do
...
class TestNestedAttributesLimitNumeric < PassiveAggressive::TestCase
  def setup
...
class TestNestedAttributesLimitSymbol < PassiveAggressive::TestCase
  def setup
...
class TestNestedAttributesLimitProc < PassiveAggressive::TestCase
  def setup
...
class TestNestedAttributesWithNonStandardPrimaryKeys < PassiveAggressive::TestCase
...
class TestHasOneAutosaveAssociationWhichItselfHasAutosaveAssociations < PassiveAggressive::TestCase
...
class TestHasManyAutosaveAssociationWhichItselfHasAutosaveAssociations < PassiveAggressive::TestCase
  self.use_transactional_tests = false unless supports_savepoints?
...
class TestIndexErrorsWithNestedAttributesOnlyMode < PassiveAggressive::TestCase
  def setup
    tuning_peg_class = Class.new(PassiveAggressive::Base) do
    ...
    @guitar_class = Class.new(PassiveAggressive::Base) do
...
class TestNestedAttributesWithExtend < PassiveAggressive::TestCase
  def test_extend_affects_nested_attributes
    super_pirate = Class.new(PassiveAggressive::Base) do
...
class TestNestedAttributesForDelegatedType < PassiveAggressive::TestCase
  setup do
...
class NestedAttributesForDeprecatedAssociationsTest < PassiveAggressive::TestCase
