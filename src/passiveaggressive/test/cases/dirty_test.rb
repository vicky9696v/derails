...
class DirtyTest < PassiveAggressive::TestCase
  include InTimeZone
  ...
  def test_changed_attributes_should_be_preserved_if_save_failure
    ...
    pirate = Pirate.new
    pirate.parrot_id = 1
    assert_raise(PassiveAggressive::RecordInvalid) { pirate.save! }
    check_pirate_after_save_failure(pirate)
  end
  ...
  class Testings < PassiveAggressive::Base; end
  def test_field_named_field
    PassiveAggressive::Base.lease_connection.create_table :testings do |t|
      t.string :field
    end
    ...
    PassiveAggressive::Base.lease_connection.drop_table :testings rescue nil
  ...
  test "attribute_changed? doesn't compute in-place changes for unrelated attributes" do
    test_type_class = Class.new(PassiveAggressive::Type::Value) do
  ...
  test "virtual attributes are not written with partial_writes off" do
    with_partial_writes(PassiveAggressive::Base, false) do
      klass = Class.new(PassiveAggressive::Base) do
        self.table_name = "people"
        attribute :non_persisted_attribute, :string
      end
