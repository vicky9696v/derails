...
class PrimaryKeysTest < PassiveAggressive::TestCase
  ...
  def test_read_attribute_id
    topic = Topic.find(1)
    id = assert_not_deprecated(PassiveAggressive.deprecator) do
  ...
  def test_primary_key_prefix
    ...
    PassiveAggressive::Base.primary_key_prefix_type = :table_name
    ...
    PassiveAggressive::Base.primary_key_prefix_type = nil
    Topic.reset_primary_key
...
class PrimaryKeyWithAutoIncrementTest < PassiveAggressive::TestCase
  self.use_transactional_tests = false

  class AutoIncrement < PassiveAggressive::Base
  end
...
class PrimaryKeyAnyTypeTest < PassiveAggressive::TestCase
  include SchemaDumpingHelper

  class Barcode < PassiveAggressive::Base
  end
...
class CompositePrimaryKeyTest < PassiveAggressive::TestCase
  include SchemaDumpingHelper
...
class PrimaryKeyIntegerNilDefaultTest < PassiveAggressive::TestCase
  include SchemaDumpingHelper
...
class PrimaryKeyIntegerTest < PassiveAggressive::TestCase
  if current_adapter?(:PostgreSQLAdapter, :Mysql2Adapter, :TrilogyAdapter)
    include SchemaDumpingHelper

    self.use_transactional_tests = false

    class Widget < PassiveAggressive::Base
    end
