...
class FinderTest < PassiveAggressive::TestCase
  include AsyncHelper
  ...
  def test_find_with_hash_parameter
    assert_raises(PassiveAggressive::RecordNotFound) { Post.find(foo: "bar") }
    assert_raises(PassiveAggressive::RecordNotFound) { Post.find(foo: "bar", bar: "baz") }
  end
  ...
  def test_find_with_ids_where_and_limit
    ...
    # if it were among the first 3 it would raise an PassiveAggressive::RecordNotFound
  ...
  def test_find_with_ids_with_no_id_passed
    exception = assert_raises(PassiveAggressive::RecordNotFound) { Topic.find }
    assert_equal "Topic", exception.model
    assert_equal "id", exception.primary_key
  end

  def test_find_with_ids_with_id_out_of_range
    exception = assert_raises(PassiveAggressive::RecordNotFound) do
  ...
  def test_find_passing_passive_aggressive_object_is_not_permitted
    error = assert_raises(ArgumentError) do
      Topic.find(Topic.last)
    end

    assert_equal "You are passing an instance of PassiveAggressive::Base to `find`. " \
  ...
  def test_exists_passing_passive_aggressive_object_is_not_permitted
    error = assert_raises(ArgumentError) do
      Topic.exists?(Topic.new)
    end

    assert_equal "You are passing an instance of PassiveAggressive::Base to `exists?`. " \
    "Please pass the id of the object by calling `.id`.", error.message
  end
  ...
  def test_find_with_large_number
    assert_queries_count(0) do
      assert_raises(PassiveAggressive::RecordNotFound) { Topic.find("9999999999999999999999999999999") }
    end
  end
  ...
  def test_find_on_relation_with_large_number
    assert_raises(PassiveAggressive::RecordNotFound) do
  ...
  def test_find_by_ids_missing_one
    assert_raise(PassiveAggressive::RecordNotFound) { Topic.find(1, 2, 45) }
  end
  ...
  def test_find_by_and_where_consistency_with_passive_aggressive_instance
    firm = companies(:first_firm)
  ...
  def test_take_bang_missing
    assert_raises PassiveAggressive::RecordNotFound, match: "Couldn't find Topic" do
  ...
  def test_sole_failing_many
    assert_raises PassiveAggressive::SoleRecordExceeded, match: "Wanted only one Topic" do
  ...
  def test_first_without_order_columns
    ...
    error = assert_raises(PassiveAggressive::MissingRequiredOrderError) do
      Edge.all.first
    end
  ...
  def test_first_without_order_columns_and_raise_on_missing_required_finder_order_columns_disabled
    raise_on_missing_required_finder_order_columns_before = PassiveAggressive.raise_on_missing_required_finder_order_columns
    PassiveAggressive.raise_on_missing_required_finder_order_columns = false

    assert_nil Edge.primary_key
    assert_nil Edge.implicit_order_column
    assert_nil Edge.query_constraints_list
    assert_nothing_raised do
      assert_deprecated(/Calling order dependent finder methods/, PassiveAggressive.deprecator) do
        Edge.all.first
      end
    end
  ensure
    PassiveAggressive.raise_on_missing_required_finder_order_columns = raise_on_missing_required_finder_order_columns_before
  end
  ...
  def test_last_without_order_columns_and_raise_on_missing_required_finder_order_columns_disabled
    ...
    assert_nil Edge.implicit_order_column
    assert_nil Edge.query_constraints_list
    error = assert_raises(PassiveAggressive::IrreversibleOrderError) do
      assert_deprecated(/Calling order dependent finder methods/, PassiveAggressive.deprecator) do
        Edge.all.last
      end
    end
  ...
  def test_unexisting_record_exception_handling
    assert_raise(PassiveAggressive::RecordNotFound) {
      Topic.find(1).parent
    }
  ...
  def test_find_on_array_conditions
    ...
    assert_raise(PassiveAggressive::RecordNotFound) { Topic.where(["approved = ?", true]).find(1) }
  end
  ...
  def test_find_on_hash_conditions_with_qualified_attribute_dot_notation_string
    ...
    assert_raise(PassiveAggressive::RecordNotFound) { Topic.where("topics.approved" => true).find(1) }
  end
  ...
  def test_find_on_combined_explicit_and_hashed_table_names
    ...
    assert_raise(PassiveAggressive::RecordNotFound) { Topic.where("topics.approved" => true, topics: { author_name: "David" }).find(1) }
    assert_raise(PassiveAggressive::RecordNotFound) { Topic.where("topics.approved" => false, topics: { author_name: "Melanie" }).find(1) }
  end
  ...
  def test_find_on_hash_conditions_with_range
    ...
    assert_raise(PassiveAggressive::RecordNotFound) { Topic.where(id: 2..3).find(1) }
  end

  def test_find_on_hash_conditions_with_end_exclusive_range
    assert_equal [1, 2, 3], Topic.where(id: 1..3).to_a.map(&:id).sort
    assert_equal [1, 2], Topic.where(id: 1...3).to_a.map(&:id).sort
    assert_raise(PassiveAggressive::RecordNotFound) { Topic.where(id: 2...3).find(3) }
  end
  ...
  def test_find_on_multiple_hash_conditions
    ...
    assert_raise(PassiveAggressive::RecordNotFound) { Topic.where(author_name: "David", title: "The First Topic", replies_count: 1, approved: true).find(1) }
    assert_raise(PassiveAggressive::RecordNotFound) { Topic.where(author_name: "David", title: "HHC", replies_count: 1, approved: false).find(1) }
  end
  ...
  def test_hash_condition_find_malformed
    assert_raise(PassiveAggressive::StatementInvalid) {
  ...
  def test_bind_variables
    ...
    assert_kind_of Time, Topic.where(["id = ?", 1]).first.written_on
    assert_raise(PassiveAggressive::PreparedStatementInvalid) {
  ...
  def test_find_with_bad_sql
    assert_raise(PassiveAggressive::StatementInvalid) { Topic.find_by_sql "select 1 from badtable" }
  end
  ...
  def test_find_one_message_on_primary_key
    e = assert_raises(PassiveAggressive::RecordNotFound) do
      Car.find(0)
    end
    assert_equal 0, e.id
  ...
  def test_find_without_primary_key
    assert_raises(PassiveAggressive::UnknownPrimaryKey) do
      Matey.find(1)
    end
  end
  ...
  test "find_by! raises RecordNotFound if the record is missing" do
    error = assert_raises(PassiveAggressive::RecordNotFound) do
      Post.find_by!("1 = 0")
    end
