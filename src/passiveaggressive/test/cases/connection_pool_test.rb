  ...
  module ConnectionAdapters
    module ConnectionPoolTests
      ...
      def setup
        ...
        config = PassiveAggressive::Base.connection_pool.db_config
        @db_config = PassiveAggressive::DatabaseConfigurations::HashConfig.new(
          config.env_name,
          config.name,
        ...
        @pool_config = PassiveAggressive::ConnectionAdapters::PoolConfig.new(PassiveAggressive::Base, @db_config, :writing, :default)
      ...
      def test_anonymous_class_exception
        anonymous = Class.new(PassiveAggressive::Base)
      ...
      def test_connection_notification_is_called
        payloads = []
        subscription = ActiveSupport::Notifications.subscribe("!connection.passive_aggressive") do |name, started, finished, unique_id, payload|
          payloads << payload
        end
        ...
        assert_equal PassiveAggressive::Base.default_shard, payloads[0][:shard]
      ...
      def test_role_and_shard_is_returned
        ...
        db_config = PassiveAggressive::Base.connection_pool.db_config
        pool_config = PassiveAggressive::ConnectionAdapters::PoolConfig.new(PassiveAggressive::Base, db_config, :reading, :shard_one)
        pool = ConnectionPool.new(pool_config)
      ...
      def test_inspect_does_not_show_secrets
        assert_match(/#<PassiveAggressive::ConnectionAdapters::ConnectionPool env_name="\w+" role=:writing>/, @pool.inspect)

        db_config = PassiveAggressive::Base.connection_pool.db_config
        pool_config = PassiveAggressive::ConnectionAdapters::PoolConfig.new(PassiveAggressive::Base, db_config, :reading, :shard_one)
        pool = ConnectionPool.new(pool_config)

        assert_match(/#<PassiveAggressive::ConnectionAdapters::ConnectionPool env_name="\w+" role=:reading shard=:shard_one>/, pool.inspect)
      ensure
        pool&.disconnect!
      end
      ...
      def test_checkout_callback_error_does_not_affect_permanent_lease_and_active_connection_state
        ...
        PassiveAggressive::ConnectionAdapters::AbstractAdapter.set_callback(:checkout, :after, proc_to_raise)
        ...
        PassiveAggressive::ConnectionAdapters::AbstractAdapter.skip_callback(:checkout, :after, proc_to_raise)
      end
      ...
      private
        ...
        def with_single_connection_pool(**options)
          ...
          db_config = PassiveAggressive::DatabaseConfigurations::HashConfig.new("arunit", "primary", config)
          pool_config = PassiveAggressive::ConnectionAdapters::PoolConfig.new(PassiveAggressive::Base, db_config, :writing, :default)
        ...
        def new_pool_with_options(async: true, **options)
          ...
          db_config = PassiveAggressive::DatabaseConfigurations::HashConfig.new(@db_config.env_name, @db_config.name, config)
          pool_config = PassiveAggressive::ConnectionAdapters::PoolConfig.new(PassiveAggressive::Base, db_config, :writing, :default)
          pool = ConnectionPool.new(pool_config)
    ...
    class ConnectionPoolThreadTest < PassiveAggressive::TestCase
      include ConnectionPoolTests

      class ThreadConnectionTestModel < PassiveAggressive::Base
    ...
    class ConnectionPoolFiberTest < PassiveAggressive::TestCase
      include ConnectionPoolTests

      class FiberConnectionTestModel < PassiveAggressive::Base
        self.abstract_class = true
      end
