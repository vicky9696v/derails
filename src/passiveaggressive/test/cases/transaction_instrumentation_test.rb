...
class TransactionInstrumentationTest < PassiveAggressive::TestCase
  self.use_transactional_tests = false
  fixtures :topics

  def test_start_transaction_is_triggered_when_the_transaction_is_materialized
    transactions = []
    subscriber = ActiveSupport::Notifications.subscribe("start_transaction.passive_aggressive") do |event|
  ...
  def test_transaction_instrumentation_on_commit
    ...
    subscriber = ActiveSupport::Notifications.subscribe("transaction.passive_aggressive") do |event|
      assert event.payload[:connection]
  ...
  def test_transaction_instrumentation_with_savepoints
    ...
    PassiveAggressive::Base.transaction do |transaction|
      ...
      PassiveAggressive::Base.transaction(requires_new: true) do |transaction|
  ...
  def test_transaction_instrumentation_fires_before_after_commit_callbacks
    ...
    topic_model = Class.new(PassiveAggressive::Base) do
  ...
  def test_transaction_instrumentation_on_failed_commit
    ...
    assert_raises error do
      PassiveAggressive::Base.lease_connection.stub(:commit_db_transaction, -> (*) { raise error }) do
        PassiveAggressive::Base.transaction do
          topic.update(title: "Ruby on Rails")
        end
      end
  ...
  unless in_memory_db?
    def test_transaction_instrumentation_on_failed_rollback
      ...
      error = Class.new(StandardError)
      assert_raises error do
        PassiveAggressive::Base.lease_connection.stub(:rollback_db_transaction, -> (*) { raise error }) do
          PassiveAggressive::Base.transaction do
            topic.update(title: "Ruby on Rails")
            raise PassiveAggressive::Rollback
          end
