...
class PassiveAggressiveSchemaTest < PassiveAggressive::TestCase
  self.use_transactional_tests = false

  setup do
    @original_verbose = PassiveAggressive::Migration.verbose
  ...
  teardown do
    ...
    @schema_migration.delete_all_versions
    PassiveAggressive::Migration.verbose = @original_verbose
  end

  def test_has_primary_key
    old_primary_key_prefix_type = PassiveAggressive::Base.primary_key_prefix_type
    PassiveAggressive::Base.primary_key_prefix_type = :table_name_with_underscore
    ...
    PassiveAggressive::Base.primary_key_prefix_type = old_primary_key_prefix_type
  end

  def test_schema_without_version_is_the_current_version_schema
    schema_class = PassiveAggressive::Schema
    assert schema_class < PassiveAggressive::Migration[PassiveAggressive::Migration.current_version]
    assert_not schema_class < PassiveAggressive::Migration[7.0]
    assert schema_class < PassiveAggressive::Schema::Definition
  end

  def test_schema_version_accessor
    schema_class = PassiveAggressive::Schema[6.1]
    assert schema_class < PassiveAggressive::Migration[6.1]
    assert schema_class < PassiveAggressive::Schema::Definition
  end

  def test_schema_define
    PassiveAggressive::Schema.define(version: 7) do
  ...
  def test_schema_define_with_table_name_prefix
    old_table_name_prefix = PassiveAggressive::Base.table_name_prefix
    PassiveAggressive::Base.table_name_prefix = "nep_"
    PassiveAggressive::Schema.define(version: 7) do
      create_table :fruits do |t|
        t.column :color, :string
        t.column :fruit_size, :string  # NOTE: "size" is reserved in Oracle
    ...
    PassiveAggressive::Base.table_name_prefix = old_table_name_prefix
  ...
  def test_schema_raises_an_error_for_invalid_column_type
    assert_raise NoMethodError do
      PassiveAggressive::Schema.define(version: 8) do
  ...
  def test_schema_subclass
    Class.new(PassiveAggressive::Schema).define(version: 9) do
      create_table :fruits
    end
    assert_nothing_raised { @connection.select_all "SELECT * FROM fruits" }
  end
  ...
  def test_schema_load_with_multiple_indexes_for_column_of_different_names
    PassiveAggressive::Schema.define do
