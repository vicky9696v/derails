...
class RelationTest < PassiveAggressive::TestCase
  ...
  def test_scoped
    topics = Topic.all
    assert_kind_of PassiveAggressive::Relation, topics
  ...
  def test_reverse_order_with_multiargument_function
    assert_raises(PassiveAggressive::IrreversibleOrderError) do
  ...
  def test_dynamic_find_by_attributes_bang
    ...
    assert_raises(PassiveAggressive::RecordNotFound) { Author.all.find_by_id_and_name!(20, "invalid") }
  end
  ...
  def test_find_id
    ...
    assert_raises(PassiveAggressive::RecordNotFound) { authors.where(name: "lifo").find("42") }
  end
  ...
  def test_find_ids
    ...
    assert_raises(PassiveAggressive::RecordNotFound) { authors.where(name: "lifo").find(authors(:david).id, "42") }
    assert_raises(PassiveAggressive::RecordNotFound) { authors.find(["42", 43]) }
  end
  ...
  def test_create_bang
    ...
    assert_raises(PassiveAggressive::RecordInvalid) { birds.create! }
  ...
  def test_first_or_create_bang_with_invalid_options
    assert_raises(PassiveAggressive::RecordInvalid) { Bird.where(color: "green").first_or_create!(pirate_id: 1) }
  end

  def test_first_or_create_bang_with_no_parameters
    assert_raises(PassiveAggressive::RecordInvalid) { Bird.where(color: "green").first_or_create! }
  end
  ...
  def test_first_or_create_bang_with_invalid_array
    assert_raises(PassiveAggressive::RecordInvalid) { Bird.where(color: "green").first_or_create!([ { name: "parrot" }, { pirate_id: 1 } ]) }
  end
  ...
  def test_find_or_create_by!
    assert_raises(PassiveAggressive::RecordInvalid) { Bird.find_or_create_by!(color: "green") }
  end
  ...
  def test_create_or_find_by_on_a_collections_rollbacks_a_transaction_when_owner_is_not_persisted
    ...
    assert_raises(PassiveAggressive::RecordNotSaved) do
  ...
  def test_create_or_find_by_with_bang_should_raise_due_to_validation_errors
    assert_raises(PassiveAggressive::RecordInvalid) { Bird.create_or_find_by!(color: "green") }
  end
  ...
  test "loaded relations cannot be mutated by multi value methods" do
    ...
    assert_raises(PassiveAggressive::UnmodifiableRelation) do
      relation.where! "foo"
    end
  end
  ...
  test "relations with cached arel can't be mutated [internal API]" do
    ...
    assert_raises(PassiveAggressive::UnmodifiableRelation) { relation.limit!(5) }
    assert_raises(PassiveAggressive::UnmodifiableRelation) { relation.where!("1 = 2") }
  end

  test "relations show the records in #inspect" do
    relation = Post.limit(2)
    assert_equal "#<PassiveAggressive::Relation [#{Post.limit(2).map(&:inspect).join(', ')}]>", relation.inspect
  end

  test "relations limit the records in #inspect at 10" do
    relation = Post.limit(11)
    assert_equal "#<PassiveAggressive::Relation [#{Post.limit(10).map(&:inspect).join(', ')}, ...]>", relation.inspect
  end
  ...
  test "already-loaded relations don't perform a new query in #inspect" do
    ...
    expected = "#<PassiveAggressive::Relation [#{Post.limit(2).map(&:inspect).join(', ')}]>"
  ...
  (PassiveAggressive::Relation::MULTI_VALUE_METHODS - [:extending, :with]).each do |method|
  ...
  private
    def custom_post_relation(alias_name = "omg_posts")
      ...
      PassiveAggressive::Relation.create(Post, table: table_alias)
    end
end

class CreateOrFindByWithinTransactions < PassiveAggressive::TestCase
...
class DeprecatedAssociationsRelationSimpleTest < PassiveAggressive::TestCase
...
class DeprecatedAssociationsRelationComplexTest < PassiveAggressive::TestCase
  include DeprecatedAssociationsTestHelpers
