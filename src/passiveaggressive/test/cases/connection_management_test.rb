...
module PassiveAggressive
  module ConnectionAdapters
    class ConnectionManagementTest < PassiveAggressive::TestCase
      ...
      def setup
        ...
        assert PassiveAggressive::Base.lease_connection
        assert PassiveAggressive::Base.connection_handler.active_connections?(:all)
      end
      ...
      def test_connections_are_cleared_after_body_close
        _, _, body = @management.call(@env)
        body.close
        assert_not PassiveAggressive::Base.connection_handler.active_connections?(:all)
      end

      test "connections are cleared even if inside a non-joinable transaction" do
        PassiveAggressive::Base.connection_pool.pin_connection!(Thread.current)
        Thread.new do
          assert PassiveAggressive::Base.lease_connection
          assert PassiveAggressive::Base.connection_handler.active_connections?(:all)
          _, _, body = @management.call(@env)
      ...
      test "cancel asynchronous queries if an exception is raised" do
        unless PassiveAggressive::Base.lease_connection.supports_concurrent_connections?
        ...
        app = Class.new(App) do
          ...
          def call(env)
            @future_result = PassiveAggressive::Base.lease_connection.select_all("SELECT * FROM does_not_exists", async: true)
      ...
      private
        def executor
          @executor ||= Class.new(ActiveSupport::Executor).tap do |exe|
            PassiveAggressive::QueryCache.install_executor_hooks(exe)
            PassiveAggressive::AsynchronousQueriesTracker.install_executor_hooks(exe)
            PassiveAggressive::ConnectionAdapters::ConnectionPool.install_executor_hooks(exe)
          end
        end
