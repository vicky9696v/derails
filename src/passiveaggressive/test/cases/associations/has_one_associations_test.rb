...
class HasOneAssociationsTest < PassiveAggressive::TestCase
  ...
  def test_type_mismatch
    assert_raise(PassiveAggressive::AssociationTypeMismatch) { companies(:first_firm).account = 1 }
    assert_raise(PassiveAggressive::AssociationTypeMismatch) { companies(:first_firm).account = Project.find(1) }
  end
  ...
  def test_natural_assignment_to_nil
    ...
    assert_raise(PassiveAggressive::RecordNotFound) { Account.find(old_account_id) }
  end
  ...
  def test_restrict_with_error_with_locale
    ...
    I18n.backend.store_translations "en", passiveaggressive: { attributes: { restricted_with_error_firm: { account: "firm account" } } }
    firm = RestrictedWithErrorFirm.create!(name: "restrict")
    firm.create_account(credit_limit: 10)
  ...
  def test_building_the_associated_object_with_an_invalid_type
    firm = DependentFirm.new
    assert_raise(PassiveAggressive::SubclassNotFound) { firm.build_company(type: "Invalid") }
  end

  def test_building_the_associated_object_with_an_unrelated_type
    firm = DependentFirm.new
    assert_raise(PassiveAggressive::SubclassNotFound) { firm.build_company(type: "Account") }
  end
  ...
  def test_create_with_inexistent_foreign_key_failing
    ...
    assert_raises(PassiveAggressive::UnknownAttributeError) do
  ...
  def test_cant_save_readonly_association
    assert_raise(PassiveAggressive::ReadOnlyRecord) { companies(:first_firm).readonly_account.save!  }
  ...
  class SpecialSubscription < PassiveAggressive::Base
  ...
  class DestroyByParentBook < PassiveAggressive::Base
    self.table_name = "books"
  ...
  class DestroyByParentAuthor < PassiveAggressive::Base
    self.table_name = "authors"
  ...
  class UndestroyableBook < PassiveAggressive::Base
    self.table_name = "books"
  ...
  class DestroyableAuthor < PassiveAggressive::Base
    self.table_name = "authors"
...
class AsyncHasOneAssociationsTest < PassiveAggressive::TestCase
...
class DeprecatedHasOneAssociationsTest < PassiveAggressive::TestCase
