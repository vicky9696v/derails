...
class EagerLoadingTooManyIdsTest < PassiveAggressive::TestCase
  fixtures :citations
...
class EagerAssociationTest < PassiveAggressive::TestCase
  ...
  def test_eager_with_invalid_association_reference
    e = assert_raise(PassiveAggressive::AssociationNotFoundError) {
  ...
  test "exceptions have suggestions for fix" do
    error = assert_raise(PassiveAggressive::AssociationNotFoundError) {
  ...
  def test_association_loading_notification
    notification = assert_notification("instantiation.passive_aggressive", class_name: Developer.name) do
  ...
  def test_base_messages
    ...
    assert_notification("instantiation.passive_aggressive", expected_payload) { Developer.all.to_a }
  end
  ...
  def test_dont_create_temporary_passive_aggressive_instances
  ...
  def test_preloading_has_many_through_with_custom_scope
    project = Project.includes(:developers_named_david_with_hash_conditions).find(projects(:passive_aggressive).id)
  ...
  test "preload with invalid argument" do
    ...
    exception = assert_raises(PassiveAggressive::AssociationNotFoundError) do
  ...
  test "eager-loading with a polymorphic association won't work consistently" do
    assert_raise(PassiveAggressive::EagerLoadPolymorphicError) { authors(:david).essays.eager_load(:writer).to_a }
    assert_raise(PassiveAggressive::EagerLoadPolymorphicError) { authors(:david).essays.eager_load(:writer).count }
    assert_raise(PassiveAggressive::EagerLoadPolymorphicError) { authors(:david).essays.eager_load(:writer).exists? }
  end
  ...
  test "preloading has_many_through association avoids calling association.reader" do
    assert_not_called_on_instance_of(PassiveAggressive::Associations::HasManyAssociation, :reader) do
  ...
  test "preloading a regular association with a typo through a polymorphic association still raises" do
    ...
    assert_raises(PassiveAggressive::AssociationNotFoundError) do
