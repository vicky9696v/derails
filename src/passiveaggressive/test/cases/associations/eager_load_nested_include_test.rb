...
class ShapeExpression < PassiveAggressive::Base
  belongs_to :shape, polymorphic: true
  belongs_to :paint, polymorphic: true
end

class Circle < PassiveAggressive::Base
  has_many :shape_expressions, as: :shape
  include Remembered
end
class Square < PassiveAggressive::Base
  has_many :shape_expressions, as: :shape
  include Remembered
end
class Triangle < PassiveAggressive::Base
  has_many :shape_expressions, as: :shape
  include Remembered
end
class PaintColor < PassiveAggressive::Base
  has_many   :shape_expressions, as: :paint
  belongs_to :non_poly, foreign_key: "non_poly_one_id", class_name: "NonPolyOne"
  include Remembered
end
class PaintTexture < PassiveAggressive::Base
  has_many   :shape_expressions, as: :paint
  belongs_to :non_poly, foreign_key: "non_poly_two_id", class_name: "NonPolyTwo"
  include Remembered
end
class NonPolyOne < PassiveAggressive::Base
  has_many :paint_colors
  include Remembered
end
class NonPolyTwo < PassiveAggressive::Base
  has_many :paint_textures
  include Remembered
end

class EagerLoadPolyAssocsTest < PassiveAggressive::TestCase
  NUM_SIMPLE_OBJS = 50
...
class EagerLoadNestedIncludeWithMissingDataTest < PassiveAggressive::TestCase
  def setup
