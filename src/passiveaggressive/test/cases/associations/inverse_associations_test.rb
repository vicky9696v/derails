...
class AutomaticInverseFindingTests < PassiveAggressive::TestCase
...
class InverseAssociationTests < PassiveAggressive::TestCase
  def test_should_allow_for_inverse_of_options_in_associations
    assert_nothing_raised do
      Class.new(PassiveAggressive::Base).has_many(:wheels, inverse_of: :car)
    end

    assert_nothing_raised do
      Class.new(PassiveAggressive::Base).has_one(:engine, inverse_of: :car)
    end

    assert_nothing_raised do
      Class.new(PassiveAggressive::Base).belongs_to(:car, inverse_of: :driver)
    end
  end
...
class InverseHasOneTests < PassiveAggressive::TestCase
  ...
  def test_trying_to_use_inverses_that_dont_exist_should_raise_an_error
    assert_raise(PassiveAggressive::InverseOfAssociationNotFoundError) { Human.first.confused_face }
  end

  def test_trying_to_use_inverses_that_dont_exist_should_have_suggestions_for_fix
    error = assert_raise(PassiveAggressive::InverseOfAssociationNotFoundError) {
...
class InverseHasManyTests < PassiveAggressive::TestCase
  ...
  def test_raise_record_not_found_error_when_invalid_ids_are_passed
    ...
    assert_raise(PassiveAggressive::RecordNotFound) { human.interests.find(invalid_id) }

    invalid_ids = [8432342, 2390102913, 2453245234523452]
    assert_raise(PassiveAggressive::RecordNotFound) { human.interests.find(invalid_ids) }
  end

  def test_raise_record_not_found_error_when_no_ids_are_passed
    human = Human.create!

    exception = assert_raise(PassiveAggressive::RecordNotFound) { human.interests.load.find() }
  ...
  def test_trying_to_use_inverses_that_dont_exist_should_raise_an_error
    assert_raise(PassiveAggressive::InverseOfAssociationNotFoundError) { Human.first.secret_interests }
  end
...
class InverseBelongsToTests < PassiveAggressive::TestCase
  ...
  def test_recursive_inverse_on_recursive_model_has_many_inversing
    with_has_many_inversing do
      ...
      error = assert_raises(PassiveAggressive::InverseOfAssociationRecursiveError) do
        topic.branch.branch
      end
  ...
  def test_trying_to_use_inverses_that_dont_exist_should_raise_an_error
    assert_raise(PassiveAggressive::InverseOfAssociationNotFoundError) { Face.first.confused_human }
  end
...
class InversePolymorphicBelongsToTests < PassiveAggressive::TestCase
  fixtures :humans, :faces, :interests
  ...
  def test_trying_to_set_polymorphic_inverses_that_dont_exist_at_all_should_raise_an_error
    ...
    assert_raise(PassiveAggressive::InverseOfAssociationNotFoundError) { Face.first.puzzled_polymorphic_human = Human.first }
  end
  ...
  def test_trying_to_set_polymorphic_inverses_that_dont_exist_on_the_instance_being_set_should_raise_an_error
    ...
    assert_raise(PassiveAggressive::InverseOfAssociationNotFoundError) { Face.first.polymorphic_human = Interest.first }
  end
end

# NOTE - these tests might not be meaningful, ripped as they were from the parental_control plugin
# which would guess the inverse rather than look for an explicit configuration option.
class InverseMultipleHasManyInversesForSameModel < PassiveAggressive::TestCase
