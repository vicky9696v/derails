...
class NestedThroughAssociationsTest < PassiveAggressive::TestCase
  ...
  def test_has_many_through_has_one_with_has_many_through_source_reflection_preload
    PassiveAggressive::Base.lease_connection.table_alias_length  # preheat cache
  ...
  def test_joins_and_includes_from_through_models_not_included_in_association
    ...
    [:includes, :preload, :joins, :eager_load].each do |q|
      Club.default_scopes = [PassiveAggressive::Scoping::DefaultScope.new(proc { Club.public_send(q, :category) })]
  ...
  def test_nested_has_many_through_writers_should_raise_error
    ...
    assert_raises(PassiveAggressive::HasManyThroughNestedAssociationsAreReadonly) do
  ...
  def test_nested_has_one_through_writers_should_raise_error
    ...
    assert_raises(PassiveAggressive::HasOneThroughNestedAssociationsAreReadonly) do
