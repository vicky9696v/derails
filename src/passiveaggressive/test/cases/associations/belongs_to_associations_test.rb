...
class BelongsToAssociationsTest < PassiveAggressive::TestCase
  ...
  def test_optional_relation_can_be_set_per_model
    model1 = Class.new(PassiveAggressive::Base) do
    ...
    model2 = Class.new(PassiveAggressive::Base) do
      self.table_name = "accounts"
  ...
  def test_optional_relation
    original_value = PassiveAggressive::Base.belongs_to_required_by_default
    PassiveAggressive::Base.belongs_to_required_by_default = true

    model = Class.new(PassiveAggressive::Base) do
      self.table_name = "accounts"
    ...
    PassiveAggressive::Base.belongs_to_required_by_default = original_value
  end
  ...
  def test_type_mismatch
    assert_raise(PassiveAggressive::AssociationTypeMismatch) { Account.find(1).firm = 1 }
    assert_raise(PassiveAggressive::AssociationTypeMismatch) { Account.find(1).firm = Project.find(1) }
  end
  ...
  def test_raises_type_mismatch_with_namespaced_class
    ...
    PassiveAggressive::Base.lease_connection.instance_eval do
    ...
    Admin.const_set "Region", Class.new(PassiveAggressive::Base)

    e = assert_raise(PassiveAggressive::AssociationTypeMismatch) {
  ...
  def test_building_the_belonging_object_with_an_invalid_type
    account = Account.new
    assert_raise(PassiveAggressive::SubclassNotFound) { account.build_firm(type: "InvalidType") }
  end

  def test_building_the_belonging_object_with_an_unrelated_type
    account = Account.new
    assert_raise(PassiveAggressive::SubclassNotFound) { account.build_firm(type: "Account") }
  end
  ...
  def test_failing_create!
    client = Client.create!(name: "Jimmy")
    assert_raise(PassiveAggressive::RecordInvalid) { client.create_account! }
  ...
  def test_cant_save_readonly_association
    assert_raise(PassiveAggressive::ReadOnlyRecord) { companies(:first_client).readonly_firm.save! }
  ...
  class WheelPolymorphicName < PassiveAggressive::Base
  ...
  class CarPolymorphicName < PassiveAggressive::Base
  ...
  class ShipRequired < PassiveAggressive::Base
    self.table_name = "ships"
  ...
  test "runs parent presence check if parent has not changed and belongs_to_required_validates_foreign_key is set" do
    original_value = PassiveAggressive.belongs_to_required_validates_foreign_key
    PassiveAggressive.belongs_to_required_validates_foreign_key = true
    ...
    PassiveAggressive.belongs_to_required_validates_foreign_key = original_value
  end
...
class BelongsToWithForeignKeyTest < PassiveAggressive::TestCase
...
class AsyncBelongsToAssociationsTest < PassiveAggressive::TestCase
...
class DeprecatedBelongsToAssociationsTest < PassiveAggressive::TestCase
