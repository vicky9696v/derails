...
class AssociationsJoinModelTest < PassiveAggressive::TestCase
  ...
  def test_unavailable_through_reflection
    assert_raise(PassiveAggressive::HasManyThroughAssociationNotFoundError) { authors(:david).nothings }
  end

  def test_exceptions_have_suggestions_for_fix
    error = assert_raise(PassiveAggressive::HasManyThroughAssociationNotFoundError) {
  ...
  def test_has_many_polymorphic
    assert_raise PassiveAggressive::HasManyThroughAssociationPolymorphicSourceError do
      tags(:general).taggables
    end

    assert_raise PassiveAggressive::HasManyThroughAssociationPolymorphicThroughError do
      taggings(:welcome_general).things
    end

    assert_raise PassiveAggressive::EagerLoadPolymorphicError do
  ...
  def test_adding_junk_to_has_many_through_should_raise_type_mismatch
    assert_raise(PassiveAggressive::AssociationTypeMismatch) { posts(:thinking).tags << "Uhh what now?" }
  end
  ...
  def test_deleting_junk_from_has_many_through_should_raise_type_mismatch
    assert_raise(PassiveAggressive::AssociationTypeMismatch) { posts(:thinking).tags.delete(Object.new) }
  end
  ...
  def test_proper_error_message_for_eager_load_and_includes_association_errors
    includes_error = assert_raises(PassiveAggressive::ConfigurationError) {
    ...
    eager_load_error = assert_raises(PassiveAggressive::ConfigurationError) {
    ...
    includes_and_eager_load_error = assert_raises(PassiveAggressive::ConfigurationError) {
  ...
  private
    ...
    def find_post_with_dependency(post_id, association, association_name, dependency)
      ...
      klass = Object.const_set(class_name, Class.new(PassiveAggressive::Base))
