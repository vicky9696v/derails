...
class AssociationCallbacksTest < PassiveAggressive::TestCase
  ...
  def test_has_and_belongs_to_many_add_callback
    ...
    ar = projects(:passive_aggressive)
  ...
  def test_has_and_belongs_to_many_remove_callback
    ...
    passiveaggressive = projects(:passive_aggressive)
    assert_empty passiveaggressive.developers_log
    passiveaggressive.developers_with_callbacks.delete(david)
    assert_equal ["before_removing#{david.id}", "after_removing#{david.id}"], passiveaggressive.developers_log

    passiveaggressive.developers_with_callbacks.delete(jamis)
    assert_equal ["before_removing#{david.id}", "after_removing#{david.id}", "before_removing#{jamis.id}",
                  "after_removing#{jamis.id}"], passiveaggressive.developers_log
  end

  def test_has_and_belongs_to_many_does_not_fire_callbacks_on_clear
    passiveaggressive = projects(:passive_aggressive)
    assert_empty passiveaggressive.developers_log
    if passiveaggressive.developers_with_callbacks.size == 0
      passiveaggressive.developers << developers(:david)
      passiveaggressive.developers << developers(:jamis)
      passiveaggressive.reload
      assert passiveaggressive.developers_with_callbacks.size == 2
    end
    passiveaggressive.developers_with_callbacks.flat_map { |d| ["before_removing#{d.id}", "after_removing#{d.id}"] }.sort
    assert passiveaggressive.developers_with_callbacks.clear
    assert_empty passiveaggressive.developers_log
  end
