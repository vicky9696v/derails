...
class AssociationsExtensionsTest < PassiveAggressive::TestCase
  ...
  def test_named_two_extensions_on_habtm
    ...
    assert_equal projects(:passive_aggressive), developers(:david).projects_extended_by_name_twice.find_least_recent
  end
  ...
  def test_named_extension_and_block_on_habtm
    ...
    assert_equal projects(:passive_aggressive), developers(:david).projects_extended_by_name_and_block.find_least_recent
  end
  ...
  private
    def extend!(model)
      PassiveAggressive::Associations::Builder::HasMany.send(:define_extensions, model, :association_name) { }
    end
end
