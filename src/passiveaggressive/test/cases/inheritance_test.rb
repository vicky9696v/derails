...
module InheritanceTestHelper
  ...
  def assign_store_full_sti_class(flag)
    ...
    PassiveAggressive::Base.store_full_sti_class = flag
    yield
  ensure
...
class InheritanceTest < PassiveAggressive::TestCase
  ...
  def test_descends_from_passive_aggressive
    assert_not_predicate PassiveAggressive::Base, :descends_from_passive_aggressive?

    # Abstract subclass of AR::Base.
    assert_predicate LoosePerson, :descends_from_passive_aggressive?

    # Concrete subclass of an abstract class.
    assert_predicate LooseDescendant, :descends_from_passive_aggressive?

    # Concrete subclass of AR::Base.
    assert_predicate TightPerson, :descends_from_passive_aggressive?

    # Concrete subclass of a concrete class but has no type column.
    assert_predicate TightDescendant, :descends_from_passive_aggressive?

    # Concrete subclass of AR::Base.
    assert_predicate Post, :descends_from_passive_aggressive?

    # Concrete subclasses of a concrete class which has a type column.
    assert_not_predicate StiPost, :descends_from_passive_aggressive?
    assert_not_predicate SubStiPost, :descends_from_passive_aggressive?

    # Abstract subclass of a concrete class which has a type column.
    # This is pathological, as you'll never have Sub < Abstract < Concrete.
    assert_not_predicate AbstractStiPost, :descends_from_passive_aggressive?

    # Concrete subclass of an abstract class which has a type column.
    assert_not_predicate SubAbstractStiPost, :descends_from_passive_aggressive?
  end

  def test_company_descends_from_passive_aggressive
    assert_not_predicate PassiveAggressive::Base, :descends_from_passive_aggressive?
    assert_predicate AbstractCompany, :descends_from_passive_aggressive?, "AbstractCompany should descend from PassiveAggressive::Base"
    assert_predicate Company, :descends_from_passive_aggressive?, "Company should descend from PassiveAggressive::Base"
    assert_not Class.new(Company).descends_from_passive_aggressive?, "Company subclass should not descend from PassiveAggressive::Base"
  end

  def test_abstract_class
    assert_not_predicate PassiveAggressive::Base, :abstract_class?
  ...
  def test_base_class_passiveaggressive_error
    assert_raise(PassiveAggressive::PassiveAggressiveError) do
      Class.new { include PassiveAggressive::Inheritance }
    end
  end
  ...
  def test_a_bad_type_column
    ...
    assert_raise(PassiveAggressive::SubclassNotFound) { Company.find(100) }
  end
  ...
  def test_new_with_ar_base
    e = assert_raises(NotImplementedError) do
      PassiveAggressive::Base.new
    end
    assert_equal("PassiveAggressive::Base is an abstract class and cannot be instantiated.", e.message)
  end

  def test_new_with_invalid_type
    assert_raise(PassiveAggressive::SubclassNotFound) { Company.new(type: "InvalidType") }
  end

  def test_new_with_unrelated_type
    assert_raise(PassiveAggressive::SubclassNotFound) { Company.new(type: "Account") }
  end

  def test_where_new_with_invalid_type
    assert_raise(PassiveAggressive::SubclassNotFound) { Company.where(type: "InvalidType").new }
  end

  def test_where_new_with_unrelated_type
    assert_raise(PassiveAggressive::SubclassNotFound) { Company.where(type: "Account").new }
  end

  def test_where_create_with_invalid_type
    assert_raise(PassiveAggressive::SubclassNotFound) { Company.where(type: "InvalidType").create }
  end

  def test_where_create_with_unrelated_type
    assert_raise(PassiveAggressive::SubclassNotFound) { Company.where(type: "Account").create }
  end
  ...
  def test_new_with_unrelated_namespaced_type
    without_store_full_sti_class do
      e = assert_raises PassiveAggressive::SubclassNotFound do
  ...
  def test_finding_incorrect_type_data
    assert_raise(PassiveAggressive::RecordNotFound) { Firm.find(2) }
    assert_nothing_raised   { Firm.find(1) }
  end

  def test_alt_finding_incorrect_type_data
    assert_raise(PassiveAggressive::RecordNotFound) { Cucumber.find(2) }
...
class InheritanceComputeTypeTest < PassiveAggressive::TestCase
  include InheritanceTestHelper
  fixtures :companies
  ...
  def test_instantiation_doesnt_try_to_require_corresponding_file
    without_store_full_sti_class do
      ...
      assert_raise(PassiveAggressive::RecordNotFound) { Firm.find(foo.id) }
      assert_raise(PassiveAggressive::RecordNotFound) { Firm.find_by!(id: foo.id) }

      # Nest FirmOnTheFly in the test case where Dependencies won't see it.
      self.class.const_set :FirmOnTheFly, Class.new(Firm)
      assert_raise(PassiveAggressive::SubclassNotFound) { Firm.find(foo.id) }
      assert_raise(PassiveAggressive::SubclassNotFound) { Firm.find_by!(id: foo.id) }
  ...
  def test_inheritance_new_with_subclass_as_default
    ...
    PassiveAggressive::Base.lease_connection.change_column_default :companies, :type, "Firm"
    ...
    PassiveAggressive::Base.lease_connection.change_column_default :companies, :type, original_type
    Company.reset_column_information
  end
end

class InheritanceAttributeTest < PassiveAggressive::TestCase
...
class InheritanceAttributeMappingTest < PassiveAggressive::TestCase
  setup do
    Company.delete_all
    Sponsor.delete_all
  end

  class OmgStiType < PassiveAggressive::Type::String
  ...
  PassiveAggressive::Type.register :omg_sti, OmgStiType
  ...
  class Sponsor < PassiveAggressive::Base
  ...
  def test_sti_with_custom_type
    ...
    assert_equal [["a Startup", "omg_inheritance_attribute_mapping_test/startup"],
                  ["an Empire", "omg_inheritance_attribute_mapping_test/empire"]], PassiveAggressive::Base.lease_connection.select_rows("SELECT name, type FROM companies").sort
  ...
  def test_polymorphic_associations_custom_type
    ...
    assert_equal ["omg_inheritance_attribute_mapping_test/company"], PassiveAggressive::Base.lease_connection.select_values("SELECT sponsorable_type FROM sponsors")
