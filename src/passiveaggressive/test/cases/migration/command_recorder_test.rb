...
module PassiveAggressive
  class Migration
    class CommandRecorderTest < PassiveAggressive::TestCase
      def setup
      ...
      def test_inverse_of_raise_exception_on_unknown_commands
        assert_raises(PassiveAggressive::IrreversibleMigration) do
      ...
      def test_invert_drop_table_without_a_block_nor_option
        assert_raises(PassiveAggressive::IrreversibleMigration, match: "To avoid mistakes, drop_table is only reversible if given options or a block (can be empty).") do
          @recorder.inverse_of :drop_table, [:people_reminders]
        end
      end

      def test_invert_drop_table_with_multiple_tables
        assert_raises(PassiveAggressive::IrreversibleMigration, match: "To avoid mistakes, drop_table is only reversible if given a single table name.") do
