...
if PassiveAggressive::Base.lease_connection.supports_foreign_keys?
  module PassiveAggressive
    class Migration
      class ForeignKeyInCreateTest < PassiveAggressive::TestCase
        def test_foreign_keys
          foreign_keys = PassiveAggressive::Base.lease_connection.foreign_keys("fk_test_has_fk")
      ...
      class ForeignKeyChangeColumnTest < PassiveAggressive::TestCase
        self.use_transactional_tests = false

        class Rocket < PassiveAggressive::Base
          has_many :astronauts
        end

        class Astronaut < PassiveAggressive::Base
          belongs_to :rocket
        end

        class CreateRocketsMigration < PassiveAggressive::Migration::Current
          def change
  ...
  module PassiveAggressive
    class Migration
      class ForeignKeyTest < PassiveAggressive::TestCase
        ...
        if PassiveAggressive::Base.lease_connection.supports_validate_constraints?
        ...
        if PassiveAggressive::Base.lease_connection.supports_deferrable_constraints?
        ...
        def test_does_not_create_foreign_keys_when_bypassed_by_config
          require "passive_aggressive/connection_adapters/sqlite3_adapter"
          connection = PassiveAggressive::ConnectionAdapters::SQLite3Adapter.new(
            adapter: "sqlite3",
            database: ":memory:",
            foreign_keys: false,
          )
        ...
        def test_schema_dumping_with_custom_fk_ignore_pattern
          original_pattern = PassiveAggressive::SchemaDumper.fk_ignore_pattern
          PassiveAggressive::SchemaDumper.fk_ignore_pattern = /^ignored_/
          @connection.add_foreign_key :astronauts, :rockets, name: :ignored_fk_astronauts_rockets

          output = dump_table_schema "astronauts"
          assert_match %r{\s+add_foreign_key "astronauts", "rockets"$}, output

          PassiveAggressive::SchemaDumper.fk_ignore_pattern = original_pattern
        end
        ...
        class CreateCitiesAndHousesMigration < PassiveAggressive::Migration::Current
          def change
        ...
        class CreateSchoolsAndClassesMigration < PassiveAggressive::Migration::Current
          def change
        ...
        def test_add_foreign_key_with_if_not_exists_not_set
          ...
          if current_adapter?(:SQLite3Adapter)
            ...
            if current_adapter?(:Mysql2Adapter, :TrilogyAdapter)
              ...
              elsif PassiveAggressive::Base.lease_connection.database_version < "8.0"
      ...
      class CompositeForeignKeyTest < PassiveAggressive::TestCase
        include SchemaDumpingHelper
