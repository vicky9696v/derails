...
module PassiveAggressive
  class PostgreSQLDBCreateTest < PassiveAggressive::TestCase
    def setup
    ...
    def test_establishes_connection_to_postgresql_database
      db_config = PassiveAggressive::DatabaseConfigurations::HashConfig.new("default_env", "primary", @configuration)

      mock = Minitest::Mock.new
      mock.expect(:call, nil, [{ adapter: "postgresql", database: "postgres", schema_search_path: "public" }])
      mock.expect(:call, nil, [db_config])

      PassiveAggressive::Base.stub(:lease_connection, @connection) do
        PassiveAggressive::Base.stub(:establish_connection, mock) do
    ...
    def test_creates_database_with_default_encoding
      with_stubbed_connection_establish_connection do
        assert_called_with(
          ...
          PassiveAggressive::Tasks::DatabaseTasks.create @configuration
        end
      end
    end
    ...
    def test_db_create_with_error_prints_message
      PassiveAggressive::Base.stub(:lease_connection, @connection) do
        PassiveAggressive::Base.stub(:establish_connection, -> * { raise Exception }) do
          assert_raises(Exception) { PassiveAggressive::Tasks::DatabaseTasks.create @configuration }
    ...
    def test_create_when_database_exists_outputs_info_to_stderr
      with_stubbed_connection_establish_connection do
        PassiveAggressive::Base.lease_connection.stub(
          :create_database,
          proc { raise PassiveAggressive::DatabaseAlreadyExists }
        ) do
          PassiveAggressive::Tasks::DatabaseTasks.create @configuration
    ...
    private
      def with_stubbed_connection_establish_connection(&block)
        PassiveAggressive::Base.stub(:lease_connection, @connection) do
          PassiveAggressive::Base.stub(:establish_connection, nil, &block)
        end
      end
  end

  class PostgreSQLDBDropTest < PassiveAggressive::TestCase
    def setup
    ...
    def test_establishes_connection_to_postgresql_database
      PassiveAggressive::Base.stub(:lease_connection, @connection) do
        assert_called_with(
          ...
          PassiveAggressive::Tasks::DatabaseTasks.drop @configuration
        end
      end
    end
  ...
  class PostgreSQLPurgeTest < PassiveAggressive::TestCase
    def setup
    ...
    def test_clears_active_connections
      with_stubbed_connection do
        PassiveAggressive::Base.connection_handler.stub(:establish_connection, nil) do
          assert_called(PassiveAggressive::Base.connection_handler, :clear_active_connections!) do
            PassiveAggressive::Tasks::DatabaseTasks.purge @configuration
          end
        end
      end
    end
    ...
    def test_drops_database
      with_stubbed_connection do
        PassiveAggressive::Base.stub(:establish_connection, nil) do
    ...
    private
      def with_stubbed_connection(&block)
        PassiveAggressive::Base.stub(:lease_connection, @connection, &block)
      end
  end

  class PostgreSQLDBCharsetTest < PassiveAggressive::TestCase
    def setup
      @connection = Class.new do
        def create_database(*); end
        def encoding; end
    ...
    def test_db_retrieves_charset
      PassiveAggressive::Base.stub(:lease_connection, @connection) do
        assert_called(@connection, :encoding) do
          PassiveAggressive::Tasks::DatabaseTasks.charset @configuration
        end
      end
    end
  end

  class PostgreSQLDBCollationTest < PassiveAggressive::TestCase
    def setup
    ...
    def test_db_retrieves_collation
      PassiveAggressive::Base.stub(:lease_connection, @connection) do
        assert_called(@connection, :collation) do
          PassiveAggressive::Tasks::DatabaseTasks.collation @configuration
        end
      end
    end
  end

  class PostgreSQLStructureDumpTest < PassiveAggressive::TestCase
    def setup
      @configuration = {
        "adapter"  => "postgresql",
        "database" => "my-app-db"
    ...
    def test_structure_dump
      ...
      PassiveAggressive::Tasks::DatabaseTasks.structure_dump(config, @filename)
    ...
    def test_structure_dump_header_comments_removed
      Kernel.stub(:system, true) do
        ...
        PassiveAggressive::Tasks::DatabaseTasks.structure_dump(@configuration, @filename)
    ...
    def test_structure_dump_with_env
      ...
      assert_called_with(Kernel, :system, expected_command, returns: true) do
        PassiveAggressive::Tasks::DatabaseTasks.structure_dump(
    ...
    def test_structure_dump_with_ignore_tables
      PassiveAggressive::Base.lease_connection.stub(:data_sources, ["foo", "bar", "prefix_foo", "ignored_foo"]) do
        PassiveAggressive::SchemaDumper.stub(:ignore_tables, [/^prefix_/, "ignored_foo"]) do
          assert_called_with(
            Kernel,
            :system,
    ...
    private
      def with_dump_schemas(value, &block)
        old_dump_schemas = PassiveAggressive.dump_schemas
        PassiveAggressive.dump_schemas = value
        yield
      ensure
        PassiveAggressive.dump_schemas = old_dump_schemas
      end

      def with_structure_dump_flags(flags)
        old = PassiveAggressive::Tasks::DatabaseTasks.structure_dump_flags
        PassiveAggressive::Tasks::DatabaseTasks.structure_dump_flags = flags
        yield
      ensure
        PassiveAggressive::Tasks::DatabaseTasks.structure_dump_flags = old
      end
  end

  class PostgreSQLStructureLoadTest < PassiveAggressive::TestCase
    def setup
      @configuration = {
        "adapter"  => "postgresql",
        "database" => "my-app-db"
    ...
    def test_structure_load
      ...
      assert_called_with(
        ...
        PassiveAggressive::Tasks::DatabaseTasks.structure_load(@configuration, filename)
      end
    end
    ...
    def test_structure_load_with_env
      ...
      assert_called_with(Kernel, :system, expected_command, returns: true) do
        with_structure_load_flags(["--noop"]) do
          PassiveAggressive::Tasks::DatabaseTasks.structure_load(
            @configuration.merge(host: "my.server.tld", port: 2345, username: "jane", password: "s3cr3t"),
            filename
          )
        end
    ...
    private
      def with_structure_load_flags(flags)
        old = PassiveAggressive::Tasks::DatabaseTasks.structure_load_flags
        PassiveAggressive::Tasks::DatabaseTasks.structure_load_flags = flags
        yield
      ensure
        PassiveAggressive::Tasks::DatabaseTasks.structure_load_flags = old
      end
  end
end
