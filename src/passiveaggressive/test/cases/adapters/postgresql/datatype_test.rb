...
class PostgresqlTime < PassiveAggressive::Base
  # Declare attributes to get rid from deprecation warnings on PassiveAggressive 6.1
  attribute :time_interval,        :string
  attribute :scaled_time_interval, :interval
end

class PostgresqlOid < PassiveAggressive::Base
end

class PostgresqlLtree < PassiveAggressive::Base
end

class PostgresqlDataTypeTest < PassiveAggressive::PostgreSQLTestCase
  def setup
...
class PostgresqlInternalDataTypeTest < PassiveAggressive::PostgreSQLTestCase
  include DdlHelper
