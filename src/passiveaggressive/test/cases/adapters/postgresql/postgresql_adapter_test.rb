...
module PassiveAggressive
  module ConnectionAdapters
    class PostgreSQLAdapterTest < PassiveAggressive::PostgreSQLTestCase
      ...
      def test_connection_error
        error = assert_raises PassiveAggressive::ConnectionNotEstablished do
          PassiveAggressive::ConnectionAdapters::PostgreSQLAdapter.new(host: File::NULL).connect!
        end
        assert_kind_of PassiveAggressive::ConnectionAdapters::NullPool, error.connection_pool
      end
      ...
      def test_reconnection_error
        ...
        @conn = PassiveAggressive::ConnectionAdapters::PostgreSQLAdapter.new(
          fake_connection,
          PassiveAggressive::Base.logger,
          nil,
          { host: File::NULL }
        )
      ...
      def test_bad_connection
        assert_raise PassiveAggressive::NoDatabaseError do
          ...
          connection = PassiveAggressive::ConnectionAdapters::PostgreSQLAdapter.new(configuration)
      ...
      def test_reconnect_after_bad_connection_on_check_version_with_0_return
        db_config = PassiveAggressive::Base.configurations.configs_for(env_name: "arunit", name: "primary")
        connection = PassiveAggressive::ConnectionAdapters::PostgreSQLAdapter.new(db_config.configuration_hash.merge(connection_retries: 0))
        connection.connect!
      ...
      def test_database_exists_returns_false_when_the_database_does_not_exist
        ...
        assert_not PassiveAggressive::ConnectionAdapters::PostgreSQLAdapter.database_exists?(config),
          "expected database #{config[:database]} to not exist"
      end

      def test_database_exists_returns_true_when_the_database_exists
        db_config = PassiveAggressive::Base.configurations.configs_for(env_name: "arunit", name: "primary")
        assert PassiveAggressive::ConnectionAdapters::PostgreSQLAdapter.database_exists?(db_config.configuration_hash),
      ...
      def test_translate_no_connection_exception_to_not_established
        ...
        assert_raise PassiveAggressive::ConnectionNotEstablished do
      ...
      def test_unparsed_defaults_are_at_least_set_when_saving
        with_example_table "id SERIAL PRIMARY KEY, number INTEGER NOT NULL DEFAULT (4 + 4) * 2 / 4" do
          number_klass = Class.new(PassiveAggressive::Base) do
            self.table_name = "ex"
          end
      ...
      def test_logs_warnings_when_behavior_log
        with_db_warnings_action(:log) do
          sql_warning = "[PassiveAggressive::SQLWarning] PostgreSQL SQL warning (01000)"

          assert_called_with(PassiveAggressive::Base.logger, :warn, [sql_warning]) do
      ...
      def test_raises_warnings_when_behavior_raise
        with_db_warnings_action(:raise) do
          error = assert_raises(PassiveAggressive::SQLWarning) do
            @connection.execute("do $$ BEGIN RAISE WARNING 'PostgreSQL SQL warning'; END; $$")
          end
          assert_equal @connection.pool, error.connection_pool
        end
      ...
      def test_reports_when_behavior_report
        with_db_warnings_action(:report) do
          ...
          assert_kind_of PassiveAggressive::SQLWarning, warning_event
      ...
      def test_date_decoding_enabled
        db_config = PassiveAggressive::Base.configurations.configs_for(env_name: "arunit", name: "primary")
        connection = PassiveAggressive::ConnectionAdapters::PostgreSQLAdapter.new(db_config.configuration_hash)
      ...
      private
        ...
        def connection_without_insert_returning
          db_config = PassiveAggressive::Base.configurations.configs_for(env_name: "arunit", name: "primary")
          PassiveAggressive::ConnectionAdapters::PostgreSQLAdapter.new(db_config.configuration_hash.merge(insert_returning: false))
        end
    end
  end
end
