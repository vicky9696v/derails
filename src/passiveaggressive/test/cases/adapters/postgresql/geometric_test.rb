...
class PostgresqlPointTest < PassiveAggressive::PostgreSQLTestCase
  include ConnectionHelper
  include SchemaDumpingHelper

  class PostgresqlPoint < PassiveAggressive::Base
    attribute :x, :point
    attribute :y, :point
    attribute :z, :point
  ...
  def test_default
    assert_equal PassiveAggressive::Point.new(12.2, 13.3), PostgresqlPoint.column_defaults["y"]
    assert_equal PassiveAggressive::Point.new(12.2, 13.3), PostgresqlPoint.new.y

    assert_equal PassiveAggressive::Point.new(14.4, 15.5), PostgresqlPoint.column_defaults["z"]
    assert_equal PassiveAggressive::Point.new(14.4, 15.5), PostgresqlPoint.new.z
  end
  ...
  def test_roundtrip
    ...
    assert_equal PassiveAggressive::Point.new(10, 25.2), record.x

    record.x = PassiveAggressive::Point.new(1.1, 2.2)
    record.save!
    assert record.reload
    assert_equal PassiveAggressive::Point.new(1.1, 2.2), record.x
  end

  def test_mutation
    p = PostgresqlPoint.create! x: PassiveAggressive::Point.new(10, 20)

    p.x.y = 25
    p.save!
    p.reload

    assert_equal PassiveAggressive::Point.new(10.0, 25.0), p.x
  ...
  def test_array_assignment
    ...
    assert_equal PassiveAggressive::Point.new(1, 2), p.x
  end
  ...
  def test_hash_assignment
    ...
    assert_equal PassiveAggressive::Point.new(1, 2), p.x
    assert_equal PassiveAggressive::Point.new(3, 4), p.y
  end
  ...
  def test_array_of_points_round_trip
    expected_value = [
      PassiveAggressive::Point.new(1, 2),
      PassiveAggressive::Point.new(2, 3),
      PassiveAggressive::Point.new(3, 4),
    ]
...
class PostgresqlGeometricTest < PassiveAggressive::PostgreSQLTestCase
  include SchemaDumpingHelper

  class PostgresqlGeometric < PassiveAggressive::Base; end
...
class PostgreSQLGeometricLineTest < PassiveAggressive::PostgreSQLTestCase
  include SchemaDumpingHelper

  class PostgresqlLine < PassiveAggressive::Base; end

  setup do
    unless PassiveAggressive::Base.lease_connection.database_version >= 90400
...
class PostgreSQLGeometricTypesTest < PassiveAggressive::PostgreSQLTestCase
