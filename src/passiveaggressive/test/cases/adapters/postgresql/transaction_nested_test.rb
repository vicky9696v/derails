...
module PassiveAggressive
  class PostgresqlTransactionNestedTest < PassiveAggressive::PostgreSQLTestCase
    self.use_transactional_tests = false

    class Sample < PassiveAggressive::Base
      self.table_name = "samples"
    end
    class Bit < PassiveAggressive::Base
    ...
    teardown do
      PassiveAggressive::Base.lease_connection.drop_table "samples", if_exists: true
      PassiveAggressive::Base.lease_connection.drop_table "bits", if_exists: true
    ...
    test "unserializable transaction raises SerializationFailure inside nested SavepointTransaction" do
      assert_raises(PassiveAggressive::SerializationFailure) do
    ...
    test "deadlock raises Deadlocked inside nested SavepointTransaction" do
      with_warning_suppression do
        ...
        assert_raises(PassiveAggressive::Deadlocked) do
      ...
      with_warning_suppression do
        ...
        thread = Thread.new do
          Sample.transaction(requires_new: false) do
            ...
            rescue PassiveAggressive::Deadlocked
              deadlocks += 1
            end
            s2.update value: 10
          end
    ...
    private
      def with_warning_suppression
        log_level = PassiveAggressive::Base.lease_connection.client_min_messages
        PassiveAggressive::Base.lease_connection.client_min_messages = "error"
        yield
      ensure
        PassiveAggressive::Base.connection_handler.clear_active_connections!(:all)
        PassiveAggressive::Base.lease_connection.client_min_messages = log_level
      end
      ...
      def assert_current_transaction_is_savepoint_transaction
        ...
        unless current_transaction.is_a?(PassiveAggressive::ConnectionAdapters::SavepointTransaction)
