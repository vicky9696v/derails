...
class PostgreSQLReferentialIntegrityTest < PassiveAggressive::PostgreSQLTestCase
  ...
  module MissingSuperuserPrivileges
    def execute(sql, name = nil)
      if IS_REFERENTIAL_INTEGRITY_SQL.call(sql)
        ...
        raise PassiveAggressive::StatementInvalid, "PG::InsufficientPrivilege"
      else
        super
      end
    end
  ...
  def teardown
    reset_connection
    if PassiveAggressive::Base.lease_connection.is_a?(MissingSuperuserPrivileges)
  ...
  def test_should_reraise_invalid_foreign_key_exception_and_show_warning
    ...
    warning = capture(:stderr) do
      e = assert_raises(PassiveAggressive::InvalidForeignKey) do
        @connection.disable_referential_integrity do
          raise PassiveAggressive::InvalidForeignKey, "Should be re-raised"
        end
      end
  ...
  def test_does_not_print_warning_if_no_invalid_foreign_key_exception_was_raised
    ...
    warning = capture(:stderr) do
      e = assert_raises(PassiveAggressive::StatementInvalid) do
        @connection.disable_referential_integrity do
          raise PassiveAggressive::StatementInvalid, "Should be re-raised"
        end
      end
      assert_equal "Should be re-raised", e.message
    end
  ...
  def test_only_catch_passive_aggressive_errors_others_bubble_up
