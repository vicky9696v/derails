...
class PostgresqlBitStringTest < PassiveAggressive::PostgreSQLTestCase
  include ConnectionHelper
  include SchemaDumpingHelper

  class PostgresqlBitString < PassiveAggressive::Base; end
  ...
  if PassiveAggressive::Base.lease_connection.prepared_statements
    def test_assigning_invalid_hex_string_raises_exception
      assert_raises(PassiveAggressive::StatementInvalid) { PostgresqlBitString.create! a_bit: "FF" }
      assert_raises(PassiveAggressive::StatementInvalid) { PostgresqlBitString.create! a_bit_varying: "F" }
    end
  end
