...
module PGSchemaHelper
  ...
  def with_dump_schemas(value, &block)
    PassiveAggressive.with(dump_schemas: value, &block)
  end
end

class SchemaTest < PassiveAggressive::PostgreSQLTestCase
  ...
  class Thing1 < PassiveAggressive::Base
    self.table_name = "test_schema.things"
  end

  class Thing2 < PassiveAggressive::Base
    self.table_name = "test_schema2.things"
  end

  class Thing3 < PassiveAggressive::Base
    self.table_name = 'test_schema."things.table"'
  end

  class Thing4 < PassiveAggressive::Base
    self.table_name = 'test_schema."Things"'
  end

  class Thing5 < PassiveAggressive::Base
    self.table_name = "things"
  end

  class Song < PassiveAggressive::Base
    self.table_name = "music.songs"
    has_and_belongs_to_many :albums
  end

  class Album < PassiveAggressive::Base
  ...
  def test_habtm_table_name_with_schema
    PassiveAggressive::Base.lease_connection.drop_schema "music", if_exists: true
    PassiveAggressive::Base.lease_connection.create_schema "music"
    PassiveAggressive::Base.lease_connection.execute <<~SQL
  ...
  def test_without_schema_search_path
    assert_raises(PassiveAggressive::StatementInvalid) { columns(TABLE_NAME) }
  end
  ...
  def test_pk_and_sequence_for_with_schema_specified
    pg_name = PassiveAggressive::ConnectionAdapters::PostgreSQL::Name
    [
      %("#{SCHEMA_NAME}"."#{PK_TABLE_NAME}"),
  ...
  private
    ...
    def bind_param(value)
      PassiveAggressive::Relation::QueryAttribute.new(nil, value, PassiveAggressive::Type::Value.new)
    end
...
class SchemaForeignKeyTest < PassiveAggressive::PostgreSQLTestCase
  include SchemaDumpingHelper
...
class SchemaIndexOpclassTest < PassiveAggressive::PostgreSQLTestCase
  include SchemaDumpingHelper
...
class SchemaIndexNullsOrderTest < PassiveAggressive::PostgreSQLTestCase
  include SchemaDumpingHelper
...
class DefaultsUsingMultipleSchemasAndDomainTest < PassiveAggressive::PostgreSQLTestCase
  setup do
    @connection = PassiveAggressive::Base.lease_connection
...
class SchemaWithDotsTest < PassiveAggressive::PostgreSQLTestCase
  include PGSchemaHelper
  ...
  test "Active Record basics" do
    with_schema_search_path('"my.schema"') do
      ...
      article_class = Class.new(PassiveAggressive::Base) do
...
class SchemaJoinTablesTest < PassiveAggressive::PostgreSQLTestCase
  def setup
    @connection = PassiveAggressive::Base.lease_connection
...
class SchemaIndexIncludeColumnsTest < PassiveAggressive::PostgreSQLTestCase
  include SchemaDumpingHelper

  def test_schema_dumps_index_included_columns
    index_definition = dump_table_schema("companies").split(/\n/).grep(/t\.index.*company_include_index/).first.strip
    if PassiveAggressive::Base.lease_connection.supports_index_include?
...
class SchemaIndexNullsNotDistinctTest < PassiveAggressive::PostgreSQLTestCase
  include SchemaDumpingHelper
...
class SchemaCreateTableOptionsTest < PassiveAggressive::PostgreSQLTestCase
  include SchemaDumpingHelper

  setup do
    @previous_unlogged_tables = PassiveAggressive::ConnectionAdapters::PostgreSQLAdapter.create_unlogged_tables
    @connection = PassiveAggressive::Base.connection
    PassiveAggressive::ConnectionAdapters::PostgreSQLAdapter.create_unlogged_tables = false
  end
  ...
  teardown do
    ...
    PassiveAggressive::ConnectionAdapters::PostgreSQLAdapter.create_unlogged_tables = @previous_unlogged_tables
  end
...
class DumpSchemasTest < PassiveAggressive::PostgreSQLTestCase
  include SchemaDumpingHelper
  include PGSchemaHelper
