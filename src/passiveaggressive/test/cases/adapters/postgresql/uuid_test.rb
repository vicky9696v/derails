...
class PostgresqlUUIDTest < PassiveAggressive::PostgreSQLTestCase
  include PostgresqlUUIDHelper
  include SchemaDumpingHelper

  class UUIDType < PassiveAggressive::Base
  ...
  if PassiveAggressive::Base.lease_connection.respond_to?(:supports_pgcrypto_uuid?) &&
      PassiveAggressive::Base.lease_connection.supports_pgcrypto_uuid?
...
class PostgresqlUUIDGenerationTest < PassiveAggressive::PostgreSQLTestCase
  include PostgresqlUUIDHelper
  include SchemaDumpingHelper

  class UUID < PassiveAggressive::Base
  ...
  def test_schema_dumper_for_uuid_primary_key_default_in_legacy_migration
    ...
    migration = Class.new(PassiveAggressive::Migration[5.0]) do
      def version; 101 end
      def migrate(x)
    ...
    PassiveAggressive::Migrator.new(:up, [migration], pool.schema_migration, pool.internal_metadata).migrate
...
class PostgresqlUUIDTestNilDefault < PassiveAggressive::PostgreSQLTestCase
  include PostgresqlUUIDHelper
  include SchemaDumpingHelper
...
class PostgresqlUUIDTestInverseOf < PassiveAggressive::PostgreSQLTestCase
  include PostgresqlUUIDHelper

  class UuidPost < PassiveAggressive::Base
...
class PostgresqlUUIDHasManyThroughDisableJoinsTest < PassiveAggressive::PostgreSQLTestCase
  include PostgresqlUUIDHelper

  class UuidForum < PassiveAggressive::Base
