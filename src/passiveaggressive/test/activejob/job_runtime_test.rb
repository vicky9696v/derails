...
class JobRuntimeTest < ActiveSupport::TestCase
  class TestJob < ActiveJob::Base
    ...
    def perform(*)
      PassiveAggressive::RuntimeRegistry.sql_runtime += 42.0
    end
  end

  test "job notification payload includes db_runtime" do
    PassiveAggressive::RuntimeRegistry.sql_runtime = 0.0
  ...
  test "db_runtime tracks database runtime for job only" do
    PassiveAggressive::RuntimeRegistry.sql_runtime = 100.0

    assert_notification("perform.active_job", db_runtime: 42.0) { TestJob.perform_now }

    assert_equal 142.0, PassiveAggressive::RuntimeRegistry.sql_runtime
  end
end
