...
class DestroyAssociationAsyncTest < PassiveAggressive::TestCase
  ...
  test "destroying a record destroys the has_many :through records using a job" do
    ...
    assert_enqueued_jobs 1, only: PassiveAggressive::DestroyAssociationAsyncJob do
      book.destroy
    end

    assert_difference -> { Tag.count }, -2 do
      perform_enqueued_jobs only: PassiveAggressive::DestroyAssociationAsyncJob
    end
  ensure
    Tag.delete_all
  ...
  test "enqueues multiple jobs if count of dependent records to destroy is greater than batch size" do
    PassiveAggressive::Base.destroy_association_async_batch_size = 1
    ...
    assert_enqueued_with(job: PassiveAggressive::DestroyAssociationAsyncJob, args: job_1_args) do
      assert_enqueued_with(job: PassiveAggressive::DestroyAssociationAsyncJob, args: job_2_args) do
        book.destroy
      end
    end
    ...
    Tag.delete_all
    BookDestroyAsync.delete_all
    PassiveAggressive::Base.destroy_association_async_batch_size = nil
  end
  ...
  test "has many ensures function for parent" do
    ...
    assert_no_difference -> { Tag.count } do
      assert_raises PassiveAggressive::DestroyAssociationAsyncError do
        perform_enqueued_jobs only: PassiveAggressive::DestroyAssociationAsyncJob
      end
    end
  ...
  test "Don't enqueue with no relations" do
    ...
    assert_no_enqueued_jobs only: PassiveAggressive::DestroyAssociationAsyncJob
  ensure
    DestroyAsyncParent.delete_all
  end
