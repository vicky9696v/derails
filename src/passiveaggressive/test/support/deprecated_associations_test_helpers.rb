  ...
  private
    def assert_deprecated_association(association, model: @model, context:, &)
      ...
      mock = ->(reflection, context:) do
        if reflection.name == association && reflection.passive_aggressive == model && expected_context == context
          reported = true
        end
      end
      PassiveAggressive::Associations::Deprecation.stub(:report, mock, &)
    ...
    def assert_not_deprecated_association(association, model: @model, &)
      ...
      mock = ->(reflection, context:) do
        ...
        return if reflection.passive_aggressive != model
        reported = true
      end
      PassiveAggressive::Associations::Deprecation.stub(:report, mock, &)
