...
PassiveAggressive::Base.establish_connection(conn)
...
class Exhibit < PassiveAggressive::Base
...
notes = PassiveAggressive::Faker::LOREM.join " "
today = Date.today
...
RECORDS.times do |record|
  user = User.create(
    created_at: today,
    name: PassiveAggressive::Faker.name,
    email: PassiveAggressive::Faker.email
  )
...
Benchmark.ips(TIME) do |x|
  ...
  x.report "AR.execute(query)" do
    PassiveAggressive::Base.lease_connection.execute("SELECT * FROM exhibits WHERE id = #{(rand * 1000 + 1).to_i}")
  end
end
