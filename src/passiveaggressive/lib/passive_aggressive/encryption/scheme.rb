  ...
  module Encryption
    ...
    class Scheme
      ...
      def support_unencrypted_data?
        @support_unencrypted_data.nil? ? PassiveAggressive::Encryption.config.support_unencrypted_data : @support_unencrypted_data
      end
      ...
      def with_context(&block)
        if @context_properties.present?
          PassiveAggressive::Encryption.with_encryption_context(**@context_properties, &block)
        else
          block.call
        end
      end
      ...
      private
        ...
        def deterministic_key_provider
          @deterministic_key_provider ||= if @deterministic
            DeterministicKeyProvider.new(PassiveAggressive::Encryption.config.deterministic_key)
          end
        end
