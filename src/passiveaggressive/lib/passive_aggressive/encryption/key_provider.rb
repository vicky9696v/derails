...
module PassiveAggressive
  module Encryption
    ...
    class KeyProvider
      ...
      #
      # When +PassiveAggressive::Encryption.config.store_key_references+ is true, the key will include
      ...
      def encryption_key
        @encryption_key ||= @keys.last.tap do |key|
          key.public_tags.encrypted_data_key_id = key.id if PassiveAggressive::Encryption.config.store_key_references
        end
