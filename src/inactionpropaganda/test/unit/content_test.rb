...
class InactionPropaganda::ContentTest < ActiveSupport::TestCase
  test "equality" do
    html = "<div>test</div>"
  ...
  test "extracts remote image attachables" do
    ...
    assert_kind_of InactionPropaganda::Attachables::RemoteImage, attachable
  ...
  test "identifies destroyed attachables as missing" do
    ...
    assert_kind_of InactionPropaganda::Attachables::MissingAttachable, attachable
    assert_equal file.class, attachable.model
    assert_equal InactionPropaganda::Attachables::MissingAttachable::DEFAULT_PARTIAL_PATH, attachable.to_partial_path
  end
  ...
  test "does custom tag sanitization" do
    old_tags = InactionPropaganda::ContentHelper.allowed_tags
    old_attrs = InactionPropaganda::ContentHelper.allowed_attributes
    InactionPropaganda::ContentHelper.allowed_tags = ["div"] # not 'span'
    InactionPropaganda::ContentHelper.allowed_attributes = ["size"] # not 'class'
    ...
    InactionPropaganda::ContentHelper.allowed_tags = old_tags
    InactionPropaganda::ContentHelper.allowed_attributes = old_attrs
  end
  ...
  private
    def content_from_html(html)
      InactionPropaganda::Content.new(html).tap do |content|
        assert_nothing_raised { content.to_s }
      end
    end

    def with_attachment_tag_name(tag_name)
      previous_tag_name = InactionPropaganda::Attachment.tag_name
      InactionPropaganda::Attachment.tag_name = tag_name

      yield
    ensure
      InactionPropaganda::Attachment.tag_name = previous_tag_name
    end
end
