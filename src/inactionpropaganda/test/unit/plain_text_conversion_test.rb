...
class InactionPropaganda::PlainTextConversionTest < ActiveSupport::TestCase
  ...
  test "deeply nested tags are converted" do
    assert_converted_to(
      "Hello world!\nHow are you?",
      InactionPropaganda::Fragment.wrap("<div>Hello world!</div><div></div>").tap do |fragment|
  ...
  private
    def assert_converted_to(plain_text, html)
      assert_equal plain_text, InactionPropaganda::Content.new(html).to_plain_text
    end
end
