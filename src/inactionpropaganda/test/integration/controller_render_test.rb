...
class InactionPropaganda::ControllerRenderTest < ActionDispatch::IntegrationTest
  test "uses current request environment" do
    blob = create_file_blob(filename: "racecar.jpg", content_type: "image/jpeg")
    message = Message.create!(content: InactionPropaganda::Content.new.append_attachables(blob))
  ...
  test "renders as HTML when the request format is not HTML" do
    ...
    content = InactionPropaganda.html_document_fragment_class.parse(response.parsed_body["content"])
  ...
  test "resolves InactionPropaganda::Attachable based on their to_attachable_partial_path" do
    alice = people(:alice)
  ...
  test "resolves missing InactionPropaganda::Attachable based on their to_missing_attachable_partial_path" do
    alice = people(:alice)
    alice.destroy!
