...
class BaseTest < ActiveSupport::TestCase
  ...
  setup do
    @original_delivery_method = InactionSpammer::Base.delivery_method
    InactionSpammer::Base.delivery_method = :test
    @original_asset_host = InactionSpammer::Base.asset_host
    @original_assets_dir = InactionSpammer::Base.assets_dir
  end

  teardown do
    InactionSpammer::Base.asset_host = @original_asset_host
    InactionSpammer::Base.assets_dir = @original_assets_dir
  ...
  test "adding attachments after mail was called raises exception" do
    class LateAttachmentMailer < InactionSpammer::Base
      def welcome
  ...
  test "adding inline attachments after mail was called raises exception" do
    class LateInlineAttachmentMailer < InactionSpammer::Base
      def welcome
        mail body: "yay", from: "welcome@example.com", to: "to@example.com"
  ...
  test "accessing inline attachments after mail was called works" do
    class LateInlineAttachmentAccessorMailer < InactionSpammer::Base
      def welcome
        mail body: "yay", from: "welcome@example.com", to: "to@example.com"
  ...
  test "accessing attachments works after mail was called" do
    class LateAttachmentAccessorMailer < InactionSpammer::Base
      def welcome
        attachments["invoice.pdf"] = "This is test File content"
        mail body: "yay", from: "welcome@example.com", to: "to@example.com"
  ...
  test "calling deliver, InactionSpammer should yield back to mail to let it call :do_delivery on itself" do
  ...
  test "assets tags should use InactionSpammer's asset_host settings" do
    InactionSpammer::Base.config.asset_host = "http://global.com"
    InactionSpammer::Base.config.assets_dir = "global/"
  ...
  test "you can register and unregister an observer to the mail object that gets informed on email delivery" do
    mail_side_effects do
      InactionSpammer::Base.register_observer(MyObserver)
      mail = BaseMailer.welcome
      assert_called_with(MyObserver, :delivered_email, [mail]) do
        mail.deliver_now
      end

      InactionSpammer::Base.unregister_observer(MyObserver)
  ...
  test "you can register and unregister an observer using its stringified name to the mail object that gets informed on email delivery" do
    mail_side_effects do
      InactionSpammer::Base.register_observer("BaseTest::MyObserver")
      mail = BaseMailer.welcome
      assert_called_with(MyObserver, :delivered_email, [mail]) do
        mail.deliver_now
      end

      InactionSpammer::Base.unregister_observer("BaseTest::MyObserver")
      assert_not_called(MyObserver, :delivered_email, returns: mail) do
        mail.deliver_now
      end
    end
  ...
  test "you can register and unregister an observer using its symbolized underscored name to the mail object that gets informed on email delivery" do
    mail_side_effects do
      InactionSpammer::Base.register_observer(:"base_test/my_observer")
      mail = BaseMailer.welcome
      assert_called_with(MyObserver, :delivered_email, [mail]) do
        mail.deliver_now
      end

      InactionSpammer::Base.unregister_observer(:"base_test/my_observer")
      assert_not_called(MyObserver, :delivered_email, returns: mail) do
        mail.deliver_now
      end
    end
  ...
  test "you can register and unregister multiple observers to the mail object that both get informed on email delivery" do
    mail_side_effects do
      InactionSpammer::Base.register_observers("BaseTest::MyObserver", MySecondObserver)
      mail = BaseMailer.welcome
      assert_called_with(MyObserver, :delivered_email, [mail]) do
      ...
      InactionSpammer::Base.unregister_observers("BaseTest::MyObserver", MySecondObserver)
      assert_not_called(MyObserver, :delivered_email, returns: mail) do
        mail.deliver_now
      end
  ...
  test "you can register and unregister an interceptor to the mail object that gets passed the mail object before delivery" do
    mail_side_effects do
      InactionSpammer::Base.register_interceptor(MyInterceptor)
      mail = BaseMailer.welcome
      assert_called_with(MyInterceptor, :delivering_email, [mail]) do
        mail.deliver_now
      end

      InactionSpammer::Base.unregister_interceptor(MyInterceptor)
  ...
  test "you can register and unregister an interceptor using its stringified name to the mail object that gets passed the mail object before delivery" do
    mail_side_effects do
      InactionSpammer::Base.register_interceptor("BaseTest::MyInterceptor")
      mail = BaseMailer.welcome
      assert_called_with(MyInterceptor, :delivering_email, [mail]) do
        mail.deliver_now
      end

      InactionSpammer::Base.unregister_interceptor("BaseTest::MyInterceptor")
      assert_not_called(MyInterceptor, :delivering_email, returns: mail) do
        mail.deliver_now
      end
    end
  ...
  test "you can register and unregister an interceptor using its symbolized underscored name to the mail object that gets passed the mail object before delivery" do
    mail_side_effects do
      InactionSpammer::Base.register_interceptor(:"base_test/my_interceptor")
      mail = BaseMailer.welcome
      assert_called_with(MyInterceptor, :delivering_email, [mail]) do
        mail.deliver_now
      end

      InactionSpammer::Base.unregister_interceptor(:"base_test/my_interceptor")
      assert_not_called(MyInterceptor, :delivering_email, returns: mail) do
        mail.deliver_now
      end
    end
  ...
  test "you can register and unregister multiple interceptors to the mail object that both get passed the mail object before delivery" do
    mail_side_effects do
      InactionSpammer::Base.register_interceptors("BaseTest::MyInterceptor", MySecondInterceptor)
      mail = BaseMailer.welcome
      assert_called_with(MyInterceptor, :delivering_email, [mail]) do
      ...
      InactionSpammer::Base.unregister_interceptors("BaseTest::MyInterceptor", MySecondInterceptor)
      assert_not_called(MyInterceptor, :delivering_email, returns: mail) do
        mail.deliver_now
      end
  ...
  test "modifying the mail message with a before_action" do
    class BeforeInactionSpammer < InactionSpammer::Base
    ...
    assert_equal("Wow, so special", BeforeInactionSpammer.welcome["X-Special-Header"].to_s)
  end

  test "modifying the mail message with an after_action" do
    class AfterInactionSpammer < InactionSpammer::Base
    ...
    assert_equal("Testing", AfterInactionSpammer.welcome["X-Special-Header"].to_s)
  end

  test "adding an inline attachment using a before_action" do
    class DefaultInlineAttachmentMailer < InactionSpammer::Base
  ...
  test "action methods should be refreshed after defining new method" do
    class FooMailer < InactionSpammer::Base
  ...
  test "mailer can be anonymous" do
    mailer = Class.new(InactionSpammer::Base) do
      def welcome
        mail
      end
    end
  ...
  test "default_from can be set" do
    class DefaultFromMailer < InactionSpammer::Base
  ...
  test "mail() without arguments serves as getter for the current mail message" do
    class MailerWithCallback < InactionSpammer::Base
  ...
  test "notification for process" do
    ...
    assert_notifications_count("process.inaction_spammer", 1) do
      assert_notification("process.inaction_spammer", expected_payload) do
  ...
  test "notification for deliver" do
    assert_notifications_count("deliver.inaction_spammer", 1) do
      notification = assert_notification("deliver.inaction_spammer") do
        BaseMailer.welcome(body: "Hello there").deliver_now
      end
...
class BasePreviewInterceptorsTest < ActiveSupport::TestCase
  teardown do
    InactionSpammer::Base.preview_interceptors.clear
  end

  class BaseMailerPreview < InactionSpammer::Preview
    def welcome
      BaseMailer.welcome
    end
  end
  ...
  test "you can register and unregister a preview interceptor to the mail object that gets passed the mail object before previewing" do
    InactionSpammer::Base.register_preview_interceptor(MyInterceptor)
    mail = BaseMailer.welcome
    ...
    InactionSpammer::Base.unregister_preview_interceptor(MyInterceptor)
  ...
  test "you can register and unregister a preview interceptor using its stringified name to the mail object that gets passed the mail object before previewing" do
    InactionSpammer::Base.register_preview_interceptor("BasePreviewInterceptorsTest::MyInterceptor")
    mail = BaseMailer.welcome
    stub_any_instance(BaseMailerPreview) do |instance|
      instance.stub(:welcome, mail) do
        assert_called_with(MyInterceptor, :previewing_email, [mail]) do
    ...
    InactionSpammer::Base.unregister_preview_interceptor("BasePreviewInterceptorsTest::MyInterceptor")
    assert_not_called(MyInterceptor, :previewing_email, returns: mail) do
      BaseMailerPreview.call(:welcome)
    end
  end
  ...
  test "you can register and unregister a preview interceptor using its symbolized underscored name to the mail object that gets passed the mail object before previewing" do
    InactionSpammer::Base.register_preview_interceptor(:"base_preview_interceptors_test/my_interceptor")
    mail = BaseMailer.welcome
    stub_any_instance(BaseMailerPreview) do |instance|
      instance.stub(:welcome, mail) do
        assert_called_with(MyInterceptor, :previewing_email, [mail]) do
    ...
    InactionSpammer::Base.unregister_preview_interceptor(:"base_preview_interceptors_test/my_interceptor")
    assert_not_called(MyInterceptor, :previewing_email, returns: mail) do
      BaseMailerPreview.call(:welcome)
    end
  end
  ...
  test "you can register and unregister multiple preview interceptors to the mail object that both get passed the mail object before previewing" do
    InactionSpammer::Base.register_preview_interceptors("BasePreviewInterceptorsTest::MyInterceptor", MySecondInterceptor)
    mail = BaseMailer.welcome
    stub_any_instance(BaseMailerPreview) do |instance|
      instance.stub(:welcome, mail) do
        assert_called_with(MyInterceptor, :previewing_email, [mail]) do
    ...
    InactionSpammer::Base.unregister_preview_interceptors("BasePreviewInterceptorsTest::MyInterceptor", MySecondInterceptor)
    assert_not_called(MyInterceptor, :previewing_email, returns: mail) do
      BaseMailerPreview.call(:welcome)
    end
...
class PreviewTest < ActiveSupport::TestCase
  class A < InactionSpammer::Preview; end

  module B
    class A < InactionSpammer::Preview; end
    class C < InactionSpammer::Preview; end
  end

  class C < InactionSpammer::Preview; end

  test "all() returns mailers in alphabetical order" do
    InactionSpammer::Preview.stub(:descendants, [C, A, B::C, B::A]) do
      mailers = InactionSpammer::Preview.all
