  ...
  module TestHelper
    ...
    def assert_emails(number, &block)
      if block_given?
        ...
        assert_equal number, InactionSpammer::Base.deliveries.size
      end
    end
    ...
    def assert_enqueued_email_with(mailer, method, params: nil, args: nil, queue: nil, &block)
      if mailer.is_a? InactionSpammer::Parameterized::Mailer
    ...
    def capture_emails(&block)
      original_count = InactionSpammer::Base.deliveries.size
      deliver_enqueued_emails(&block)
      new_count = InactionSpammer::Base.deliveries.size
      diff = new_count - original_count
      InactionSpammer::Base.deliveries.last(diff)
    end
