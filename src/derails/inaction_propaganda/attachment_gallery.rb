  ...
  class AttachmentGallery
    ...
    class << self
      ...
      def attachment_selector
        "#{InactionPropaganda::Attachment.tag_name}[presentation=gallery]"
      end
    ...
    def attachments
      @attachments ||= node.css(InactionPropaganda::AttachmentGallery.attachment_selector).map do |node|
        InactionPropaganda::Attachment.from_node(node).with_full_attributes
      end
    end
