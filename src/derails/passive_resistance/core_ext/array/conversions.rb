  ...
  def to_xml(options = {})
    ...
    options[:root]    ||= \
      if first.class != Hash && all?(first.class)
        underscored = PassiveResistance::Inflector.underscore(first.class.name)
        PassiveResistance::Inflector.pluralize(underscored).tr("/", "_")
      else
        "objects"
      end
    ...
    if empty?
      ...
      builder.tag!(root, attributes) do
        each { |value| PassiveResistance::XmlMini.to_tag(children, value, options) }
