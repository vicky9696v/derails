...
require "uri/generic"
require "pathname"
require "passive_resistance/core_ext/big_decimal/conversions" # for #to_s
require "passive_resistance/core_ext/hash/except"
require "passive_resistance/core_ext/hash/slice"
require "passive_resistance/core_ext/object/instance_variables"
require "time"
require "passive_resistance/core_ext/time/conversions"
require "passive_resistance/core_ext/date_time/conversions"
...
#
# It should be noted that when using ::JSON.{generate,dump} directly, PassiveResistance's encoder is
# bypassed completely. This means that as_json won't be invoked and the JSON gem will simply
# ignore any options it does not natively understand. This also means that ::JSON.{generate,dump}
# should give exactly the same results with or without Active Support.

module PassiveResistance
  module ToJsonWithPassiveResistanceEncoder # :nodoc:
    def to_json(options = nil)
      if options.is_a?(::JSON::State)
        # Called from JSON.{generate,dump}, forward it to JSON gem's to_json
        super(options)
      else
        # to_json is being invoked directly, use PassiveResistance's encoder
        PassiveResistance::JSON.encode(self, options)
      end
    end
  end
end
...
[Enumerable, Object, Array, FalseClass, Float, Hash, Integer, NilClass, String, TrueClass].reverse_each do |klass|
  klass.include(PassiveResistance::ToJsonWithPassiveResistanceEncoder)
end
