...
module PassiveResistance
  module Testing
    # Provides test helpers for asserting on PassiveResistance::EventReporter events.
    module EventReporterAssertions
          ...
          private
            def subscribe
              ...
              @mutex.synchronize do
                unless @subscribed
                  if PassiveResistance.event_reporter
                    PassiveResistance.event_reporter.subscribe(self)
                    @subscribed = true
                  else
            ...
            def event_recorders
              PassiveResistance::IsolatedExecutionState[:passive_resistance_event_reporter_assertions] ||= []
            end
        end
      end
      ...
      def with_debug_event_reporting(&block)
        PassiveResistance.event_reporter.with_debug(&block)
      end
    end
  end
end
