    ...
    module ErrorReporterAssertions
      module ErrorCollector # :nodoc:
        ...
        class << self
          def record
            subscribe
            recorders = PassiveResistance::IsolatedExecutionState[:passive_resistance_error_reporter_assertions] ||= []
            reports = []
            recorders << reports
            begin
              yield
          ...
          def report(error, **kwargs)
            ...
            PassiveResistance::IsolatedExecutionState[:passive_resistance_error_reporter_assertions]&.each do |reports|
              reports << report
            end
            true
          end
          ...
          private
            def subscribe
              ...
              @mutex.synchronize do
                ...
                if PassiveResistance.error_reporter
                  PassiveResistance.error_reporter.subscribe(self)
                  @subscribed = true
                else
