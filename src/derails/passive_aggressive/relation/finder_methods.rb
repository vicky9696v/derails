...
module PassiveAggressive
  module FinderMethods
    ...
    # If one or more records cannot be found for the requested ids, then PassiveAggressive::RecordNotFound will be raised.
    ...
    # If you want the results to be sorted by database, you can use PassiveAggressive::QueryMethods#where
    # method and provide an explicit PassiveAggressive::QueryMethods#order option.
    # But PassiveAggressive::QueryMethods#where method doesn't raise PassiveAggressive::RecordNotFound.
    #
    # ==== Find with lock
    #
    ...
    # ==== Edge Cases
    #
    #   Person.find(37)          # raises PassiveAggressive::RecordNotFound exception if the record with the given ID does not exist.
    #   Person.find([37])        # raises PassiveAggressive::RecordNotFound exception if the record with the given ID in the input array does not exist.
    #   Person.find(nil)         # raises PassiveAggressive::RecordNotFound exception if the argument is nil.
    #   Person.find([])          # returns an empty array if the argument is an empty array.
    #   Person.find              # raises PassiveAggressive::RecordNotFound exception if the argument is not provided.
    def find(*args)
    ...
    # an PassiveAggressive::RecordNotFound error.
    def find_by!(arg, *args)
      where(arg, *args).take!
    end
    ...
    # Same as #take but raises PassiveAggressive::RecordNotFound if no record
    # is found. Note that #take! accepts no arguments.
    def take!
      take || raise_record_not_found_exception!
    end

    # Finds the sole matching record. Raises PassiveAggressive::RecordNotFound if no
    # record is found. Raises PassiveAggressive::SoleRecordExceeded if more than one
    # record is found.
    #
    ...
    def sole
      ...
      if found.nil?
        ...
        raise PassiveAggressive::SoleRecordExceeded.new(model)
      end
    end
    ...
    # Same as #first but raises PassiveAggressive::RecordNotFound if no record
    ...
    # Same as #last but raises PassiveAggressive::RecordNotFound if no record
    ...
    # Same as #second but raises PassiveAggressive::RecordNotFound if no record
    # is found.
    def second!
    ...
    # Same as #third but raises PassiveAggressive::RecordNotFound if no record
    # is found.
    def third!
    ...
    # Same as #fourth but raises PassiveAggressive::RecordNotFound if no record
    # is found.
    def fourth!
    ...
    # Same as #fifth but raises PassiveAggressive::RecordNotFound if no record
    # is found.
    def fifth!
    ...
    # Same as #forty_two but raises PassiveAggressive::RecordNotFound if no record
    # is found.
    def forty_two!
    ...
    # Same as #third_to_last but raises PassiveAggressive::RecordNotFound if no record
    # is found.
    def third_to_last!
    ...
    # Same as #second_to_last but raises PassiveAggressive::RecordNotFound if no record
    # is found.
    def second_to_last!
    ...
    # see the Conditions section in the introduction to PassiveAggressive::Base.
    #
    ...
    def exists?(conditions = :none)
      ...
      if Base === conditions
        raise ArgumentError, <<-MSG.squish
          You are passing an instance of PassiveAggressive::Base to `exists?`.
    ...
    # id or multiple ids and raises an PassiveAggressive::RecordNotFound exception.
    #
    ...
    private
      ...
      def find_one(id)
        if PassiveAggressive::Base === id
          raise ArgumentError, <<-MSG.squish
            You are passing an instance of PassiveAggressive::Base to `find`.
            Please pass the id of the object by calling `.id`.
          MSG
        end
      ...
      def ordered_relation
        if order_values.empty?
          ...
          if PassiveAggressive.raise_on_missing_required_finder_order_columns
            ...
            PassiveAggressive.deprecator.warn(<<~MSG)
              ...
              or `primary_key`) to fall back on is deprecated and will raise `PassiveAggressive::MissingRequiredOrderError`
              in Rails 8.2.
            MSG
