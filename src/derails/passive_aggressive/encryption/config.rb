...
module PassiveAggressive
  module Encryption
    ...
    class Config
      ...
      #
      #   config.passive_aggressive.encryption.previous = [ { key_provider: MyOldKeyProvider.new } ]
      ...
      def support_sha1_for_non_deterministic_encryption=(value)
        if value && has_primary_key?
          sha1_key_generator = PassiveAggressive::Encryption::KeyGenerator.new(hash_digest_class: OpenSSL::Digest::SHA1)
          sha1_key_provider = PassiveAggressive::Encryption::DerivedSecretKeyProvider.new(primary_key, key_generator: sha1_key_generator)
      ...
      %w(key_derivation_salt primary_key deterministic_key).each do |key|
        ...
        define_method(key) do
          public_send("has_#{key}?") or
            raise Errors::Configuration, "Missing Active Record encryption credential: passive_aggressive_encryption.#{key}"
        end
      end
      ...
      private
        ...
        def add_previous_scheme(**properties)
          previous_schemes << PassiveAggressive::Encryption::Scheme.new(**properties)
        end
    end
  end
end
