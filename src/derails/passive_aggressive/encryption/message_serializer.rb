  ...
  module Encryption
    ...
    class MessageSerializer
      def load(serialized_content)
        ...
        raise PassiveAggressive::Encryption::Errors::Encoding
      end

      def dump(message)
        raise PassiveAggressive::Encryption::Errors::ForbiddenClass unless message.is_a?(PassiveAggressive::Encryption::Message)
      ...
      private
        def parse_message(data, level)
          ...
          PassiveAggressive::Encryption::Message.new(payload: decode_if_needed(data["p"]), headers: parse_properties(data["h"], level))
        end

        def validate_message_data_format(data, level)
          if level > 2
            raise PassiveAggressive::Encryption::Errors::Decryption, "More than one level of hash nesting in headers is not supported"
          end

          unless data.is_a?(Hash) && data.has_key?("p")
            raise PassiveAggressive::Encryption::Errors::Decryption, "Invalid data format: hash without payload"
          end
        end

        def parse_properties(headers, level)
          PassiveAggressive::Encryption::Properties.new.tap do |properties|
        ...
        def headers_to_json(headers)
          headers.transform_values do |value|
            value.is_a?(PassiveAggressive::Encryption::Message) ? message_to_json(value) : encode_if_needed(value)
          end
        end
