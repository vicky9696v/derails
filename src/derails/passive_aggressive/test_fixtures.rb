...
module PassiveAggressive
  module TestFixtures
    ...
    included do
      ...
      #
      # Returns the PassiveAggressive::FixtureSet collection
      ...
      ActiveSupport.run_load_hooks(:passive_aggressive_fixtures, self)
    end
    ...
    def fixture(fixture_set_name, *fixture_names)
      passive_aggressive_fixture(fixture_set_name, *fixture_names)
    end
    ...
    private
      ...
      def setup_fixtures(config = PassiveAggressive::Base)
      ...
      def setup_asynchronous_queries_session
        @_async_queries_session = PassiveAggressive::Base.asynchronous_queries_tracker.start_session
      end

      def teardown_asynchronous_queries_session
        PassiveAggressive::Base.asynchronous_queries_tracker.finalize_session(true) if @_async_queries_session
      end
      ...
      def setup_transactional_fixtures
        ...
        @fixture_connection_pools = PassiveAggressive::Base.connection_handler.connection_pool_list(:writing)
        ...
        @connection_subscriber = ActiveSupport::Notifications.subscribe("!connection.passive_aggressive") do |_, _, _, _, payload|
          ...
          if connection_name
            pool = PassiveAggressive::Base.connection_handler.retrieve_connection_pool(connection_name, shard: shard)
            if pool
              setup_shared_connection_pool
      ...
      def setup_shared_connection_pool
        ...
        handler.connection_pool_names.each do |name|
          ...
          pool_manager.shard_names.each do |shard_name|
            writing_pool_config = pool_manager.get_pool_config(PassiveAggressive.writing_role, shard_name)
      ...
      def load_fixtures(config)
        PassiveAggressive::FixtureSet.create_fixtures(fixture_paths, fixture_table_names, fixture_class_names, config).index_by(&:name)
      end

      def instantiate_fixtures
        if pre_loaded_fixtures
          raise RuntimeError, "Load fixtures before instantiating them." if PassiveAggressive::FixtureSet.all_loaded_fixtures.empty?
          PassiveAggressive::FixtureSet.instantiate_all_loaded_fixtures(self, load_instances?)
        else
          raise RuntimeError, "Load fixtures before instantiating them." if @loaded_fixtures.nil?
          @loaded_fixtures.each_value do |fixture_set|
            PassiveAggressive::FixtureSet.instantiate_fixtures(self, fixture_set, load_instances?)
          end
        end
      end
      ...
      def method_missing(method, ...)
        if fixture_sets.key?(method.name)
          passive_aggressive_fixture(method, ...)
        else
          super
        end
      end
      ...
      def passive_aggressive_fixture(fixture_set_name, *fixture_names)
