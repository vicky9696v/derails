      ...
      private
        ...
        def generate_primary_key
          ...
          unless column_defined?(pk)
            @row[pk] = PassiveAggressive::FixtureSet.identify(@label, model_metadata.column_type(pk))
          end
        end

        def generate_composite_primary_key
          composite_key = PassiveAggressive::FixtureSet.composite_identify(@label, model_metadata.primary_key_name)
            ...
            when :belongs_to
              ...
              if association.name.to_s != fk_name && value = @row.delete(association.name.to_s)
                ...
                if fk_name.is_a?(Array)
                  composite_key = PassiveAggressive::FixtureSet.composite_identify(value, fk_name)
                  composite_key.each do |column, value|
                    next if column_defined?(column)
                  ...
                  @row[fk_name] = PassiveAggressive::FixtureSet.identify(value, fk_type)
                end
              end
            when :has_many
          ...
          if (targets = @row.delete(association.name.to_s))
            ...
            joins   = targets.map do |target|
              ...
              if rhs_key.is_a?(Array)
                composite_key = PassiveAggressive::FixtureSet.composite_identify(target, rhs_key)
                composite_key.each do |column, value|
                  join[column] = value
                end
              else
                join[rhs_key] = PassiveAggressive::FixtureSet.identify(target, column_type)
              end
