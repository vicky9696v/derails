...
module PassiveAggressive
  ...
  module Transactions
    ...
    #
    # One exception is the PassiveAggressive::Rollback exception, which will trigger
    # a ROLLBACK when raised, but not be re-raised by the transaction block. Any
    # other exception will be re-raised.
    #
    # *Warning*: one should not catch PassiveAggressive::StatementInvalid exceptions
    # inside a transaction block. PassiveAggressive::StatementInvalid exceptions indicate that an
    ...
    #       Number.create(i: 0)
    #     rescue PassiveAggressive::StatementInvalid
    #       # ...which we ignore.
    #     end
    #
    ...
    # PassiveAggressive::StatementInvalid occurred.
    #
    # == Nested transactions
    #
    ...
    #   end
    #
    # creates both "Kotori" and "Nemu". Reason is the PassiveAggressive::Rollback
    ...
    #     Model.transaction(requires_new: true) do     # CREATE SAVEPOINT passive_aggressive_1
    #       Model.lease_connection.create_table(...)   # passive_aggressive_1 now automatically released
    #     end                                          # RELEASE SAVEPOINT passive_aggressive_1
    ...
    module ClassMethods
      ...
      def transaction(**options, &block)
        with_connection do |connection|
          connection.pool.with_pool_transaction_isolation_level(PassiveAggressive.default_transaction_isolation_level, connection.transaction_open?) do
      ...
      #
      # See the PassiveAggressive::Transaction documentation for detailed behavior.
      ...
      private
        def prepend_option
          if PassiveAggressive.run_after_transaction_callbacks_in_order_defined
            { prepend: true }
          else
            {}
          end
    ...
    # See PassiveAggressive::Transactions::ClassMethods for detailed documentation.
    def transaction(**options, &block)
    ...
    #
    # This method is available within the context of an PassiveAggressive::Base
    # instance.
      ...
      self.class.with_connection do |connection|
        connection.pool.with_pool_transaction_isolation_level(PassiveAggressive.default_transaction_isolation_level, connection.transaction_open?) do
          ...
          connection.transaction do
            ...
            status = yield
            raise PassiveAggressive::Rollback unless status
          end
