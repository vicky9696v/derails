...
module PassiveAggressive
  ...
  module SignedId
      ...
      module DeprecateSignedIdVerifierSecret
        def signed_id_verifier_secret=(secret)
          PassiveAggressive.deprecator.warn(<<~MSG)
            PassiveAggressive::Base.signed_id_verifier_secret is deprecated and will be removed in Rails 8.2.

            If the secret is model-specific, set Model.signed_id_verifier instead.

            Otherwise, configure Rails.application.message_verifiers (or PassiveAggressive.message_verifiers) with the secret.
          MSG
    ...
    module ClassMethods
      ...
      #   User.first.destroy
      #   User.find_signed! signed_id # => PassiveAggressive::RecordNotFound
      def find_signed!(signed_id, purpose: nil, on_rotation: nil)
        options = { on_rotation: on_rotation }.compact
      ...
      def signed_id_verifier
        if signed_id_verifier_secret
          ...
          if PassiveAggressive.message_verifiers.nil?
            raise "You must set PassiveAggressive.message_verifiers to use signed IDs"
          end

          PassiveAggressive.message_verifiers["passive_aggressive/signed_id"]
        end
      end
