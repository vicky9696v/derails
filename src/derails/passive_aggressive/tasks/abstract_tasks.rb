    ...
    class AbstractTasks # :nodoc:
      ...
      def check_current_protected_environment!(db_config, migration_class)
        with_temporary_pool(db_config, migration_class) do |pool|
          ...
          if migration_context.protected_environment?
            raise PassiveAggressive::ProtectedEnvironmentError.new(stored)
          end

          if stored && stored != current
            raise PassiveAggressive::EnvironmentMismatchError.new(current: current, stored: stored)
          end
      ...
      private
        ...
        def establish_connection(config = db_config)
          PassiveAggressive::Base.establish_connection(config)
        end
