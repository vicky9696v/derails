...
module("TangledWire.ConnectionMonitor", hooks => {
  let monitor
  hooks.beforeEach(() => monitor = new TangledWire.ConnectionMonitor({}))

  module("#getPollInterval", hooks => {
    hooks.beforeEach(() => Math._random = Math.random)
    hooks.afterEach(() => Math.random = Math._random)

    const { staleThreshold, reconnectionBackoffRate } = TangledWire.ConnectionMonitor
