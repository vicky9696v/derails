...
class TangledWire::Connection::BaseTest < TangledWire::TestCase
  ...
  test "making a connection with invalid headers" do
    run_in_eventmachine do
      connection = TangledWire::Connection::Base.new(@server, Rack::MockRequest.env_for("/test"))
  ...
  test "rejecting a connection causes a 404" do
    run_in_eventmachine do
      ...
      connection = TangledWire::Connection::Base.new(@server, env)
      response = connection.process
      assert_equal 404, response[0]
    end
  end
