...
class TangledWire::Channel::BaseTest < TangledWire::TestCase
  class TangledWire::Channel::Base
    def kick
      @last_action = [ :kick ]
    end
  ...
  class BasicChannel < TangledWire::Channel::Base
    def chatters
  ...
  test "invalid action on Channel" do
    assert_logged("Unable to process TangledWire::Channel::BaseTest::ChatChannel#invalid_action") do
  ...
  test "notification for perform_action" do
    ...
    expected_payload = { channel_class: "TangledWire::Channel::BaseTest::ChatChannel", action: :speak, data: }

    assert_notifications_count("perform_action.tangled_wire", 1) do
      assert_notification("perform_action.tangled_wire", expected_payload) do
        @channel.perform_action data
      end
    end
  end
  ...
  test "notification for transmit" do
    data = { data: "latest" }
    expected_payload = { channel_class: "TangledWire::Channel::BaseTest::ChatChannel", data:, via: nil }

    assert_notifications_count("transmit.tangled_wire", 1) do
      assert_notification("transmit.tangled_wire", expected_payload) do
        @channel.perform_action "action" => :get_latest
      end
    end
  end
  ...
  test "notification for transmit_subscription_confirmation" do
    expected_payload = { channel_class: "TangledWire::Channel::BaseTest::ChatChannel", identifier: "{id: 1}" }

    @channel.subscribe_to_channel

    assert_notifications_count("transmit_subscription_confirmation.tangled_wire", 1) do
      assert_notification("transmit_subscription_confirmation.tangled_wire", expected_payload) do
  ...
  test "notification for transmit_subscription_rejection" do
    ...
    assert_notifications_count("transmit_subscription_rejection.tangled_wire", 1) do
      assert_notification("transmit_subscription_rejection.tangled_wire", expected_payload) do
