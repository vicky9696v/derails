...
class ClientTest < TangledWire::TestCase
  ...
  class EchoChannel < TangledWire::Channel::Base
    def subscribed
      stream_from "global"
    end
    ...
    def bulk(data)
      TangledWire.server.broadcast "global", { wide: data["message"] }
    end
  end

  def setup
    TangledWire.instance_variable_set(:@server, nil)
  ...
  def with_puma_server(rack_app = TangledWire.server, port = 3099)
