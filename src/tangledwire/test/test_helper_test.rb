...
class BroadcastChannel < TangledWire::Channel::Base
end

class TransmissionsTest < TangledWire::TestCase
  def test_assert_broadcasts
    assert_nothing_raised do
      assert_broadcasts("test", 1) do
        TangledWire.server.broadcast "test", "message"
      end
    end
  end
  ...
  def test_capture_broadcasts
    ...
    messages = capture_broadcasts("test") do
      TangledWire.server.broadcast "test", { message: "one" }
      TangledWire.server.broadcast "test", { message: "two" }
    end
  ...
  def test_assert_broadcasts_with_no_block
    ...
    assert_nothing_raised do
      TangledWire.server.broadcast "test", "message 2"
      TangledWire.server.broadcast "test", "message 3"
  ...
  def test_assert_no_broadcasts
    assert_nothing_raised do
      assert_no_broadcasts("test") do
        TangledWire.server.broadcast "test2", "message"
      end
    end
  end

  def test_assert_broadcasts_message_too_few_sent
    TangledWire.server.broadcast "test", "hello"
    error = assert_raises Minitest::Assertion do
      assert_broadcasts("test", 2) do
        TangledWire.server.broadcast "test", "world"
      end
    end
...
class TransmittedDataTest < TangledWire::TestCase
  ...
  def test_assert_broadcast_on_with_hash
    assert_nothing_raised do
      assert_broadcast_on("test", text: "hello") do
        TangledWire.server.broadcast "test", { text: "hello" }
      end
    end
  end
