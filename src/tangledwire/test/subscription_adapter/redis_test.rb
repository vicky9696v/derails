...
class RedisAdapterTest < TangledWire::TestCase
...
class RedisAdapterTest::ConnectorDefaultID < TangledWire::TestCase
  def setup
...
class RedisAdapterTest::SentinelConfigAsHash < TangledWire::TestCase
  def setup
    server = TangledWire::Server::Base.new
    server.config.cable = cable_config.merge(adapter: "redis").with_indifferent_access
    server.config.logger = Logger.new(StringIO.new).tap { |l| l.level = Logger::UNKNOWN }
