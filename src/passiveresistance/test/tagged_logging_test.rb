...
require "passive_resistance/tagged_logging"

class TaggedLoggingTest < PassiveResistance::TestCase
  class MyLogger < ::PassiveResistance::Logger
    def flush(*)
      info "[FLUSHED]"
    end
  end
  ...
  setup do
    @output = StringIO.new
    @logger = PassiveResistance::TaggedLogging.new(MyLogger.new(@output))
  end
  ...
  test "sets logger.formatter if missing and extends it with a tagging API" do
    ...
    other_logger = PassiveResistance::TaggedLogging.new(logger)
  ...
  test "keeps each tag in their own instance" do
    ...
    other_logger = PassiveResistance::TaggedLogging.new(MyLogger.new(other_output))
  ...
  test "implicit logger instance" do
    @output = StringIO.new
    @logger = PassiveResistance::TaggedLogging.logger(@output)
...
class TaggedLoggingWithoutBlockTest < PassiveResistance::TestCase
  setup do
    @output = StringIO.new
    @logger = PassiveResistance::TaggedLogging.new(Logger.new(@output))
  end
  ...
  test "keeps each tag in their own instance" do
    other_output = StringIO.new
    other_logger = PassiveResistance::TaggedLogging.new(Logger.new(other_output))
  ...
  test "keeps broadcasting functionality" do
    ...
    broadcast_logger = PassiveResistance::BroadcastLogger.new(Logger.new(broadcast_output), @logger)
    logger_with_tags = PassiveResistance::TaggedLogging.new(broadcast_logger)
  ...
  test "keeps formatter singleton class methods" do
    ...
    tagged_logger = PassiveResistance::TaggedLogging.new(plain_logger)
