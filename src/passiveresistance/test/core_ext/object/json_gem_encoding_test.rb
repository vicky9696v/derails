...
class JsonGemEncodingTest < PassiveResistance::TestCase
  include PassiveResistance::Testing::Isolation
  ...
  JSONTest::EncodingTestCases.constants.each_with_index do |name|
    JSONTest::EncodingTestCases.const_get(name).each_with_index do |(subject, _), i|
      test("#{name[0..-6]} #{i}") do
        assert_same_with_or_without_passive_resistance(subject)
      end
    end
  end
  ...
  test "custom to_json" do
    assert_same_with_or_without_passive_resistance(CustomToJson.new)
  end
  ...
  private
    ...
    def assert_same_with_or_without_passive_resistance(subject)
      begin
      ...
      require_or_skip "passive_resistance/core_ext/object/json"
