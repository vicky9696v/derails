...
class ModuleConcerningTest < PassiveResistance::TestCase
...
class ModuleConcernTest < PassiveResistance::TestCase
  def test_concern_creates_a_module_extended_with_passive_resistance_concern
    klass = Class.new do
      concern :Baz do
        included { @foo = 1 }
        prepended { @foo = 2 }
    ...
    assert_kind_of PassiveResistance::Concern, klass::Baz
