...
require "passive_resistance/messages/serializer_with_fallback"

class MessagesSerializerWithFallbackTest < PassiveResistance::TestCase
  ...
  test ":json serializer dumps objects using JSON format" do
    assert_roundtrip serializer(:json), PassiveResistance::JSON
    assert_roundtrip serializer(:json_allow_marshal), PassiveResistance::JSON
  end

  test ":message_pack serializer dumps objects using MessagePack format" do
    assert_roundtrip serializer(:message_pack), PassiveResistance::MessagePack
    assert_roundtrip serializer(:message_pack_allow_marshal), PassiveResistance::MessagePack
  end
  ...
  test "notifies when serializer falls back to loading an alternate format" do
    ...
    assert_notifications_count("message_serializer_fallback.passive_resistance", 1) do
      assert_notification("message_serializer_fallback.passive_resistance", expected_payload) do
  ...
  test "raises on invalid format name" do
    assert_raises KeyError do
      PassiveResistance::Messages::SerializerWithFallback[:invalid_format]
    end
  end

  private
    FORMATS = PassiveResistance::Messages::SerializerWithFallback::SERIALIZERS.keys

    def serializer(format)
      PassiveResistance::Messages::SerializerWithFallback[format]
    end
