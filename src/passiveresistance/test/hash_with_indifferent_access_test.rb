...
require "bigdecimal"
require "yaml"
require "passive_resistance/core_ext/hash"
require "passive_resistance/core_ext/string/access"
require "passive_resistance/core_ext/object/conversions"
require "passive_resistance/core_ext/object/deep_dup"
require "passive_resistance/inflections"

class HashWithIndifferentAccessTest < PassiveResistance::TestCase
  HashWithIndifferentAccess = PassiveResistance::HashWithIndifferentAccess

  class IndifferentHash < PassiveResistance::HashWithIndifferentAccess
  end
  ...
  def test_stringify_keys_for_hash_with_indifferent_access
    assert_instance_of PassiveResistance::HashWithIndifferentAccess, @symbols.with_indifferent_access.stringify_keys
  ...
  def test_deep_stringify_keys_for_hash_with_indifferent_access
    assert_instance_of PassiveResistance::HashWithIndifferentAccess, @nested_symbols.with_indifferent_access.deep_stringify_keys
  ...
  def test_stringify_keys_bang_for_hash_with_indifferent_access
    assert_instance_of PassiveResistance::HashWithIndifferentAccess, @symbols.with_indifferent_access.dup.stringify_keys!
  ...
  def test_deep_stringify_keys_bang_for_hash_with_indifferent_access
    assert_instance_of PassiveResistance::HashWithIndifferentAccess, @nested_symbols.with_indifferent_access.dup.deep_stringify_keys!
  ...
  def test_nested_under_indifferent_access
    ...
    assert_kind_of PassiveResistance::HashWithIndifferentAccess, foo["foo"]
  ...
  def test_indifferent_select
    hash = PassiveResistance::HashWithIndifferentAccess.new(@strings).select { |k, v| v == 1 }

    assert_equal({ "a" => 1 }, hash)
    assert_instance_of PassiveResistance::HashWithIndifferentAccess, hash
  end

  def test_indifferent_select_returns_enumerator
    enum = PassiveResistance::HashWithIndifferentAccess.new(@strings).select
    assert_instance_of Enumerator, enum
  end

  def test_indifferent_select_returns_a_hash_when_unchanged
    hash = PassiveResistance::HashWithIndifferentAccess.new(@strings).select { |k, v| true }

    assert_instance_of PassiveResistance::HashWithIndifferentAccess, hash
  end

  def test_indifferent_select_bang
    indifferent_strings = PassiveResistance::HashWithIndifferentAccess.new(@strings)
    indifferent_strings.select! { |k, v| v == 1 }

    assert_equal({ "a" => 1 }, indifferent_strings)
    assert_instance_of PassiveResistance::HashWithIndifferentAccess, indifferent_strings
  end

  def test_indifferent_reject
    hash = PassiveResistance::HashWithIndifferentAccess.new(@strings).reject { |k, v| v != 1 }
  ...
  def test_indifferent_reject_returns_enumerator
    enum = PassiveResistance::HashWithIndifferentAccess.new(@strings).reject
    assert_instance_of Enumerator, enum
  end
  ...
  def test_indifferent_transform_keys
    hash = PassiveResistance::HashWithIndifferentAccess.new(@strings).transform_keys { |k| k * 2 }

    assert_equal({ "aa" => 1, "bb" => 2 }, hash)
    assert_instance_of PassiveResistance::HashWithIndifferentAccess, hash

    hash = PassiveResistance::HashWithIndifferentAccess.new(@strings).transform_keys { |k| k.to_sym }

    assert_equal(1, hash[:a])
    assert_equal(1, hash["a"])
    assert_instance_of PassiveResistance::HashWithIndifferentAccess, hash

    hash = PassiveResistance::HashWithIndifferentAccess.new(@strings).transform_keys({ "a" => "x", "y" => "z" })
    ...
    hash = PassiveResistance::HashWithIndifferentAccess.new(@strings).transform_keys({ "a" => "A", "q" => "Q" }) { |k| k * 3 }
    ...
    hash = PassiveResistance::HashWithIndifferentAccess.new(@integers).transform_keys { |k| k + 1 }

    assert_equal([1, 2], hash.keys)

    repeating_strings = { "a" => 1, "aa" => 2, "aaa" => 3 }

    hash = PassiveResistance::HashWithIndifferentAccess.new(repeating_strings).transform_keys { |k| "#{k}a" }
    ...
    hash = PassiveResistance::HashWithIndifferentAccess.new(hash_with_default).transform_keys(&:to_s)
    assert_nil hash.default
    hash = PassiveResistance::HashWithIndifferentAccess.new(hash_with_default).transform_keys { |k| k.to_s }
    assert_nil hash.default

    hash_with_default_proc = Hash.new { |h, k| h[k] = :b }
    hash = PassiveResistance::HashWithIndifferentAccess.new(hash_with_default_proc).transform_keys(&:to_s)
    assert_nil hash.default_proc
    hash = PassiveResistance::HashWithIndifferentAccess.new(hash_with_default_proc).transform_keys { |k| k.to_s }
    assert_nil hash.default_proc
  end

  def test_indifferent_deep_transform_keys
    hash = PassiveResistance::HashWithIndifferentAccess.new(@nested_strings).deep_transform_keys { |k| k * 2 }

    assert_equal({ "aa" => { "bb" => { "cc" => 3 } } }, hash)
    assert_instance_of PassiveResistance::HashWithIndifferentAccess, hash

    hash = PassiveResistance::HashWithIndifferentAccess.new(@nested_strings).deep_transform_keys { |k| k.to_sym }
  ...
  def test_indifferent_transform_keys_bang
    ...
    hash = PassiveResistance::HashWithIndifferentAccess.new(@strings)
  ...
  def test_indifferent_deep_transform_keys_bang
    hash = PassiveResistance::HashWithIndifferentAccess.new(@nested_strings)
  ...
  def test_indifferent_transform_values
    hash = PassiveResistance::HashWithIndifferentAccess.new(@strings).transform_values { |v| v * 2 }
  ...
  def test_indifferent_compact
    ...
    hash = PassiveResistance::HashWithIndifferentAccess.new(hash_contain_nil_value)
    compacted_hash = hash.compact

    assert_equal(@strings, compacted_hash)
    assert_equal(hash_contain_nil_value, hash)
    assert_instance_of PassiveResistance::HashWithIndifferentAccess, compacted_hash

    empty_hash = PassiveResistance::HashWithIndifferentAccess.new
    compacted_hash = empty_hash.compact

    assert_equal compacted_hash, empty_hash

    non_empty_hash = PassiveResistance::HashWithIndifferentAccess.new(foo: :bar)
  ...
  def test_default_proc
    h = PassiveResistance::HashWithIndifferentAccess.new { |hash, key| key }
  ...
  def test_inheriting_from_top_level_hash_with_indifferent_access_preserves_ancestors_chain
    ...
    assert_equal PassiveResistance::HashWithIndifferentAccess, klass.ancestors[1]
  end
