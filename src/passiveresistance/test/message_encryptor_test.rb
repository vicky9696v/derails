...
class MessageEncryptorTest < PassiveResistance::TestCase
  ...
  def setup
    ...
    @verifier  = PassiveResistance::MessageVerifier.new(@secret, serializer: PassiveResistance::MessageEncryptor::NullSerializer)
    @encryptor = PassiveResistance::MessageEncryptor.new(@secret)
  ...
  def test_backwards_compat_for_64_bytes_key
    ...
    encryptor = PassiveResistance::MessageEncryptor.new(secret[0..31], secret)
  ...
  def test_alternative_serialization_method
    ...
    encryptor = PassiveResistance::MessageEncryptor.new(SecureRandom.random_bytes(32), SecureRandom.random_bytes(128), serializer: JSONSerializer.new)
  ...
  test "supports URL-safe encoding when using authenticated encryption" do
    encryptor = PassiveResistance::MessageEncryptor.new(@secret, url_safe: true, cipher: "aes-256-gcm")
  ...
  test "supports URL-safe encoding when using unauthenticated encryption" do
    encryptor = PassiveResistance::MessageEncryptor.new(@secret, url_safe: true, cipher: "aes-256-cbc")
  ...
  def test_aead_mode_encryption
    encryptor = PassiveResistance::MessageEncryptor.new(@secret, cipher: "aes-256-gcm")
    message = encryptor.encrypt_and_sign(@data)
    assert_equal @data, encryptor.decrypt_and_verify(message)
  end
  ...
  def test_inspect_does_not_show_secrets
    encryptor = PassiveResistance::MessageEncryptor.new(@secret, cipher: "aes-256-gcm")
    assert_match(/\A#<PassiveResistance::MessageEncryptor:0x[0-9a-f]+>\z/, encryptor.inspect)
  end
  ...
  private
    ...
    def assert_aead_not_decrypted(encryptor, value)
      assert_raise(PassiveResistance::MessageEncryptor::InvalidMessage) do
