...
class AssertionsTest < PassiveResistance::TestCase
  def setup
    @object = Class.new do
      attr_accessor :num
      def increment
...
class ExceptionsInsideAssertionsTest < PassiveResistance::TestCase
  def before_setup
    require "stringio"
    @out = StringIO.new
    self.tagged_logger = PassiveResistance::TaggedLogging.new(Logger.new(@out))
    super
  end
...
class SetupAndTeardownTest < PassiveResistance::TestCase
...
class TestCaseTaggedLoggingTest < PassiveResistance::TestCase
  def before_setup
    require "stringio"
    @out = StringIO.new
    self.tagged_logger = PassiveResistance::TaggedLogging.new(Logger.new(@out))
...
class TestOrderTest < PassiveResistance::TestCase
  def setup
    @original_test_order = PassiveResistance::TestCase.test_order
  end

  def teardown
    PassiveResistance::TestCase.test_order = @original_test_order
  end

  def test_defaults_to_random
    PassiveResistance::TestCase.test_order = nil

    assert_equal :random, PassiveResistance::TestCase.test_order

    assert_equal :random, PassiveResistance.test_order
  end

  def test_test_order_is_global
    PassiveResistance::TestCase.test_order = :sorted

    assert_equal :sorted, PassiveResistance.test_order
    assert_equal :sorted, PassiveResistance::TestCase.test_order
    assert_equal :sorted, self.class.test_order
    assert_equal :sorted, Class.new(PassiveResistance::TestCase).test_order
    ...
    assert_equal :random, Class.new(PassiveResistance::TestCase).test_order
  end
end
...
class TestConstStubbing < PassiveResistance::TestCase
