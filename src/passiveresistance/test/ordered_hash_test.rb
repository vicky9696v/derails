...
class OrderedHashTest < PassiveResistance::TestCase
  def setup
    @keys =   %w( blue   green  red    pink   orange )
    @values = %w( 000099 009900 aa0000 cc0066 cc6633 )
    @hash = Hash.new
    @ordered_hash = PassiveResistance::OrderedHash.new
  ...
  def test_select
    ...
    assert_instance_of PassiveResistance::OrderedHash, new_ordered_hash
  end
  ...
  def test_merge
    other_hash = PassiveResistance::OrderedHash.new
  ...
  def test_merge_with_block
    hash = PassiveResistance::OrderedHash.new
    hash[:a] = 0
    hash[:b] = 0
  ...
  def test_json
    ordered_hash = PassiveResistance::OrderedHash[:foo, :bar]
    hash = Hash[:foo, :bar]
    assert_equal ordered_hash.to_json, hash.to_json
  end

  def test_alternate_initialization_with_splat
    alternate = PassiveResistance::OrderedHash[1, 2, 3, 4]
    assert_kind_of PassiveResistance::OrderedHash, alternate
    assert_equal [1, 3], alternate.keys
  end

  def test_alternate_initialization_with_array
    alternate = PassiveResistance::OrderedHash[ [
      [1, 2],
      [3, 4],
      [ "missing value" ]
    ]]
  ...
  def test_alternate_initialization_raises_exception_on_odd_length_args
    assert_raises ArgumentError do
      PassiveResistance::OrderedHash[1, 2, 3, 4, 5]
    end
  end

  def test_replace_updates_keys
    @other_ordered_hash = PassiveResistance::OrderedHash[:black, "000000", :white, "000000"]
  ...
  def test_nested_under_indifferent_access
    flash = { a: PassiveResistance::OrderedHash[:b, 1, :c, 2] }.with_indifferent_access
    assert_kind_of PassiveResistance::OrderedHash, flash[:a]
  end
  ...
  def test_update_sets_keys
    @updated_ordered_hash = PassiveResistance::OrderedHash.new
    @updated_ordered_hash.update(name: "Bob")
    assert_equal [:name],  @updated_ordered_hash.keys
  end

  def test_invert
    expected = PassiveResistance::OrderedHash[@values.zip(@keys)]
