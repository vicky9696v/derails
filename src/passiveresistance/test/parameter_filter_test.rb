...
class ParameterFilterTest < PassiveResistance::TestCase
  test "process parameter filter" do
    ...
    test_hashes.each do |before_filter, after_filter, filter_words|
      parameter_filter = PassiveResistance::ParameterFilter.new(filter_words)
  ...
  test "filter should return mask option when value is filtered" do
    ...
    test_hashes.each do |before_filter, after_filter, filter_words|
      parameter_filter = PassiveResistance::ParameterFilter.new(filter_words, mask: mask)
      assert_equal after_filter, parameter_filter.filter(before_filter)
  ...
  test "filter_param" do
    parameter_filter = PassiveResistance::ParameterFilter.new(["foo", /bar/])
  ...
  test "filter_param can work with empty filters" do
    parameter_filter = PassiveResistance::ParameterFilter.new
  ...
  test "parameter filter should maintain hash with indifferent access" do
    ...
    test_hashes.each do |before_filter, filter_words|
      parameter_filter = PassiveResistance::ParameterFilter.new(filter_words)
      assert_instance_of PassiveResistance::HashWithIndifferentAccess,
  ...
  test "filter_param should return mask option when value is filtered" do
    mask = Object.new.freeze
    parameter_filter = PassiveResistance::ParameterFilter.new(["foo", /bar/], mask: mask)
  ...
  test "precompile_filters" do
    ...
    precompiled = PassiveResistance::ParameterFilter.precompile_filters([*patterns, *deep_patterns, *procs])
