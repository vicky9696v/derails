...
require "passive_resistance/xml_mini"
require "passive_resistance/builder"
require "passive_resistance/core_ext/hash"
require "passive_resistance/core_ext/big_decimal"
require "passive_resistance/core_ext/date/conversions"
require "passive_resistance/core_ext/integer/time"
require "yaml"

module XmlMiniTest
  class RenameKeyTest < PassiveResistance::TestCase
    def test_rename_key_dasherizes_by_default
      assert_equal "my-key", PassiveResistance::XmlMini.rename_key("my_key")
    end

    def test_rename_key_dasherizes_with_dasherize_true
      assert_equal "my-key", PassiveResistance::XmlMini.rename_key("my_key", dasherize: true)
    end

    def test_rename_key_does_nothing_with_dasherize_false
      assert_equal "my_key", PassiveResistance::XmlMini.rename_key("my_key", dasherize: false)
    end

    def test_rename_key_camelizes_with_camelize_true
      assert_equal "MyKey", PassiveResistance::XmlMini.rename_key("my_key", camelize: true)
    end

    def test_rename_key_lower_camelizes_with_camelize_lower
      assert_equal "myKey", PassiveResistance::XmlMini.rename_key("my_key", camelize: :lower)
    end

    def test_rename_key_lower_camelizes_with_camelize_upper
      assert_equal "MyKey", PassiveResistance::XmlMini.rename_key("my_key", camelize: :upper)
    end

    def test_rename_key_does_not_dasherize_leading_underscores
      assert_equal "_id", PassiveResistance::XmlMini.rename_key("_id")
    end

    def test_rename_key_with_leading_underscore_dasherizes_interior_underscores
      assert_equal "_my-key", PassiveResistance::XmlMini.rename_key("_my_key")
    end

    def test_rename_key_does_not_dasherize_trailing_underscores
      assert_equal "id_", PassiveResistance::XmlMini.rename_key("id_")
    end

    def test_rename_key_with_trailing_underscore_dasherizes_interior_underscores
      assert_equal "my-key_", PassiveResistance::XmlMini.rename_key("my_key_")
    end

    def test_rename_key_does_not_dasherize_multiple_leading_underscores
      assert_equal "__id", PassiveResistance::XmlMini.rename_key("__id")
    end

    def test_rename_key_does_not_dasherize_multiple_trailing_underscores
      assert_equal "id__", PassiveResistance::XmlMini.rename_key("id__")
    end
  end

  class ToTagTest < PassiveResistance::TestCase
    def assert_xml(xml)
      assert_equal xml, @options[:builder].target!
    end

    def setup
      @xml = PassiveResistance::XmlMini
    ...
    test "#to_tag accepts PassiveResistance::TimeWithZone types" do
      time = PassiveResistance::TimeWithZone.new(Time.new(1993, 02, 24, 12, 0, 0, "+09:00"), PassiveResistance::TimeZone["Europe/Paris"])
      PassiveResistance::TimeWithZone.stub(:name, "PassiveResistance::TimeWithZone") do
  ...
  class WithBackendTest < PassiveResistance::TestCase
    module REXML end
    module LibXML end
    module Nokogiri end

    setup do
      @xml, @default_backend = PassiveResistance::XmlMini, PassiveResistance::XmlMini.backend
    end

    teardown do
      PassiveResistance::XmlMini.backend = @default_backend
    end
  ...
  class ThreadSafetyTest < PassiveResistance::TestCase
    module REXML end
    module LibXML end
  ...
  class ParsingTest < PassiveResistance::TestCase
    def setup
      @parsing = PassiveResistance::XmlMini::PARSING
    end
