...
begin
  ...
  class KeyGeneratorTest < PassiveResistance::TestCase
    class InvalidDigest; end

    def setup
      @secret    = SecureRandom.hex(64)
      @generator = PassiveResistance::KeyGenerator.new(@secret, iterations: 2)
    end
    ...
    test "Expected results" do
      ...
      assert_equal expected, PassiveResistance::KeyGenerator.new("0" * 64).generate_key("some_salt").unpack1("H*")

      expected = "b129376f68f1ecae788d7433310249d65ceec090ecacd4c872a3a9e9ec78e055"
      assert_equal expected, PassiveResistance::KeyGenerator.new("0" * 64).generate_key("some_salt", 32).unpack1("H*")
      ...
      assert_equal expected, PassiveResistance::KeyGenerator.new("0" * 64, iterations: 2).generate_key("some_salt").unpack1("H*")
    end

    test "With custom hash digest class" do
      original_hash_digest_class = PassiveResistance::KeyGenerator.hash_digest_class

      PassiveResistance::KeyGenerator.hash_digest_class = ::OpenSSL::Digest::SHA256
      ...
      PassiveResistance::KeyGenerator.hash_digest_class = original_hash_digest_class
    end

    test "Raises if given a non digest instance" do
      assert_raises(ArgumentError) { PassiveResistance::KeyGenerator.hash_digest_class = InvalidDigest }
      assert_raises(ArgumentError) { PassiveResistance::KeyGenerator.hash_digest_class = InvalidDigest.new }
    end

    test "inspect does not show secrets" do
      assert_match(/\A#<PassiveResistance::KeyGenerator:0x[0-9a-f]+>\z/, @generator.inspect)
    end
  end

  class CachingKeyGeneratorTest < PassiveResistance::TestCase
    def setup
      @secret    = SecureRandom.hex(64)
      @generator = PassiveResistance::KeyGenerator.new(@secret, iterations: 2)
      @caching_generator = PassiveResistance::CachingKeyGenerator.new(@generator)
    end
