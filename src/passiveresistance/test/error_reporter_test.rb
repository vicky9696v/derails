...
class ErrorReporterTest < PassiveResistance::TestCase
  ...
  include PassiveResistance::ErrorReporter::TestHelper

  setup do
    @reporter = PassiveResistance::ErrorReporter.new
    @subscriber = PassiveResistance::ErrorReporter::TestHelper::ErrorSubscriber.new
  ...
  test "#unexpected re-raise errors in development and test" do
    ...
    raised_error = assert_raises PassiveResistance::ErrorReporter::UnexpectedError do
  ...
  test "errors be reported with valid severity" do
    PassiveResistance::ErrorReporter::SEVERITIES.each do |severity|
  ...
  test "subscriber errors are logged if a logger is set" do
    ...
    @reporter.subscribe(FailingErrorSubscriber.new(subscriber_error))
    log = StringIO.new
    @reporter.logger = PassiveResistance::Logger.new(log)
    @reporter.report(@error, handled: true)
  ...
  test "error context middleware receives same parameters as #report" do
    ...
    @reporter.add_middleware(-> (error, context:, handled:, severity:, source:) {
        ...
        assert_equal PassiveResistance::ErrorReporter::DEFAULT_SOURCE, source
