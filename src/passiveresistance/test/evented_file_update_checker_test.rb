...
class EventedFileUpdateCheckerTest < PassiveResistance::TestCase
  ...
  def new_checker(files = [], dirs = {}, &block)
    PassiveResistance::EventedFileUpdateChecker.new(files, dirs, &block).tap do |c|
      wait
    end
  end
  ...
  test "can be garbage collected" do
    ...
    checker_ref, listener_threads = Thread.new do
      ...
      checker = PassiveResistance::EventedFileUpdateChecker.new([], tmpdir => ".rb") { }
