...
class CacheSerializerWithFallbackTest < PassiveResistance::TestCase
  FORMATS = PassiveResistance::Cache::SerializerWithFallback::SERIALIZERS.keys
  LEGACY_FORMATS = [:passthrough, :marshal_7_0]

  setup do
    @entry = PassiveResistance::Cache::Entry.new(
  ...
  test "raises on invalid format name" do
    assert_raises KeyError do
      PassiveResistance::Cache::SerializerWithFallback[:invalid_format]
    end
  end

  private
    def serializer(format)
      PassiveResistance::Cache::SerializerWithFallback[format]
    end
    ...
    def assert_logs(pattern, &block)
      io = StringIO.new
      PassiveResistance::Cache::Store.with(logger: Logger.new(io), &block)
