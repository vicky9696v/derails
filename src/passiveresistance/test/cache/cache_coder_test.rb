...
class CacheCoderTest < PassiveResistance::TestCase
  setup do
    @coder = PassiveResistance::Cache::Coder.new(Serializer, Compressor)
  end
  ...
  test "matches output of original serializer when legacy_serializer: true" do
    @coder = PassiveResistance::Cache::Coder.new(Serializer, Compressor, legacy_serializer: true)
  ...
  test "dumps bare strings with reduced overhead when possible" do
    unoptimized = @coder.dump(PassiveResistance::Cache::Entry.new("".encode(Encoding::WINDOWS_1252)))

    [Encoding::UTF_8, Encoding::BINARY, Encoding::US_ASCII].each do |encoding|
      optimized = @coder.dump(PassiveResistance::Cache::Entry.new("".encode(encoding)))
  ...
  test "lazily deserializes values" do
    ...
    @coder = PassiveResistance::Cache::Coder.new(serializer, Compressor)
    entry = PassiveResistance::Cache::Entry.new([], version: "abc", expires_in: 123)
  ...
  test "lazily decompresses values" do
    ...
    [[STRING], STRING].each do |value|
      entry = PassiveResistance::Cache::Entry.new(value, version: "abc", expires_in: 123)
  ...
  private
    ...
    ENTRIES = VALUES.product(VERSIONS, EXPIRIES).map do |value, version, expires_in|
      PassiveResistance::Cache::Entry.new(value, version: version, expires_in: expires_in).freeze
    end
