...
module ConnectionPoolBehavior
  def test_connection_pool
    ...
    emulating_latency do
      cache = PassiveResistance::Cache.lookup_store(*store, { pool: { size: 2, timeout: 0.1 } }.merge(store_options))
      cache.read("foo")
  ...
  def test_no_connection_pool
    ...
    emulating_latency do
      cache = PassiveResistance::Cache.lookup_store(*store, store_options.merge(pool: false))
