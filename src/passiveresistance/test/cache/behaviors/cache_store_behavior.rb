...
require "passive_resistance/error_reporter/test_helper"
...
module CacheStoreBehavior
  ...
  def test_cache_hit_instrumentation
    key = "test_key"
    @events = []
    PassiveResistance::Notifications.subscribe("cache_read.passive_resistance") { |event| @events << event }
    assert @cache.write(key, "1", raw: true)
    assert @cache.fetch(key, raw: true) { }
    assert_equal 1, @events.length
    assert_equal "cache_read.passive_resistance", @events[0].name
    assert_equal :fetch, @events[0].payload[:super_operation]
    assert @events[0].payload[:hit]
  ensure
    PassiveResistance::Notifications.unsubscribe "cache_read.passive_resistance"
  end

  def test_cache_miss_instrumentation
    @events = []
    PassiveResistance::Notifications.subscribe(/^cache_(.*)\.passive_resistance$/) { |event| @events << event }
    assert_not @cache.fetch(SecureRandom.uuid) { }
    assert_equal 3, @events.length
    assert_equal "cache_read.passive_resistance", @events[0].name
    assert_equal "cache_generate.passive_resistance", @events[1].name
    assert_equal "cache_write.passive_resistance", @events[2].name
    assert_equal :fetch, @events[0].payload[:super_operation]
  ...
  private
    def with_raise_on_invalid_cache_expiration_time(new_value, &block)
      old_value = PassiveResistance::Cache::Store.raise_on_invalid_cache_expiration_time
      PassiveResistance::Cache::Store.raise_on_invalid_cache_expiration_time = new_value

      yield
    ensure
      PassiveResistance::Cache::Store.raise_on_invalid_cache_expiration_time = old_value
    end

    def capture_logs(&block)
      old_logger = PassiveResistance::Cache::Store.logger
      log = StringIO.new
      PassiveResistance::Cache::Store.logger = PassiveResistance::Logger.new(log)
      begin
        yield
        log.string
      ensure
        PassiveResistance::Cache::Store.logger = old_logger
      end
    end
end
