...
class StoreTest < PassiveResistance::TestCase
  def lookup_store(options = {})
    PassiveResistance::Cache.lookup_store(:memory_store, options)
  end
end
...
class MemoryStoreTest < StoreTest
  ...
  def test_cleanup_instrumentation
    ...
    assert_notification("cache_cleanup.passive_resistance", size: size, store: @cache.class.name) do
      @cache.cleanup
    end
  end
...
class MemoryStorePruningTest < StoreTest
  def setup
    @record_size = PassiveResistance::Cache.lookup_store(:memory_store).send(:cached_size, 1, PassiveResistance::Cache::Entry.new("aaaaaaaaaa"))
    @cache = PassiveResistance::Cache.lookup_store(:memory_store, expires_in: 60, size: @record_size * 10 + 1)
  end
