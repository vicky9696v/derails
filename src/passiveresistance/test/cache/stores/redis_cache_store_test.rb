...
require "passive_resistance/cache/redis_cache_store"
...
module PassiveResistance::Cache::RedisCacheStoreTests
  ...
  class LookupTest < PassiveResistance::TestCase
    test "may be looked up as :redis_cache_store" do
      assert_kind_of PassiveResistance::Cache::RedisCacheStore,
        PassiveResistance::Cache.lookup_store(:redis_cache_store)
    end
  end

  class InitializationTest < PassiveResistance::TestCase
    ...
    private
      def build(**kwargs)
        PassiveResistance::Cache::RedisCacheStore.new(pool: false, **kwargs).tap(&:redis)
      end
  end

  class StoreTest < PassiveResistance::TestCase
    setup do
      @cache = nil
    ...
    def lookup_store(options = {})
      PassiveResistance::Cache.lookup_store(:redis_cache_store, { timeout: 0.1, namespace: @namespace, pool: false }.merge(options))
    end
  ...
  class RedisCacheStoreCommonBehaviorTest < StoreTest
    ...
    test "skip_nil is passed to PassiveResistance::Cache" do
  ...
  class ConnectionPoolBehaviorTest < StoreTest
    ...
    def test_pool_options_work
      cache = PassiveResistance::Cache.lookup_store(:redis_cache_store, pool: { size: 2, timeout: 1 })
    ...
    def test_connection_pooling_by_default
      cache = PassiveResistance::Cache.lookup_store(:redis_cache_store)
      pool = cache.redis
      assert_kind_of ::ConnectionPool, pool
    ...
    def test_no_connection_pooling_by_default_when_already_a_pool
      ...
      cache = PassiveResistance::Cache.lookup_store(:redis_cache_store, redis: redis)
      pool = cache.redis
      assert_kind_of ::ConnectionPool, pool
  ...
  class FailureRaisingFromUnavailableClientTest < StoreTest
    ...
    private
      def emulating_unavailability
        ...
        yield PassiveResistance::Cache::RedisCacheStore.new(namespace: @namespace,
  ...
  class ClearTest < StoreTest
    ...
    test "clear all cache key with Redis::Distributed" do
      cache = PassiveResistance::Cache::RedisCacheStore.new(
        url: REDIS_URLS,
  ...
  class RawTest < StoreTest
    test "does not compress values read with \"raw\" enabled" do
      ...
      assert_not_called_on_instance_of PassiveResistance::Cache::Entry, :compressed do
