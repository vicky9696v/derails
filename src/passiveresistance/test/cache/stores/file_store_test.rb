...
class FileStoreTest < PassiveResistance::TestCase
  attr_reader :cache_dir

  def lookup_store(options = {})
    cache_dir = options.delete(:cache_dir) { @cache_dir }
    PassiveResistance::Cache.lookup_store(:file_store, cache_dir, options)
  end
  ...
  def setup
    ...
    @buffer = StringIO.new
    @cache.logger = PassiveResistance::Logger.new(@buffer)
  end
  ...
  def test_filename_max_size
    key = "#{'A' * PassiveResistance::Cache::FileStore::FILENAME_MAX_SIZE}"
  ...
  def test_key_transformation_max_filename_size
    key = "#{'A' * PassiveResistance::Cache::FileStore::FILENAME_MAX_SIZE}B"
    path = @cache.send(:normalize_key, key, {})
    assert path.split("/").all? { |dir_name| dir_name.size <= PassiveResistance::Cache::FileStore::FILENAME_MAX_SIZE }
    assert_equal "B", File.basename(path)
  end

  def test_delete_matched_when_key_exceeds_max_filename_size
    submaximal_key = "_" * (PassiveResistance::Cache::FileStore::FILENAME_MAX_SIZE - 1)
  ...
  def test_delete_matched_when_cache_directory_does_not_exist
    assert_nothing_raised do
      PassiveResistance::Cache::FileStore.new("/test/cache/directory").delete_matched(/does_not_exist/)
    end
  end

  def test_delete_does_not_delete_empty_parent_dir
    sub_cache_dir = File.join(cache_dir, "subdir/")
    sub_cache_store = PassiveResistance::Cache::FileStore.new(sub_cache_dir)
    assert_nothing_raised do
  ...
  def test_cleanup_when_non_passive_resistance_cache_file_exists
