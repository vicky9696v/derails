...
class CurrentAttributesTest < PassiveResistance::TestCase
  ...
  class Current < PassiveResistance::CurrentAttributes
  ...
  class Session < PassiveResistance::CurrentAttributes
  ...
  test "CurrentAttributes defaults do not leak between classes" do
    Class.new(PassiveResistance::CurrentAttributes) { attribute :counter_integer, default: 100 }
    Current.reset
  ...
  test "CurrentAttributes use fiber-local variables" do
    previous_level = PassiveResistance::IsolatedExecutionState.isolation_level
    ...
    PassiveResistance::IsolatedExecutionState.isolation_level = previous_level
  end
  ...
  test "CurrentAttributes restricted attribute names" do
    assert_raises ArgumentError, match: /Restricted attribute names: reset, set/ do
      class InvalidAttributeNames < PassiveResistance::CurrentAttributes
  ...
  test "method_added hook doesn't reach the instance. Fix for #54646" do
    current = Class.new(PassiveResistance::CurrentAttributes) do
      def self.name
        "MyCurrent"
      end
  ...
  test "set and restore attributes when re-entering the executor" do
    PassiveResistance::ExecutionContext.with(nestable: true) do
      # simulate executor hooks from passive_resistance/railtie.rb
      executor = Class.new(PassiveResistance::Executor)
      executor.to_run do
