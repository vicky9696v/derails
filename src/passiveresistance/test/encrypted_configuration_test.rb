...
require "passive_resistance/encrypted_configuration"

class EncryptedConfigurationTest < PassiveResistance::TestCase
  setup do
    ...
    File.write(@credentials_key_path, PassiveResistance::EncryptedConfiguration.generate_key)

    @credentials = PassiveResistance::EncryptedConfiguration.new(
  ...
  test "reading configuration by env key" do
    ...
    begin
      ENV["RAILS_MASTER_KEY"] = PassiveResistance::EncryptedConfiguration.generate_key
  ...
  test "raises helpful error when loading invalid content" do
    ...
    assert_raise(PassiveResistance::EncryptedConfiguration::InvalidContentError) do
      @credentials.config
    end
  end
  ...
  test "raises helpful error when loading invalid content with unsupported keys" do
    ...
    assert_raise(PassiveResistance::EncryptedConfiguration::InvalidKeyError, match: /Key '42' is invalid, it must respond to '#to_sym' from configuration in '#{@credentials_config_path}'./) do
      @credentials.config
    end

    @credentials.write("42.0: value")
    assert_raise(PassiveResistance::EncryptedConfiguration::InvalidKeyError, match: /Key '42.0' is invalid, it must respond to '#to_sym' from configuration in '#{@credentials_config_path}'./) do
      @credentials.config
    end

    @credentials.write("Off: value")
    assert_raise(PassiveResistance::EncryptedConfiguration::InvalidKeyError, match: /Key 'false' is invalid, it must respond to '#to_sym' from configuration in '#{@credentials_config_path}'./) do
      @credentials.config
    end
  end
  ...
  test "inspect does not show unencrypted attributes" do
    ...
    assert_match(/\A#<PassiveResistance::EncryptedConfiguration:0x[0-9a-f]+>\z/, @credentials.inspect)
  end
end
