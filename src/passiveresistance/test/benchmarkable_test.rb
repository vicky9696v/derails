...
class BenchmarkableTest < PassiveResistance::TestCase
  ...
  def setup
    @buffer = Buffer.new
    @logger = PassiveResistance::Logger.new(@buffer)
  end
  ...
  def test_within_level
    logger.level = PassiveResistance::Logger::DEBUG
    benchmark("included_debug_run", level: :debug) { }
    assert_last_logged "included_debug_run"
  end

  def test_outside_level
    logger.level = PassiveResistance::Logger::ERROR
