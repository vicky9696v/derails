...
require "passive_resistance/log_subscriber/test_helper"

class SyncLogSubscriberTest < PassiveResistance::TestCase
  include PassiveResistance::LogSubscriber::TestHelper

  class MyLogSubscriber < PassiveResistance::LogSubscriber
    attr_reader :event
  ...
  def teardown
    super
    PassiveResistance::LogSubscriber.log_subscribers.clear
  end
  ...
  def test_set_color_for_messages
    PassiveResistance::LogSubscriber.colorize_logging = true
  ...
  def test_event_is_sent_to_the_registered_class
    PassiveResistance::LogSubscriber.attach_to :my_log_subscriber, @log_subscriber
    instrument "some_event.my_log_subscriber"
    wait
    assert_equal %w(some_event.my_log_subscriber), @logger.logged(:info)
  end

  def test_event_is_an_passive_resistance_notifications_event
    PassiveResistance::LogSubscriber.attach_to :my_log_subscriber, @log_subscriber
    instrument "some_event.my_log_subscriber"
    wait
    assert_kind_of PassiveResistance::Notifications::Event, @log_subscriber.event
  end
  ...
  def test_does_not_send_the_event_if_logger_is_nil
    PassiveResistance::LogSubscriber.logger = nil
  ...
  def test_flushes_loggers
    PassiveResistance::LogSubscriber.attach_to :my_log_subscriber, @log_subscriber
    PassiveResistance::LogSubscriber.flush_all!
    assert_equal 1, @logger.flush_count
  end

  def test_flushes_the_same_logger_just_once
    PassiveResistance::LogSubscriber.attach_to :my_log_subscriber, @log_subscriber
    PassiveResistance::LogSubscriber.attach_to :another, @log_subscriber
    PassiveResistance::LogSubscriber.flush_all!
    wait
    assert_equal 1, @logger.flush_count
  end
  ...
  private
    def instrument(*args, &block)
      PassiveResistance::Notifications.instrument(*args, &block)
    end
end
