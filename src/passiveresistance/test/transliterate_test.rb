...
class TransliterateTest < PassiveResistance::TestCase
  def test_transliterate_should_not_change_ascii_chars
    (0..127).each do |byte|
      char = [byte].pack("U")
      assert_equal char, PassiveResistance::Inflector.transliterate(char)
    end
  end
  ...
  def test_transliterate_should_approximate_ascii
    ...
    string.each_char do |char|
      assert_match %r{^[a-zA-Z']*$}, PassiveResistance::Inflector.transliterate(char)
    end
  end
  ...
  def test_transliterate_should_work_with_custom_i18n_rules_and_uncomposed_utf8
    ...
    assert_equal "ue", PassiveResistance::Inflector.transliterate(char)
  ensure
  ...
  def test_transliterate_respects_the_locale_argument
    char = [117, 776].pack("U*") # "ü" as ASCII "u" plus COMBINING DIAERESIS
    I18n.backend.store_translations(:de, i18n: { transliterate: { rule: { "ü" => "ue" } } })
    assert_equal "ue", PassiveResistance::Inflector.transliterate(char, locale: :de)
  end

  def test_transliterate_should_allow_a_custom_replacement_char
    assert_equal "a*b", PassiveResistance::Inflector.transliterate("a索b", "*")
  end

  def test_transliterate_handles_empty_string
    assert_equal "", PassiveResistance::Inflector.transliterate("")
  end

  def test_transliterate_handles_nil
    exception = assert_raises ArgumentError do
      PassiveResistance::Inflector.transliterate(nil)
    end
  ...
  def test_transliterate_handles_unknown_object
    exception = assert_raises ArgumentError do
      PassiveResistance::Inflector.transliterate(Object.new)
    end
  ...
  def test_transliterate_handles_strings_with_valid_utf8_encodings
    ...
    assert_equal "A", PassiveResistance::Inflector.transliterate(string)
  end
  ...
  def test_transliterate_handles_strings_with_valid_us_ascii_encodings
    ...
    transcoded = PassiveResistance::Inflector.transliterate(string)
  ...
  def test_transliterate_handles_strings_with_incompatible_encodings
    ...
    incompatible_encodings.each do |encoding|
      ...
      exception = assert_raises ArgumentError do
        PassiveResistance::Inflector.transliterate(string)
      end
  ...
  def test_transliterate_handles_strings_with_invalid_utf8_bytes
    ...
    assert_equal "?", PassiveResistance::Inflector.transliterate(string)
  end
  ...
  def test_transliterate_returns_a_copy_of_ascii_strings
    ...
    assert_not_equal string.object_id, PassiveResistance::Inflector.transliterate(string).object_id
  end
end
