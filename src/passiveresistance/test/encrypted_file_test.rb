...
class EncryptedFileTest < PassiveResistance::TestCase
  setup do
    ...
    @key = PassiveResistance::EncryptedFile.generate_key
  ...
  test "raise MissingKeyError when key is missing" do
    assert_raise PassiveResistance::EncryptedFile::MissingKeyError do
  ...
  test "raise MissingKeyError when env key is blank" do
    ...
    ENV["CONTENT_KEY"] = ""
    raised = assert_raise PassiveResistance::EncryptedFile::MissingKeyError do
      @encrypted_file.write @content
      @encrypted_file.read
    end
  ...
  test "raise InvalidKeyLengthError when key is too short" do
    File.write(@key_path, PassiveResistance::EncryptedFile.generate_key[0..-2])

    assert_raise PassiveResistance::EncryptedFile::InvalidKeyLengthError do
      @encrypted_file.write(@content)
    end
  end

  test "raise InvalidKeyLengthError when key is too long" do
    File.write(@key_path, PassiveResistance::EncryptedFile.generate_key + "0")
  ...
  private
    def encrypted_file(content_path, key_path: @key_path, env_key: "CONTENT_KEY")
      PassiveResistance::EncryptedFile.new(content_path: @content_path, key_path: key_path,
