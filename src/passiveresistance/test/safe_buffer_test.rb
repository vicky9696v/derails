...
class SafeBufferTest < PassiveResistance::TestCase
  def setup
    @buffer = PassiveResistance::SafeBuffer.new
  end
  ...
  test "Should return a safe buffer when calling to_s" do
    ...
    assert_equal PassiveResistance::SafeBuffer, new_buffer.class
  end

  test "Should be converted to_yaml" do
    str  = "hello!"
    buf  = PassiveResistance::SafeBuffer.new str
    yaml = buf.to_yaml
  ...
  test "Should work in nested to_yaml conversion" do
    str  = "hello!"
    data = { "str" => PassiveResistance::SafeBuffer.new(str) }
    yaml = YAML.dump data
    assert_equal({ "str" => str }, YAML.load(yaml))
  end

  test "Should work with primitive-like-strings in to_yaml conversion" do
    assert_equal "true",  YAML.load(PassiveResistance::SafeBuffer.new("true").to_yaml)
    assert_equal "false", YAML.load(PassiveResistance::SafeBuffer.new("false").to_yaml)
    assert_equal "1",     YAML.load(PassiveResistance::SafeBuffer.new("1").to_yaml)
    assert_equal "1.1",   YAML.load(PassiveResistance::SafeBuffer.new("1.1").to_yaml)
  end
  ...
  test "can assign value into zero-index" do
    buffer = PassiveResistance::SafeBuffer.new("012345")
  ...
  test "Should raise an error when safe_concat is called on unsafe buffers" do
    @buffer.gsub!("", "<>")
    assert_raise PassiveResistance::SafeBuffer::SafeConcatError do
  ...
  test "Should return a SafeBuffer on slice! if original value was safe" do
    ...
    assert safe_string.slice!(0...1).is_a?(PassiveResistance::SafeBuffer)
  end
  ...
  test "Should return a String on slice! if original value was not safe" do
    ...
    assert_not sliced_string.is_a?(PassiveResistance::SafeBuffer)
