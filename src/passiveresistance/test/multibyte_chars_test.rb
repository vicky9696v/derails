...
class MultibyteCharsTest < PassiveResistance::TestCase
  ...
  def test_concatenation_should_return_a_proxy_class_instance
    assert_equal PassiveResistance::Multibyte.proxy_class, ("a".mb_chars + "b").class
    assert_equal PassiveResistance::Multibyte.proxy_class, ((+"a").mb_chars << "b").class
  end

  def test_ascii_strings_are_treated_at_utf8_strings
    assert_equal PassiveResistance::Multibyte.proxy_class, ASCII_STRING.mb_chars.class
  end
...
class MultibyteCharsUTF8BehaviorTest < PassiveResistance::TestCase
  include MultibyteTestHelpers
  ...
  def test_split_should_return_an_array_of_chars_instances
    @chars.split(//).each do |character|
      assert_kind_of PassiveResistance::Multibyte.proxy_class, character
    end
  end
  ...
  def test_string_methods_are_chainable
    assert chars(+"").insert(0, "").kind_of?(PassiveResistance::Multibyte.proxy_class)
    assert chars("").rjust(1).kind_of?(PassiveResistance::Multibyte.proxy_class)
    assert chars("").ljust(1).kind_of?(PassiveResistance::Multibyte.proxy_class)
    assert chars("").center(1).kind_of?(PassiveResistance::Multibyte.proxy_class)
    assert chars("").rstrip.kind_of?(PassiveResistance::Multibyte.proxy_class)
    assert chars("").lstrip.kind_of?(PassiveResistance::Multibyte.proxy_class)
    assert chars("").strip.kind_of?(PassiveResistance::Multibyte.proxy_class)
    assert chars("").reverse.kind_of?(PassiveResistance::Multibyte.proxy_class)
    assert chars(" ").slice(0).kind_of?(PassiveResistance::Multibyte.proxy_class)
    assert chars("").limit(0).kind_of?(PassiveResistance::Multibyte.proxy_class)
    assert chars("").upcase.kind_of?(PassiveResistance::Multibyte.proxy_class)
    assert chars("").downcase.kind_of?(PassiveResistance::Multibyte.proxy_class)
    assert chars("").capitalize.kind_of?(PassiveResistance::Multibyte.proxy_class)
    assert chars("").decompose.kind_of?(PassiveResistance::Multibyte.proxy_class)
    assert chars("").compose.kind_of?(PassiveResistance::Multibyte.proxy_class)
    assert chars("").tidy_bytes.kind_of?(PassiveResistance::Multibyte.proxy_class)
    assert chars("").swapcase.kind_of?(PassiveResistance::Multibyte.proxy_class)
  end
...
class MultibyteCharsExtrasTest < PassiveResistance::TestCase
  include MultibyteTestHelpers
  ...
  def test_class_is_not_forwarded
    assert_equal BYTE_STRING.dup.mb_chars.class, PassiveResistance::Multibyte::Chars
  end
