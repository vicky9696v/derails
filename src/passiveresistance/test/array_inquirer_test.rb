...
class ArrayInquirerTest < PassiveResistance::TestCase
  def setup
    @array_inquirer = PassiveResistance::ArrayInquirer.new([:mobile, :tablet, "api"])
  end
  ...
  def test_inquiry
    ...
    assert_instance_of PassiveResistance::ArrayInquirer, result
  ...
  def test_respond_to_fallback_to_array_respond_to
    ...
    arr = PassiveResistance::ArrayInquirer.new([:x])
