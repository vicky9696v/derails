...
module Notifications
  ...
  private
    def event(*args)
      PassiveResistance::Notifications::Event.new(*args)
    end
  end
  ...
  class SubscribeEventObjectsTest < TestCase
    def test_subscribe_events
      ...
      PassiveResistance::Notifications.instrument("foo")
      event = events.first
    ...
    def test_subscribe_to_events_where_payload_is_changed_during_instrumentation
      ...
      PassiveResistance::Notifications.instrument("foo") do |payload|
    ...
    def test_subscribe_to_events_can_handle_nested_hashes_in_the_paylaod
      ...
      PassiveResistance::Notifications.instrument("foo", some_key: { key_one: "success!" }) do |payload|
    ...
    def test_subscribe_via_top_level_api
      old_notifier = PassiveResistance::Notifications.notifier
      PassiveResistance::Notifications.notifier = PassiveResistance::Notifications::Fanout.new

      event = nil
      PassiveResistance::Notifications.subscribe("foo") do |e|
        event = e
      end

      PassiveResistance::Notifications.instrument("foo") do
    ...
    def test_subscribe_with_a_single_arity_lambda_listener
      ...
      PassiveResistance::Notifications.instrument("event_name")
  ...
  class TimedAndMonotonicTimedSubscriberTest < TestCase
    def test_subscribe
      ...
      PassiveResistance::Notifications.subscribe(event_name) do |name, started, finished, unique_id, data|
    ...
    def test_monotonic_subscribe
      ...
      PassiveResistance::Notifications.monotonic_subscribe(event_name) do |name, started, finished, unique_id, data|
        class_of_started = started.class
        class_of_finished = finished.class
      end
  ...
  class BuildHandleTest < TestCase
    ...
    def test_subscribed_interleaved_with_event
      ...
      PassiveResistance::Notifications.subscribed(callback1, name) do
        handle = instrumenter.build_handle(name, {})
        handle.start

        PassiveResistance::Notifications.subscribed(callback2, name) do
          handle.finish
        end
      end
  ...
  class SubscribedTest < TestCase
    def test_subscribed
      ...
      PassiveResistance::Notifications.subscribed(callback, name) do
        PassiveResistance::Notifications.instrument(name)
        PassiveResistance::Notifications.instrument(name2)
        PassiveResistance::Notifications.instrument(name)
      end
    ...
    def test_subscribed_all_messages
      ...
      events   = []
      callback = lambda { |*_| events << _.first }
      PassiveResistance::Notifications.subscribed(callback) do
        PassiveResistance::Notifications.instrument(name)
        PassiveResistance::Notifications.instrument(name2)
        PassiveResistance::Notifications.instrument(name)
      end
    ...
    def test_subscribing_to_instrumentation_while_inside_it
      ...
      PassiveResistance::Notifications.subscribe("foo", TestSubscriber.new)

      assert_nothing_raised do
        PassiveResistance::Notifications.instrument("foo") do
          PassiveResistance::Notifications.subscribe("foo") { }
        end
      end
    ensure
      PassiveResistance::Notifications.notifier = old_notifier
    ...
    def test_timed_subscribed
      ...
      PassiveResistance::Notifications.subscribed(callback, event_name) do
        PassiveResistance::Notifications.instrument(event_name)
      end
    ...
    def test_monotonic_timed_subscribed
      ...
      PassiveResistance::Notifications.subscribed(callback, event_name, monotonic: true) do
        PassiveResistance::Notifications.instrument(event_name)
      end
  ...
  class InspectTest < TestCase
    def test_inspect_output_is_small
      expected = "#<PassiveResistance::Notifications::Fanout (2 patterns)>"
  ...
  class InstrumentationTest < TestCase
    delegate :instrument, to: PassiveResistance::Notifications
    ...
    def test_instrumenter_exposes_its_id
      assert_equal 20, PassiveResistance::Notifications.instrumenter.id.size
    end
