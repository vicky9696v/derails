...
module PassiveResistance
  ...
  #
  # +PassiveResistance::EventReporter+ provides an interface for reporting structured events to subscribers.
  #
  ...
  #
  # A custom context store can be configured via +config.passive_resistance.event_reporter_context_store+.
  #
  #     # config/application.rb
  #     config.passive_resistance.event_reporter_context_store = CustomContextStore
  #
  ...
  #
  # If an {event object}[rdoc-ref:EventReporter@Event+Objects] is given instead, subscribers will need to filter sensitive data themselves, e.g. with PassiveResistance::ParameterFilter.
  class EventReporter
    ...
    def notify(name_or_object, payload = nil, caller_depth: 1, **kwargs)
      ...
      @subscribers.each do |subscriber_entry|
        ...
        if raise_on_error?
          ...
          PassiveResistance.error_reporter.report(subscriber_error, handled: true)
        end
      end
    ...
    private
      ...
      def payload_filter
        @payload_filter ||= begin
          mask = PassiveResistance::ParameterFilter::FILTERED
          PassiveResistance::ParameterFilter.new(PassiveResistance.filter_parameters, mask: mask)
        end
      end
      ...
      def handle_unexpected_args(name_or_object, payload, kwargs)
        ...
        if raise_on_error?
          ...
          PassiveResistance.error_reporter.report(ArgumentError.new(message), handled: true)
        end
      end
  end
end
